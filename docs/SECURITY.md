...
- `T1-APP-STORAGE-POLICY` 已交付纯 JVM 层的存储路由：六类受保护数据只落在经该原语验证并保存的 CE/no-backup 根下、每次检查后的类别目录；DP 环境先转换为 CE，转换失败或仍为 DP 即拒绝。拒绝消息固定、不带 cause，端口的普通异常不外泄路径。大媒体只用 app-specific external 端口，卷缺失/未挂载/只读/不可写/探针失败均为 Unavailable，不回退共享存储。真实 Android 端口由 `T1-APP-STORAGE-ANDROID`（PR #351）交付：标记与各根取自所包装 context 的 getter，卷状态、可写性与空间只依据给定目录，DP→CE 转换失败为固定消息且无 cause；debug-only 设备探针只对持 DUMP 权限的调用方导出，回执不含路径与文件内容，在 API 33 真机与 API 35 模拟器上验证，见 `docs/storage-android-probe.md`。
...
- `T1-LOCAL-SECRET-BOX`（[PR #371](https://github.com/Asun28/MyInspection/pull/371)）已交付 `LocalSecretBox` 的纯 JVM 核心：信封只含版本字节、提供方生成的 96 位 nonce 与带 128 位 tag 的密文；alias 与 AAD 都绑定 purpose 和版本，跨 purpose 或跨版本的信封认证失败；每个失败映射为可重试的 NEEDS_UNLOCK 或带封闭原因的 NEEDS_PASSPHRASE，不返回明文、open 不写信封；明文字节缓冲使用后清零。key、解锁状态与信封文件经端口注入。信封文件存储由 `T1-LOCAL-SECRET-STORE`（[PR #377](https://github.com/Asun28/MyInspection/pull/377)）交付：每次调用都经 `AppStoragePolicy` 取 no-backup 信封目录，替换为同目录临时文件写入、sync 后原子移动，失败只抛固定消息、无 cause 的异常并带封闭阶段码，不含路径。AndroidKeyStore 适配由 `T1-LOCAL-DATA-SECURITY`（[PR #394](https://github.com/Asun28/MyInspection/pull/394)）交付：key 为 AndroidKeyStore 中不可导出的 AES-256-GCM key，要求随机化加密，不要求用户认证，也不设 unlocked-device-required，使后台备份在首次解锁后可解封；设备已解锁时，无法读取、不是对称密钥或无法加密的 alias 条目在 seal 时替换。SM-A346E（API 33，key 在 TEE 内）与 API 35 模拟器上的 debug 探针自验证通过；锁屏移除、凭据清除与 key 失效的真机证据仍归 `T7-SMOKE-POLISH`。
...
- 本机健康状态只从 typed events、保留期外仍存的本地 projection 与权威回执派生；因果顺序只看 UUIDv7-keyed counter 分配的唯一单调序列（不是主键），时间戳只用于显示/年龄。operation/reason/context 封闭码、单位、outcome 约束及 `FINALIZE_FAILED / PDF_FAILED / BACKUP_LAST_FAILED / BACKUP_STALE_7D / BACKUP_FAILED_3X / INTEGRITY_FAILED / RESTORE_FAILED / RESTORE_ROLLED_BACK / PREVIOUS_CRASH / STARTUP_SLOW` 的精确派生/清除规则以 `docs/DATABASE-DESIGN.md`“Diagnostic registry v1”为唯一注册表。来源变化后 1 秒内显示一个可操作动作。它不是远程监控；v1 不自动遥测、Wi-Fi 上传或发送远程告警。
