...
`scaffold-selftest` 不进 PR 必需检查；默认分支 push 仅覆盖脚手架权威面，排除产品 `configs/compliance/**`，保留 `configs/licenses/**`、`configs/secrets/**` 与手动触发。Windows/Ubuntu 各跑 core、workflow 与三个 seeded 子片（共 10 jobs）；三子片并集仍是完整闸 17，wall time 取最慢片。产品卡使用相关产品测试 + verify + R3；脚手架卡可用聚焦 fixture 迭代，最终验收仍按卡执行。
