...
`allow_paths` 的条目数只在建卡期提示共享面与所有权；目录前缀可覆盖大量文件，单个脚本也可产生巨大 diff，因此它不是规模证明。实现后的真实 diff 预算才是 push/R3 的硬闸。超限卡必须拆成有依赖的 1→N 卡，不能扩 `allow_paths` 或提高 CLI 参数绕过；`MaxChangedLines`/`MaxDiffChars` 只允许临时收紧。
