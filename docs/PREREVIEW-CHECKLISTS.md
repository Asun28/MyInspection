...
- C3 Negative case in the sibling file: for each new guard, the unit's test file (the existing one for an existing unit, not a parallel new file) holds a case that reaches the guard with a parseable-but-wrong input (lesson:L165, rubric:11).
...
- C1 Acceptance rows and `dod_assert` are declarations that need evidence: each row marked automated names a test and a mutant present in this diff; a "covered" or "verified" phrase that can point to no test, mutant or receipt is a finding (lesson:L317).
