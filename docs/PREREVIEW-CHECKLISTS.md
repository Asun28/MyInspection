...
- C2 Empty, null, zero, negative and overflow inputs: walk each parameter and each arithmetic step with the empty collection, null, 0, a negative value and the type's MAX; a bound or containment check computed without saturation or a wider sum can wrap and pass (lesson:L228).
- C2 Fail-open branches: a classification, completeness or validation gate derives its universe from the contract source, enumerates exhaustively and treats an unclassified value as a defect; a default branch, a swallowed exception or a universe derived from the data under check is fail-open (lesson:L228, rubric:9).
- C2 Ordering assumptions: a collection that feeds a hash, a receipt, a canonical serialisation or a persisted order has a total order stated in code; iterating a set or map without one is hidden non-determinism (rubric:10).
...
- C3 A test that goes red without this change: name the test in the unit's existing test file that fails when the change is reverted; none, or one that lives in a source set no task runs, is missing evidence (rubric:6, lesson:L281, lesson:L280).
...
- C3 Assertion face equals the contract: an assertion over whole stdout, over localised text, over a keyword count or over "any non-zero exit" stays green while the contract is absent; compare the verdict line, match ASCII sentinels, and make the negative case reach the guarded statement (lesson:L165).
- C3 One mutant per assertion: each assertion names a single-point mutation that only it catches; an earlier assertion in short-circuit order shields a later one, and a mutant's expected failure code is anchored, not a bare substring (lesson:L225).
...
- C3 Expected values come from the contract: an expected value computed by calling the production function follows the mutation and stays green; one transcribed from the implementation's current output encodes its defects as expected (lesson:L165, rubric:6).
...
- C3 Negative case in the sibling file: for each new guard, the existing test file of that unit (not a parallel new file) holds a case that reaches the guard with a parseable-but-wrong input (lesson:L165, rubric:11).
...
- C1 Acceptance rows and `dod_assert` are declarations that need evidence: each row marked automated names a test and a mutant present in this diff; a "covered" or "verified" phrase without them is a finding (lesson:L317).
...
- C2 Every error branch open or closed: for each catch, each `-ErrorAction`, each empty-result path and each native call, state whether the branch fails closed (non-zero exit with a sentinel) or open (continues on a default); an open branch on a gate path is a defect, and "0 items processed, exit 0" is the standard fake green (lesson:L228, lesson:L235, rubric:9).
- C2 Every exit code reachable and read correctly: each exit code the script documents has an input that produces it; `$LASTEXITCODE` is read right after the native call, never after `Select-Object -First N`, and every native call is guarded (lesson:L93, lesson:L49).
- C2 Every ASCII code unique: each `[NAME-...]` sentinel is printed for exactly one meaning, no two branches share a code, and the table that documents it, where one exists, lists it once; a sentinel that is a prefix of another needs anchored matching (lesson:L165, lesson:L225).
