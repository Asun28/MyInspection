...
| W2 | T2-ROOM-REPEATABLE | 房间定义带 repeatable 标记：模板契约 + `.sqm` 迁移 + 入库读回往返 | T1-TEMPLATE-ENGINE,T0-DEBT-MIGRATION-SNAPSHOT-ALLOWLIST | M | DeepSeek V4 Pro · high | Sonnet 5 max | **merged**（master `deb01fe1`，PR #190，R3 第 2 轮 pass）；schema v2 + 房间往返 + 历史读回索引落地，TD6/TD7/TD8 paid；运行时实例化仍归 T2-REPEATABLE-ROOM-RUNTIME |
