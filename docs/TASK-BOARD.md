...
| W0 | T0-DEBT-SELFTEST-FAIL-DIAGNOSTICS | 单分片与 all 汇总以稳定哨兵点名失败 shard/gate（TD9 1/3） | T0-DEBT-SELFTEST-CRITICAL-PATH | M | GPT-5.6 Terra · high | Sonnet 5 max | **merged**（master `b8dee45`，PR #31；稳定 ASCII gate、协议 fail-closed、hermetic/mutation 覆盖、core/verify/R3 绿；TD9 仍 carded） |
| W0 | T0-LICENSE-SELFTEST-DRIFT | 恢复 Gradle diagnostics 的 selftest 回归覆盖并迁移非 vacuous mutation（TD137） | T0-LICENSE-GAV-BOUNDS | M | GPT-5.6 Terra · high | Sonnet 5 max | **ready**；PR #33 R3 block 后拆出的独立前置，不删/并/弱化既有覆盖；与 TD9 卡共享 selftest，串行 |
| W0 | T0-DEBT-SELFTEST-SKIP-VISIBILITY | 有意 skip 与前置失败裁剪进入确定性执行台账（TD9 2/3） | T0-DEBT-SELFTEST-CRITICAL-PATH + T0-LICENSE-SELFTEST-DRIFT | M | GPT-5.6 Terra · high | Sonnet 5 max | PR #33 暂停；待 TD137 独立 PR merged 后合入最新 master，重放无 git seeded/core/R3 |
...
| W3 | T2-PHOTO-DIRECTORY-DURABILITY | marker 祖先目录 fsync + JPEG 删除 durable 后再清 sidecar（TD137） | T2-PHOTO-ORPHAN-CLEANUP-SCHEDULER | S | GPT-5.6 Terra · high | Sonnet 5 · high | PR #32 最终 R3 follow-up；不重开 worker/lease 设计，不触 schema/selftest |
