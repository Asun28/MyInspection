...
| W3 | T2-PHOTO-ORPHAN-CLEANUP-SCHEDULER | `.jpg.pending` durable lease + 24h WorkManager 回收无行/软删照片孤儿（TD14） | T2-PHOTO-PIPELINE | M | GPT-5.6 Terra · max | Sonnet 5 · max | 既有 `OrphanedAssetCleanup` + `PhotoAssetCleanupExecutor`；内部 `filesDir/media` 与 `myinspection.db` 为未来 ingest 的唯一运行时组成；T1-SPIKE-PLATFORM 仅 app 资源串行，非业务依赖；冻结全局 rel_path 查询交 TD4 后 follow-up |
