...
| W3 | T5-MEDIA-ARCHIVE-SCHEMA ★ | schema v5 四表形态、约束、迁移快照与完整查询面 | T0-DEBT-MIGRATION-SNAPSHOT-ALLOWLIST,T2-PHOTO-PROPERTY-DEDUPE,T5-BACKUP-FORMAT | M | GPT-5.6 Luna · max | GPT-5.6 Terra · max | **已批准** v4→v5：`4.sqm` + `databases/4.db`；不改备份 format_version |
| W3 | T5-MEDIA-ARCHIVE-ELIGIBILITY ★ | 本机状态+PDF 回执+exact tuple/scope/revocation 资格 | T5-MEDIA-ARCHIVE-SCHEMA | M | GPT-5.6 Terra · max | GPT-5.6 Luna · max | 串行 2/3；provider 信号不构成内容证明 |
| W3 | T5-MEDIA-ARCHIVE-CONTRACT ★ | 重新打开逐字节核验+原子 verified receipt+finalized 不变性 | T5-MEDIA-ARCHIVE-ELIGIBILITY | M | GPT-5.6 Terra · max | GPT-5.6 Luna · max | 串行 3/3；保持下游依赖 ID；不接 provider SDK |
