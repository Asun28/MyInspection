...
| W5 | T4-SCHEDULE-REMINDER-DELIVERY | Worker、通知发布、并发唯一 post 与不重投边界 | T4-SCHEDULE-REMINDER-RECEIPTS | S | GPT-5.6 Luna · max | GPT-5.6 Terra · max | **ready**（依赖 RECEIPTS 已合并 `3c08c2dd`；Light Plan Forge 3/3。**接口已变**：`ReminderPreferencePort` 多了 `backingStore`，`ReminderReceipt.causeCode` 是 `ReminderCause?`，`recoverPermissionBlocked` 收**当前**回执而非 next，失败提交后任何 occurrence 的写入一律 `WriteUncertain`——delivery 的重试/放弃分支必须显式处理这个态） |
