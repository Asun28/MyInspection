...
- **心跳的发现信号**：`scripts/triage.ps1` 的十一枚探针里，**三枚的下一步始终直接指回本文**；`lessons-cap` 仅在驻留 id 成功解析且达/超封顶时指回本文，`lessons-promote` 仅在候选 `>5` 时指回本文，合为本评审的**自动触发点**——
  ...
  另六枚不进本仪式：`tech-debt-open` · `cards-active` · `handoff-open` · `worktree-orphan` · `delivery-blocked`（交付停摆 → task-loop）走交付链；`scaffold-stale` 走 `docs/SCAFFOLD-SYNC.md` 的逐版决策账。
