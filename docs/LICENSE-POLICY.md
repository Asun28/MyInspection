...
> 随开发把每个依赖登记到下表。`scripts/check-licenses.ps1` 自动扫描 PyPI/npm，并从 Gradle **真实解析的
> classpath**逐坐标读取缓存 POM 许可；模型权重/数据/字体/素材仍须**逐项**人工登记。Gradle 的缺失、未知或
...
> 本表保存直接依赖/构建工具的人工证据；自动扫描的传递闭包规则见 §3.2。人工证据须对准每条坐标的**发布方主库
...
- **受控例外 B（有效 POM 的名称映射）**：有效 POM 中某个**未识别**的 `license/name` 可附
  ...
  BSD-3-Clause，依据为其 POM 指向的 libyuv `README.chromium`。
