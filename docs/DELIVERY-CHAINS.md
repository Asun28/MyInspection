...
| 确定性 CI 闸 | `.github/workflows/ci.yml`（job 名 `verify`）+ `scripts/verify.ps1`（含占位「闸门 2」） | **windows-latest**（T0-TOOLCHAIN 起：Android/Gradle 闸与本机同源，以确定性换 2× 计费分钟）、无密钥；**网络**：仅 setup-java / setup-android / 一次在线 `gradlew build` 预热依赖缓存属引导步，此后各闸恒 `--offline`（**非「全程无网络」**——旧表述已随 Android 闸落地作废）；触发 **push + pull_request**（分支 `[main, master]`；push 侧 `paths-ignore: ['**.md','docs/**']`——**纯文档直推不触发**、混合推送仍全跑（`scaffold-selftest.yml` 的 push 侧进一步**正向 `paths` 收窄至权威面** scripts/.claude/.github/configs，见其头注），**PR 侧刻意不过滤**（必需检查 + path filter 不相容会卡 doc-only PR）；两份工作流的触发形态（push+PR 到 main+master）由 selftest **8.2d** 锁死）；selftest **14f** 按 `_config.ps1` 的 `DocSyncMap` 检查源脚本与权威文档同步（`[doc-sync:none]` 可显式豁免）——**当前是本地 best-effort 闸**：比对**已提交** `base..HEAD`（与 `[doc-sync:none]` 逃生门同源于提交历史，语义自洽），真强制点在**提交后跑 selftest（CI push/PR 或本地自检、自 master 分叉、能算 merge-base）时**——**非** ship 的 pre-commit DoD（那是卡的 `dod_command`）；无法算 base（detached / 首提交 / 浅克隆 / master 或 merge-base 不可解析）**一律 fail-open 跳过、不 Fail**（文档纪律闸、非安全闸），在 master 上/CI 无分叉时变更集为空亦 no-op，故**权威的 CI-PR 侧强制腿为显式后续**（本卡 non_goals），当下 **push 侧是事后检测层、非 push 前强制**：提交落地后才跑，故在无可强制规则集（需 Pro/public）的仓里，它保证的是直推提交**败即显式变红**而非被拦下；push 前的真强制归 `gh-bootstrap.ps1` 装的 pre-push 钩子与服务端规则集（见 `ci.yml` 头注）；job 内依次跑 `check-cards` → pytest（有 `pyproject.toml` 才跑）→ `check-secrets`（普通扫描捕活密钥；`-Strict` 留作变 public 前一次性闸，不进每 PR）→ `check-licenses`（许可闸；无依赖清单优雅过，堵裸 push 绕本地许可闸）→ `verify.ps1`（脚本缺失/占位时各步优雅跳过） | — |
