...
| 确定性 CI 闸 | `.github/workflows/ci.yml` + `scripts/verify.ps1` | **windows-latest**、无密钥；setup 与一次 Gradle 预热可联网，闸内 `--offline`。`ci.yml` 跑 `[main, master]` push+PR（仅 push 忽略 Markdown）；`scaffold-selftest.yml` 的默认分支 push 排除产品 `configs/compliance/**`，保留脚手架 license/secret 配置及手动触发，跑 2 OS × 5 shard canary，不进 PR。产品卡使用相关产品测试 + verify + R3。selftest 8.2d/8.2e 锁触发/矩阵；14f 按 `DocSyncMap` 检查已提交 diff，无法算 base 时 fail-open，非 pre-commit 闸。push 前由钩子/规则集守门。`verify` 顺序：cards → pytest（如有）→ secrets → licenses → verify。 | — |
