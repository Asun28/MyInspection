...
| 链路 | 入口脚本 | 配套 | 权威文档 |
|---|---|---|---|
| 单卡闭环 R1–R5 | `scripts/task.ps1 -Phase start\|ship\|cleanup` | `_guard.ps1`（账号守卫）· `_config.ps1` · `check-cards.ps1`（start 前置校验卡）· `_scope.ps1`（范围闸判定核，与独立入口 `check-scope.ps1` 共用；后者是「已推送恢复」序列第 3 步的可执行投影，越界/不可判即非零退出、不做定向 fetch）· 远端 ship：DoD → **verify 总闸** → 提交 → 定向刷新 `origin/<base>` → **范围闸** → 许可 → 防泄露 → **真实 diff 预算闸** → push/PR → PR base 确认 → R3 → **candidate CI 闸**（逐层身份绑定：已评审本地 HEAD ≡ PR `headRefOid` ≡ run `head_sha`，run 的 `path`/`event`/`pull_requests[].number` 三处一律大小写敏感比较（禁 `-eq`/`-in`/`-contains` 承担身份判定），候选 `ci.yml` 声明的 job 集须与 run 返回集逐名相等否则 `[CI-GATE-JOBS-DRIFT]`；单一 wall-clock deadline `SCAFFOLD_CI_TIMEOUT_SEC` 罩住全部 gh/git 子进程、超时杀进程树、重试只花剩余预算；决策前再取 exact-head/base 快照，base 前移 / retarget / head 前移 / run 身份漂移均不合并；`-NoAutoMerge` 只跳过合并腿、不放松任何一层；机检 `T37-CIGATE/WORKFLOW-BINDING` + `T37-CIGATE/JOBS-DRIFT`） → merge 前 base 复查 → 合并；fetch/base 查询失败或错配均 fail-closed，不回退陈旧本地基线 | `docs/DEVOPS-WORKFLOW.md` |
...
| 确定性 CI 闸 | `.github/workflows/ci.yml` + `scripts/verify.ps1` | **windows-latest**、无密钥；setup 与一次 Gradle 预热可联网，闸内 `--offline`。`ci.yml` 跑 `[main, master]` push+PR（仅 push 忽略 Markdown）；`scaffold-selftest.yml` 只在默认分支权威面 push 或手动触发，跑 2 OS × 5 shard canary，不进 PR。selftest 8.2d/8.2e 锁触发/矩阵；14f 按 `DocSyncMap` 检查已提交 diff，无法算 base 时 fail-open，非 pre-commit 闸。push 前由钩子/规则集守门。`verify` 顺序：cards → pytest（如有）→ secrets → licenses → verify。 | — |
