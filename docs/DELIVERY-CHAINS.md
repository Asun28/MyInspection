...
| 确定性 CI 闸 | `.github/workflows/ci.yml` + `scripts/verify.ps1` | **windows-latest**、无密钥；setup 与一次 Gradle 预热可联网，闸内 `--offline`。`ci.yml` 跑 `[main, master]` push+PR（仅 push 忽略 Markdown）；`scaffold-selftest.yml` 只在默认分支脚手架权威面 push（排除产品规则 `configs/compliance/**`）或手动触发，跑 2 OS × 5 shard canary，不进 PR。selftest 8.2d/8.2e 锁触发/矩阵；14f 按 `DocSyncMap` 检查已提交 diff，无法算 base 时 fail-open，非 pre-commit 闸。push 前由钩子/规则集守门。`verify` 顺序：cards → pytest（如有）→ secrets → licenses → verify。 | — |
| 前端生成闭环（T2 · 串联非新引擎） | `.claude/skills/frontend-flow`（驱动卡） | 四段串现有件:生成前 PRD=`shape-idea`/`PROJECT-BRIEF`(+前端补充节)·规则=`frontend/README` 5 闸+design tokens 真相源;生成中**流程卡(页面地图)→经 `plan-forge` 审计、`decompose-cards` 投影任务卡**、**意图卡(单页)→`grill-design`** 拷问敲定;生成后路由 **pencil MCP**/Claude Design/v0(L26 不绑死)+`frontend-design`/`taste-skill` 局部改;验证过区块回流 `context/frontend-assets/`。**不重造拷问/审计/编辑器**;仅复杂多页前端用,T0/T1 直接 `frontend-design`+pencil | `docs/FRONTEND-FLOW.md` |
