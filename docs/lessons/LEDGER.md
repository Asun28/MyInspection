...
- root_cause: 轮次与卡片行数几乎无关：8 张已合并卡实测 Pearson **r=0.42、r^2=0.18**（八对全数据与逐行来源见 specs/tasks/T0-CARD-ACCEPTANCE-SETS.md「为什么」节，可复算），最大 diff（T5-BACKUP-FORMAT 2740 行）4 轮收，最小（T2-ROUTINE-CONTENT 277 行）9 轮——**这一半结论有完整可复算数据**。较可能的驱动是卡片只声明行为、不封闭验收集合：24 份终局裁决共 25 条 finding 里 18 条（72%）是 rubric #6「测试缺失」，#6 的搜索面在验收集合开放时无界；同时 #8（过度设计）0 命中，卡片没要求的防御性代码遂被判「补测试」而非「删掉」，每轮修复扩大下轮未测面。**但这后一半只是指向性证据**：计数取自各 worktree `.review/<branch>.json`，该产物每轮覆写、随 worktree 生灭、按设计不入库，读者无法从仓库历史复现，且 24 份是终局快照而非历次轮次普查。据此**不足以**改任何评审规则，只足以支持「作者先把验收清单写出来」这一条不依赖 rubric 的做法。
- rule: 轮次通胀先看卡片有没有编号封闭的验收清单，再谈拆卡——**写清单本身就有价值**（作者一次性想清「完成=哪些事实」，比在 N 轮评审里被动发现便宜），这一条现在就能做、不依赖任何 rubric 改动。对照：T1-CANON-HASH 把哈希域逐字段列全 → 2 轮收；T4-COMPLIANCE-ENGINE 只说「加载器做校验」→ 评审列出 12 条卡片从未提过的拒绝用例。**另有两条只是提给上游的提案，本仓在上游落地前一律不采用、不得据以裁决**：① 让 rubric #6 只判「清单内每条是否有可证伪测试」、清单外记 [FOLLOW-UP]（Asun28/claude-devops-scaffold#203）；② 让不能追溯到验收条目的守卫/包装/分支判为删除而非补测试、#8 优先于 #6（同仓 #204）。现行裁决仍完全按 docs/QUALITY-RUBRIC.md 当前文本，[FOLLOW-UP] 仍只在「修法超出 allow_paths 或落在 non_goals」时适用。本条同时限定 L206 的适用面：拆卡治的是 allow_paths 蔓延与单 PR 过大，不治轮次通胀——把拆卡当轮次通胀的解药会拆出更多卡、更多仪式 PR，轮次照旧。
