...
- root_cause: 轮次与卡片行数**几乎无关**：8 张已合并卡实测 Pearson **r=0.44、r^2=0.19**（八对全数据与逐行来源见 specs/tasks/T0-CARD-ACCEPTANCE-SETS.md，可复算；轮次以 docs/TASK-BOARD.md 逐轮记述为准——CLAUDE.md 概述里的轮次与之冲突时以 TASK-BOARD 为准，本条初稿就是照 CLAUDE.md 取了 T2-CAPTURE-CORE=7 而实为 6、把 T2-PHOTO-PIPELINE 记成触顶那轮 9 而实为裁后又跑到 11）。最大 diff（T5-BACKUP-FORMAT 2740 行）只用 4 轮、是**第二少**，最小（T2-ROUTINE-CONTENT 277 行）用了 9 轮。**但「体量不解释轮次」不等于已知真因**：曾观察到 24 份终局裁决 25 条 finding 里 18 条点名 rubric #6、#8 零命中，据此推断「验收集合不封闭 ⇒ #6 搜索面无界」——该计数取自各 worktree `.review/*.json`，按设计不入库、每轮覆写、且是终局快照而非普查，**不可审计**，只算指向性线索。
- rule: **别把「拆卡」当轮次通胀的解药**——这一条有可复算数据（r^2=0.19）：拆卡治的是 allow_paths 蔓延与单 PR 过大（L206 的适用面），不治轮次。**「写封闭验收清单」是否治轮次，本仓目前有两个互相矛盾的数据点，尚无定论，别当已知结论用**：㊀ PR#124 给两张 round-cap 卡各写 15/16 条清单，第 1 轮零条 #6（看似奏效），第 2 轮出现 2 条 #6 且**全部指向清单本身**（名字黑名单证明不了不存在别名旁路；两条清单项在默认/非默认配置上互相矛盾；不可变项未要求「被拒绝的修改之后原集合仍未变」），读作「无界搜索被上移一层：从『有没有漏测』变成『清单有没有漏项』」；㊁ 同期 PR#126 拆出的三张卡也各带清单，但条目**精确到夹具级**（如「恰好 999 changed lines 放行、恰好 60000 字符放行，各断言精确度量值而不只断言 exit 0」），远细于 #124 那种「断言公开 API 面无 disable/skip/force 形态参数」的粗条目。**故 #124 的反证也可能只证明了「粗清单招 #6」而非「清单机制无效」**——判别要看 #126 那三张卡实际跑出的轮次，届时回填本条。可确定的只有：**清单值得写**（作者一次想清「完成=哪些事实」比在 N 轮里被动发现便宜），**但把它当收敛机制、或据它改判评审语义，证据都还不够**。
- enforced_by: none（判断类经验，无机检；且本条的结论之一正是「别指望机械化封闭验收集合来收敛评审」）
- refs: PR#124（粗清单：第 2 轮 #6 反指清单自身，人裁撤回）; PR#126 / 24c0a33（精确到夹具级的清单，待观察对照）; docs/TASK-BOARD.md（R3 轮次的权威源，与 CLAUDE.md 概述冲突时以它为准）; L206（拆卡的适用面）; 上游 Asun28/claude-devops-scaffold#203 #204 #205
