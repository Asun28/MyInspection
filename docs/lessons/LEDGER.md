...
## L80
- date: 2026-07-07 ｜ tags: powershell,array-unwrap,validation,type-check ｜ tier: ledger ｜ kind: pitfall ｜ severity: major ｜ recurrence: 2
...
## L178
- date: 2026-07-31 ｜ tags: encoding,evidence,restore ｜ tier: ledger ｜ kind: pitfall ｜ severity: major ｜ recurrence: 2
...
## L206
- date: 2026-08-15 ｜ tags: review,scope,task-loop,orchestration,card-sizing ｜ tier: ledger ｜ kind: pitfall ｜ severity: major ｜ recurrence: 1
- symptom: 一张卡在 R3 反馈循环里越审越大、轮次不收敛：每轮评审提出一条「本卡范围外但确实该修」的缺陷，编排者判它有理便扩一次 allow_paths，下一轮评审面随之变大、又在新进来的代码里找到新缺陷。T0-TOOLCHAIN 实测六轮、23 文件 1017 行、约 2 小时/726k token，且第 4/5/6 轮多数 finding 都落在**破例带进来的那部分**，不在卡本来的产出上。
- root_cause: 每次破例单独看都成立（假绿的闸/一行的确定性漂移/没测试的守卫都是真问题），但**评审压力下的扩范围是单调递增的**：卡越大→评审面越大→找到的「范围外真问题」越多→再扩。评审者按当前 diff 判断，不会替你守卡的宪章；check-cards 的「allow_paths > 5 卡可能过大」告警其实已经报了警，但它是**建议级**，容易被当噪音略过。
...
## L211
- date: 2026-08-15 ｜ tags: cards,scope,review,planning,arbitration ｜ tier: ledger ｜ kind: pitfall ｜ severity: major ｜ recurrence: 1
- symptom: 两张卡对「某个类型/查询归谁建」各执一词，R3 与执行者连续两轮不认可：A 卡卡文写「输入是 model 层数据类（B 卡已定义）」，B 卡卡文却从没点名要建它，且把 A 卡列为 non_goal。评审据 A 卡文判 B 卡缺东西，执行者据 B 卡文拒绝，谁都能引到「原文」。
- root_cause: 把**卡文措辞**当归属依据。措辞是人写的注解、会互相矛盾；真正的归属声明是 **allow_paths 的路径分配**——它是机检的、唯一的、且 ship 的范围闸就照它执行。本例一查即明：B 卡 allow_paths **含** `core/model/`（且至今无任何产出用到，说明当初就是为此分配），A 卡 allow_paths **只有** `core/canon/` 且 forbid 明写不许碰别的包——A 卡**无权**建那个类型，B 卡不建则 A 卡开工即死。
...
- symptom: 重构测试/守卫结构后连续多轮 R3 block，每轮都在点名 stale 注释、失效的库导出清单、死变量、以及描述上一版形状的失败文案；实现本身是对的
- root_cause: 改结构时只改了代码路径，narration（注释/头注/失败文案/导出清单/变量清理）留在上一版形状；评审者按 rubric #16 逐轮点名，一轮只点最刺眼的一批
- rule: 每次结构性重构后，在自查阶段把「随结构一起改的面」当清单逐条过：①被改文件自身的注释与头注 ②库导出/参数清单（-AsLibrary 之类）③失败文案与总结行 ④因重构变成死代码的变量 ⑤引用旧标识符的注释。用 grep 找旧标识符（改名前的函数名/gate 名/变量名）确认零残留，再交付
- enforced_by: 
- refs: 
...
- symptom: R3 轮次通胀被归因为「卡太大、颗粒度不够」，于是拆卡；拆完轮次没有按预期降下来（规模确实占约四成，但不是主因）。评审每轮都提出新的细节问题，像是永远修不完。试图用「把验收集合一次写全」来终结它，也没终结。
- root_cause: 轮次与卡片行数是**中等相关、非决定关系**：**n=14** 张已合并产品卡实测 Pearson **r=0.6316、r^2=0.3989**——规模解释约四成方差，余下约六成另有驱动。**入选规则（先定口径再取数）**：已合并的**产品卡**（T1–T5 前缀，实现落 android/** 或 data/templates/**），**且** docs/TASK-BOARD.md 或 CLAUDE.md 至少一处留有 R3 轮次记录；T0 harness/脚手架卡整体排除（被审面是脚本与文档，不同质），`*-R3-CLOSURE` 承接卡排除（它们本身是另一张卡轮次的产物，计入即重复计数）。**14 行全数据（新增行 / 轮次）**：T2-PHOTO-PROPERTY-DEDUPE #116 163/1 · T1-SKELETON-E2E 19fd908 258/2 · T2-ROUTINE-CONTENT #5 277/9 · T1-CANON-HASH #2 556/2 · T2-PHOTO-QUALITY-PROFILES #30 751/2 · T5-RETENTION #11 791/5 · T2-PHRASELIB #10 867/4 · T1-TEMPLATE-ENGINE #3 1008/5 · T2-PHOTO-STREAMING-ENCODE #29 1151/2 · T2-CAPTURE-CORE #8 1862/6 · T3-FINALIZE #7 2027/15 · T2-PHOTO-PIPELINE #6 2168/11 · T1-SCHEMA-CORE fcdc88d 2632/17 · T5-BACKUP-FORMAT #9 2740/4。**合规则但因两处权威面都无轮次记录而取不到数、须点名的三张**：T2-PHOTO-ORPHAN-CLEANUP-SCHEDULER（#32，3282 行——全部产品卡里最大的一张，它缺记录即本数据集最大的不确定性来源）· T2-PHOTO-DIRECTORY-DURABILITY（#35，334 行）· T2-FIELD-LEDGER-THEME（#55，只记「R3 全绿」无轮次）。**订正史**：本条初稿用没有入选规则的八行样本，得 r=0.44 / r^2=0.19 并据此断言「几乎无关」；R3 第 3 轮指出样本无口径且漏卡，补齐后为 0.3989，「几乎无关」属过度解读，已作废、勿再引用。全表**最大**的 diff（T5-BACKUP-FORMAT 2740 行）只用 4 轮，**低于轮次中位数 4.5**；T2-ROUTINE-CONTENT 只有 277 行（**第 3 小**，比它更小的还有 163 与 258）却用了 9 轮（**第 4 高**）。轮次层级依次为 1/2/4/5/6/9/11/15/17，4 是第三档并列第 6–7 位——**不是「第二少」**，「277 行是最小」亦不成立；两处都是八行样本时代的排名，随 14 行订正。**轮次的来源分两类，逐行点名（14 行全覆盖）**：**10 行取自 docs/TASK-BOARD.md 的逐轮记述**——T2-PHOTO-PROPERTY-DEDUPE=1 / T2-ROUTINE-CONTENT=9 / T2-PHOTO-QUALITY-PROFILES=2 / T5-RETENTION=5 / T2-PHRASELIB=4 / T2-PHOTO-STREAMING-ENCODE=2 / T2-CAPTURE-CORE=6 / T3-FINALIZE=15 / T2-PHOTO-PIPELINE=11 / T5-BACKUP-FORMAT=4；**4 行在 TASK-BOARD 里根本没有轮次记录**，只能取自 CLAUDE.md 的「当前阶段」节——T1-SKELETON-E2E=2 / T1-CANON-HASH=2 / T1-TEMPLATE-ENGINE=5 / T1-SCHEMA-CORE=17，那是它们唯一的记录。**新增行的来源同样分两类**：12 行来自 gh pr view --json additions；**两行无 PR、走本地合并**，取 git show --stat 的 insertions——T1-SKELETON-E2E(19fd908) 与 T1-SCHEMA-CORE(fcdc88d)。故准确的优先级规则是「**两者都有时以 TASK-BOARD 为准**」，不是「一律以 TASK-BOARD 为准」；本条初稿说成后者，是把当时五行的情况推广到了八行。（「五行+三行」是八行样本时代的账，14 行里有 6 行当时无来源声明，已补齐。）初稿另有两处取值错误（照 CLAUDE.md 概述记 T2-CAPTURE-CORE=7 而实为 6、把 T2-PHOTO-PIPELINE 记成触顶那轮 9 而实为裁后又跑到 11），已订正，r^2 由误算的 0.18 变为 0.1906（T0-CARD-ACCEPTANCE-SETS 卡的验收条目 A5 已随之正式修订为该值——起初为避免再动一次被评审钉住的契约面而保留 0.18，但那让契约与数据长期不一致，R3 据此 block，遂改正）。**但「体量不解释轮次」不等于已知真因**：曾观察到 24 份终局裁决 25 条 finding 里 18 条点名 rubric #6、#8 零命中，据此推断「验收集合不封闭 ⇒ #6 搜索面无界」——该计数取自各 worktree `.review/*.json`，按设计不入库、每轮覆写、且是终局快照而非普查，**不可审计**，只算指向性线索。
...
## L283
- date: 2026-09-02 ｜ tags: mutation,testing,handoff ｜ tier: ledger ｜ kind: pitfall ｜ severity: minor ｜ recurrence: 1
...
## L299
- date: 2026-09-05 ｜ tags: requirements,authority-docs,verification ｜ tier: ledger ｜ kind: pitfall ｜ severity: major ｜ recurrence: 1
- symptom: 把一段需求当作 [定] 条款引用、据此把实现选择记成「对冻结条款的偏离」，并以此为前提向用户和子代理提问。实查后三处皆错：该段本身无 [定] 标记（带标记的是它上面那段）、其所依赖的双语条款仍是 [待]、被用作对照的「PDF 侧已能正常渲染中文」其实是一张 todo 卡且字体资产目录根本不存在。整个问题框架建立在不成立的不对称上。
- root_cause: 读权威文档时只读了段落正文，没有逐条核对它的状态标记与相邻标记的作用范围；引用其他卡片的能力时只读了卡片描述，没有核 status 字段与产物是否真的存在。散文读起来像已定，就被当成已定。
