...
- refs: PR#124（两张 round-cap 卡的封闭清单；第一稿粗、第 2 轮 #6 反指清单自身，遂按 #126 精度重写后交付）; e40d74e / d7cdc27（PR#124 第 1 / 2 轮 #6 分类的来源＝分支提交信息；**仅可经 GitHub PR #124 的提交列表审计**——squash 合并后本地 `git log` 不含这两个提交，区别于不入库的 `.review` 计数）; PR#126 / 24c0a33（精确到夹具级的清单，本条的精度基准）; PR#39 / PR#43（清单所属的两张卡，其轮次是本条的待回填判据）; docs/TASK-BOARD.md（五张卡 R3 轮次的权威源；另三张只有 CLAUDE.md 有记录）; L206（拆卡的适用面）; L165（「断言面必须恰好等于被测契约」——粗清单招 #6 是它在卡片层的同一形态）; 上游 Asun28/claude-devops-scaffold#203 #204 #205
