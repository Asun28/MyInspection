...
- [ ] **Gradle 传递依赖许可全量核验通过** `[GRADLE-LIC-SCANNER-ONLY]`（阻断项）—— `android/` 子树约 220 个传递坐标当前未逐一核验（见 `docs/LICENSE-POLICY.md` §3.2）。**唯一解锁路径 = 接入自动扫描器（TD2 偿还卡 `specs/tasks/T0-LICENSE-SCANNER.md`）跑过全量传递依赖坐标并记录结果**；它落地前本项不得勾选，未清零前不得分发（打包 APK 对外分发 / 变 public 均算分发）。**触发点澄清**：GPL 系纯 copyleft 的义务触发点是**分发**；但 AGPL/SSPL/EUPL/非商用/研究限等触发点与分发无关，即便当前未分发也不豁免（见 `docs/LICENSE-POLICY.md` §1.1/§3.2「不是自动豁免」）——这些坐标一旦在传递依赖中现身须立即处理，不得拖到发布前才查。**刻意不设人工核验替代路径**（人裁 2026-08-16，承接 T0-GATE-HARDENING 事后 R3）：手工表在下次依赖变动即过期，且 `docs/LICENSE-POLICY.md` §5 已定「脚本不会自动清除覆盖缺口告警」是刻意的 fail-safe——若允许"人工核验完就勾选"，本项与上一项 `-Strict` 必须通过就会相邻互斥，逼人在闸红着时勾选。故本项与上一项**由同一个动作解锁**：扫描器落地 ⇒ Gradle 覆盖缺口消失 ⇒ `-Strict` 退出 0 ⇒ 两项同时可勾。（本契约由 `selftest.ps1` 闸 17ee 机检：哨兵在场、指名偿还卡、且不得出现第二条替代路径。）
