...
- [ ] **Gradle 传递依赖许可全量核验通过** `[GRADLE-LIC-SCANNER-ONLY]`（阻断项）——运行 `pwsh -NoProfile -File scripts/license-scanner-check.ps1 -Suite integration`；该套件聚合 graph/policy/diagnostics/gav-bounds，并对真实仓执行 `scripts/check-licenses.ps1 -Strict`、确认 `org.testng:testng` 被逐坐标报告。CI 必须先完成 JDK/Android/Gradle setup 与在线 cache warm-up，再运行强制离线 scanner。缺失、未知、禁列或无法解析的许可元数据均 fail-closed；模型权重、数据、字体、素材仍按 `docs/LICENSE-POLICY.md` 人工登记。承接 `T0-LICENSE-SCANNER` 的 `T0-LICENSE-CI-INTEGRATION` 合并且总验收通过后方可勾选，不设人工逐坐标核验替代路径。
