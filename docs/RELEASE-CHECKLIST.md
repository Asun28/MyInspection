...
- [ ] **Gradle 传递依赖许可全量核验通过**（阻断项，T0-GATE-HARDENING 仲裁）—— `android/` 子树约 220 个传递坐标当前未逐一核验（见 `docs/LICENSE-POLICY.md` §3.2）；许可义务触发点是**分发**，`check-licenses.ps1` 不扫 Gradle 生态，故此项**不由任何自动闸覆盖**，必须在本清单人工勾选：接入自动扫描器（TD2）跑过全量传递依赖 **或** 人工逐条核验并登记进 `docs/LICENSE-POLICY.md` §3，两者选一，未清零前不得分发（打包 APK 对外分发 / 变 public 均算分发）。
