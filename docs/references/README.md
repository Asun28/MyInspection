...
| `claude-opus-5-5-prompting-llms.txt` | Opus 5.5 提示词专属细则，**相对 Opus 5 的差异层**（**相对 Opus 5 的九处变化**，含四项破坏性变更 / effort 默认 medium 且同档想得更多 / thinking 恒开 / 长回合以纯文字汇报结束 / 进度更新杠杆 / 新增 bio 与 `reasoning_extraction` 拒答 / 多应用先探索 / 多代理时间信号 / 粘贴文本标记 / 视觉与前端默认） | 给 Opus（想/架构/评审角色，现任 Opus 5.5）调提示、或写 Opus 面向的 skill/hook/rubric 时——**连同基线 `claude-opus-5-prompting-llms.txt` 一起读**，两篇说法不同处以本篇为准；**拿为 Opus 5 写的旧 prompt 迁移时必读「相对 Opus 5 的变化」表** |
| `claude-opus-5-prompting-llms.txt` | Opus 5 提示词专属细则（**相对 4.8 的五处反转** / 拒答与回退 / effort 起点 / thinking 默认开 / 对话与写盘两种冗长度 / 子代理上限（含 Claude Code 的确定性上限）/ 范围收窄 / 评审覆盖） | **Opus 5.5 的基线层，仍在服役**：给 Opus 5.5 调提示时先读本篇、再读上一行差异层；**拿为 4.8 写的旧 prompt 迁移时必读「五处反转」段**（那几条在 4.8 上对、在 Opus 5 上反着伤你） |
| `claude-opus-4-8-prompting-llms.txt` | Opus 4.8 提示词专属细则（effort/thinking 默认关/字面遵循/子代理偏少/**前端家风 AEFRM 正文**/computer use/评审 recall） | **兜底席位，非日常档——没有退休。** Fable 5.1、Fable 5、Opus 5.5 与 Opus 5 都带安全分类器，官方默认回退按拒答类目（该类目无推荐兜底则拒答维持）（5 个：`cyber`/`bio`/`frontier_llm`/`reasoning_extraction`/`general_harms`）改道到**当时推荐**的兜底模型，官方示例选中的就是这一档——**示例非契约，映射会变，读响应的 `fallback` 块别写死**。**配回退链、排查「怎么突然是另一个模型答的」、或取 AEFRM 设计规格正文时读**（opus-5 与 sonnet-5 两篇的设计节都指向这里，三页同文防重复） |
