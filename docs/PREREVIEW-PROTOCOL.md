...
> Remote adoption: this document describes the approved target protocol. Only the schema and policy foundation are available at this PR; FACTS-LIB follows next. RECORDS, workers, runner and the trust-manifest/ship integration are pending. See `docs/plans/PREREVIEW-REMOTE-ADOPTION.md`; source local card references below are design lineage, not claims that those cards or scripts exist on remote master.
...
> lists what they found and what they covered. It is **not a gate**: codex R3 and deterministic delivery checks remain required, the packet
> never enters the codex prompt, no record has a field named `verdict`, and no status value is `pass` or `block`.
> **Status**: the approved target protocol, adopted in the stages listed in `docs/plans/PREREVIEW-REMOTE-ADOPTION.md`.
> Source Phase-1a card names below identify planned responsibilities. Operational packet execution remains pending
> the runner, workers, RECORDS and their separate integration delivery.
> **Conventions**: commands are named by subcommand (`prereview.ps1 run`); paths may name pending artifacts.
> Current availability follows the remote adoption plan. Contract sources: `specs/prereview-record.schema.json` (frozen record contract), `scripts/_config.ps1`
> (`Prereview*` knobs, values only), `docs/PREREVIEW-CHECKLISTS.md` (worker-facing checklists, named here and never
> reproduced). Future outbound integration must first update `docs/TRUST-MANIFEST.md`; this adoption authorizes no provider or network execution.
...
2. `prereview.ps1 run -TaskId <id> -Base <name> [-Local]`. Pass `-Local` only when the card will ship `-Local`; omit it for remote delivery. Base resolution follows the ship mode through `Resolve-ScaffoldBaseRef -PreferLocal`
...
- **The packet and worker outputs are not written into the reviewed worktree or its `.review/`**. The codex reviewer
  uses a workspace-write sandbox rooted at that worktree and may open any file there (`scripts/review.ps1`, codex invocation), so the packet and the worker outputs are
...
`.secrets/`, `.review/`) cannot enter. Existing tracked files remain eligible regardless of ignore rules; tracked deletions are reflected in the
snapshot by `add -A`. The guarantee for tracked content rests on the repository's own
...
| `tree/**` | the snapshot exported with `git -c core.autocrlf=false checkout-index --prefix`: exactly the snapshot tree's content: existing tracked files plus untracked non-ignored files, with tracked deletions reflected; an ignored untracked file cannot enter | readable, `Read`/`Grep`/`Glob` | **never** |
...
|---|---|---|
| C1 | written guarantee exceeds evidence: comment, KDoc, test name, card invariant or acceptance row wider than the code or the assertion; a universal claim contradicted by an instance elsewhere in the tree; a sibling-clause contradiction | L224; rubric 6 |
...
| C4 | base or process drift: branch behind base, evidence pinned to another tree, PR-body-only evidence | L148, L227 |
...
worktree (the linker reads the current verdict; `review.ps1` also preserves per-round `.r<n>.json` files), lists `reasons[i]` with their SHA-256, and
...
of the predicate: packet wall time P50 and the `[PRE-BAD-RECORD]` count on live runs. Before a remote live checkpoint, establish its baseline from the then-current remote archive. The local source
study (143 archived cards, first-round pass 32.9%, 3.9 blocks per card) is historical provenance only; it is not
a measured remote baseline. Adoption scope is recorded in `docs/plans/PREREVIEW-REMOTE-ADOPTION.md`.
