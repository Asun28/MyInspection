...
日期：2026-08-14 · 状态：accepted（**待用户签认**——本条推翻需求 §11 一处 [定]） · 决策方式：3 方一致
...
2. 备份 = **加密归档导出**，满足 MVP 必需：
   ...
   - 格式：ZipOutputStream → AES-256-GCM 流式加密；PBKDF2WithHmacSHA256 派生口令密钥；盐/迭代数/nonce/格式版本/GCM tag 入包头（T5-BACKUP-FORMAT 冻结）。
