...
日期：2026-08-20 · 状态：**accepted（用户已签认 2026-08-20）** · supersedes：ADR-0002 的活数据落位、按物业导出与后台口令保管部分；保留其 SAF 加密归档、无服务端找回、整包替换恢复决策
...
2. format v1 的“按物业包”仍含整库 `db.sqlite`，会把其他物业、租约和通知一并带出。
...
- v1 没有账号、服务器同步、遥测、广告或后台上传。引入任何新出站目的地必须另走 ADR/任务卡和 payload 评审。
...
- **format v1 UI 只提供全量数据集备份。** 按物业导出隐藏并拒绝创建，因为整库 `db.sqlite` 无法满足物业级保密边界。真正按物业的包需要新 format version、过滤后逻辑/SQLite 快照和独立恢复语义评审。
...
- **继续提供按物业 v1 包**：标签与实际内容不一致，存在跨物业泄露；弃。
...
- format v1 的产品范围收窄为全量备份，换来真实的保密边界；物业级迁移推迟到格式 v2。
