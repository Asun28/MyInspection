...
2. format v1 的按物业包不能直接复用整库 `db.sqlite`，否则会把其他物业、租约和通知一并带出；必须构造经过授权过滤的最小快照。
...
- **format v1 UI 同时保留全量与按物业备份。** 全量包使用一致性整库快照；按物业包从授权 read model 构造只含目标物业、关联租约/巡检/通知和被引用媒体的最小 SQLite 快照，绝不直接打包整库 `db.sqlite`。恢复仍采用“全部验证后整包替换”，按物业包会明确预览其替换范围，不做隐式合并。
...
- **按物业包直接包含整库 `db.sqlite`**：标签与实际内容不一致，存在跨物业泄露；弃，改为过滤后的最小快照。
...
- format v1 保留全量与按物业两种范围；按物业包增加过滤快照与范围预览成本，换取真实的物业级保密边界。
