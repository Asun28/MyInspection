...
`T3-DOCX-IMAGE-QUALIFICATION`, `T3-DOCX-XML-TREE` and `T3-DOCX-EXTRACTION-MANIFEST` precede the registered extractor card. Each has independent delivery gates; the reference contract below fixes synthetic integration acceptance, including the approved image-retention correction. No review-size limit changes.
...
The binding [extraction contract](../references/docx-extraction-contract-llms.txt) specifies the future remote API, field/null/order encoding, independent golden vectors, normalization and parser behavior; it does not claim these implementations already exist remotely. The internal XML tree rejects DTD/entities before external access, with package resource limits upstream. The manifest copies all eight lists and uses the specified DOCX-EXTRACT-1 encoding and six-ASCII-whitespace normalization.
