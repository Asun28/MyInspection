...
- finalize/PDF/备份/恢复失败、连续 3 次备份失败、备份超过 7 天、完整性失败、恢复回滚和上次崩溃恢复必须在本机 1 秒内形成可操作状态提示。PDF 使用 registry 的 `PDF_GENERATE` 封闭事件并按巡检与房东/房客版本分别清除；所有事件先后只按数据库自增序列，活动告警投影不得因 90 天/20,000 行事件裁剪而消失。
