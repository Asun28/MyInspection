...
- 卡片使用精简核心字段；支持字段只承接本卡确有的上下文或约束，不重复填写项目硬边界。
- 可选 `acceptance` 使用一条起的双引号块式 `A1..An`，不为数量凑条目。需要需求追踪时才加可选 `requirements`（唯一非空 `R-id` + 可测试内容），验收可用 `[R1] [R2]` 引用；缺失不增加义务，不强制特定句式或逐项覆盖。格式以 `specs/README.md` 为准。
