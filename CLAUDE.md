...
**2026-09-17 本地合并**：`T0-PREREVIEW-STATE-1A`（master `62ec5f3b`，首轮 R3 pass、零 finding）交付 state v1 校验、原子写入、dispute 追加与 packet/view 输出。复用 RECORDS 重建候选和覆盖记录；输出留在 git common dir，合法 finding 文案保留。86 项自检、25/25 定向变异及 verify/范围/许可/防泄露闸通过。执行前已按用户授权修订验收：禁 verdict 字段与 pass/block 状态值；review_status 仍留给 1b。
