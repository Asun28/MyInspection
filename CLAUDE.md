...
- **R3 pre-push + PR/Codex 评审代替人工**：ship 在 push 前依次强制 DoD、verify、范围、许可、防泄露与**真实 diff 预算闸**；随后 `review.ps1` 按 `docs/QUALITY-RUBRIC.md` 判（注入 rubric + 反自我开脱立场），出 `{verdict:pass|block}`→回贴 `codex-review` 状态；默认 blocking，仅基线明确授权的非权威竞品笔记闭集 advisory 可保留有效 `block` 并满足评审腿（rubric §0.1），关键/冻结/未知路径和运行故障仍阻断；
