...
- **[L17][L177] PowerShell 语义须由 PowerShell 工具与完整计数自证**：Bash 工具吞反斜杠路径（exit 64）且控制台编码与 pwsh 不同源，会让 `.ps1` 的路径/中文断言产出假结果；异常失败先用 PowerShell 工具重跑。PowerShell 变量名大小写不敏感，循环变量不得与集合变量仅大小写不同（如 `$m`/`$M`）；批量 mutation/证据结束时必须核“实际记录条数 == 计划条数”，不能只看已运行项均 OK。
