...
- **[L181][L190][L193] 不可见码位只写转义形态，且分类必须按 Unicode 标量验证**：写文件的工具层会把「反斜杠u四位十六进制」字面静默解码成真字符——组合符/控制符落盘后肉眼与显示层都看不见，黄金向量/双语内容/判别脚本首当其冲。凡源码字符串须钉死非 ASCII 或控制符内容：转义用代码拼出（`chr(92)+'u'`、`0xD800.toChar()`、`appendCodePoint`），写完立刻字节级验证；剥控制/格式字符只处理标量类别 `Cc/Cf`，禁止宽泛类别 `C`（还含 `Cs/Co/Cn`）以及直接用按 UTF-16 码元工作的 .NET regex 判增补类别；类别契约须由标量级权威 oracle 枚举全集，再逐个走生产机制验证，malformed UTF-16 fail-closed，并回归证明普通增补字符原样留存。判别工具与被测实现矛盾时先停手修工具，靶串找不到=中止而非「测不出」。
