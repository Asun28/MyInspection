...
## 当前阶段
<!-- 随 R5 文档同步更新。 -->
需求已收口 + **设计已定稿**（ADR-0001–0004）+ **用户已签认**（2026-08-15：ADR-0002 / 2 套以上物业部分在租 / 租客联系方式留 12 个月 / 不做双刻度与费用字段，见 `docs/TASK-BOARD.md`「用户已定」）。技术路线 = **原生 Kotlin + Compose**（ADR-0001）；任务卡 `specs/tasks/` 存未合并活卡、`specs/archive/tasks/` 存已合并历史，模型路由总表 `docs/TASK-BOARD.md`。
...
- **[L164][L171] 新入口与下游委托必须显式钉住信任边界**：本进程拒写/拒删不等于守住路径；凡把路径经环境变量、参数或回调交给下游，必须在唤起前经过与本进程写/删相同的安全判据，判不过即中止且不唤起。同一安全决策的 fail-closed 响应只留一个函数供所有站点复用，禁止第二站点各自复制。给既有 fail-closed 闸新增独立 CLI、恢复序列或 CI 腿时，还必须逐项显式绑定：判定对象钉不可变 id；判定标准/任务卡取受信基线；检查器及依赖取受信检出；入参只收纯名或完整 OID，并按完整提交身份精确比较；引用只解析一次，随后全程钉同一 SHA，禁止再读可变引用。
- **[L165][L167] 断言面必须恰好等于被测契约，并用「只删那一句」的变异证明它在测**：宽于契约的断言（整份 stdout ⊃ 判定行 · 本地化文案 ⊃ ASCII 哨兵 · 关键词出现次数 ⊃ 可执行命令行 · 任一非零 ⊃ 该守卫拦下）在契约还在时照样绿，契约被摘掉后又常因别的原因满足 ⇒ 静默失效。故：只比判定行 · 机检认 ASCII 哨兵（本地化文案只给人读，编码链一变即假红/假绿）· 文档契约锚到可执行命令行形态 · 「不符」用例要让被测那句**真被执行到**。**每道守卫配一枚单句删除变异，它红了才算数**。**变异本身也会撒谎**——`exit≠0` 可能来自靶未命中/语法坏/运行时异常/更早的闸抢先中断，故变异脚本须带**判据分类器**（只有「非零 **且** 命中指定断言文本」才算 OK；其余均是无效证据，须单独分类）。
- **[L17][L162][L172][L177] `.ps1` 一律用 PowerShell 工具，不用 Bash；批量证据必须完整计数**：Bash 工具吞反斜杠路径（exit 64）且控制台编码与 pwsh 不同源——后者会让 `selftest.ps1` 等中文断言脚本产出**假 FAIL**，连事后核验也会被误导；异常失败先用 PowerShell 工具重跑再下结论。凡在 harness 外（detached/计划任务/CI 外壳）启动 pwsh 跑本仓脚本，必须先 dot-source `scripts/_encoding.ps1`；机检文本优先 ASCII 哨兵，中文只给人读。第三方/插件 Python 读本仓文件前必须设 `PYTHONUTF8=1`；自有 Python 的 `open()`/`write_text()` 必须显式指定 `encoding='utf-8'`；需即时看到长驻进程输出时用 `python -u` 且避免接管道。PowerShell 变量名大小写不敏感，循环变量不得与集合变量仅大小写不同（如 `$m`/`$M`）；批量 mutation/证据结束时必须核“实际记录条数 == 计划条数”，不能只看已运行项均 OK。
...
- **[L196][L214] 变异前保全未提交工作；续接前与还原后都核 SHA256**：变异批/长批被会话拆除或进程树 kill 杀在「植入后、还原前」时，finally 不会跑，文件会跨会话停在变异态；而 `git checkout/restore -- <path>` 恢复的是 HEAD/索引，会抹掉同文件未提交的新工作。故被测文件须先提交，或先保存逐字节副本与基线 SHA 并只从副本还原；批启动及**每次会话续接第一步**都核当前 SHA==基线，不符先还原再谈 diff/证据；还原后再核 SHA256 完全一致。批须落 per-mutation 日志，续跑只补缺失枚、不整批重来。
- **[L181][L193] 不可见码位只写转义形态，且转义形态用代码拼**：写文件的工具层会把「反斜杠u四位十六进制」字面静默解码成真字符——组合符/控制符落盘后肉眼与显示层都看不见，黄金向量/双语内容/判别脚本首当其冲。凡源码字符串须钉死非 ASCII 或控制符内容：转义用代码拼出（`chr(92)+'u'`、`0xD800.toChar()`、`appendCodePoint`），写完立刻字节级验证；所谓剥控制/格式字符只匹配 `[\p{Cc}\p{Cf}]`，禁止宽泛 `\p{C}`（还含 `Cs/Co/Cn`，会拆坏代理对与合法增补平面字符），并须回归证明增补平面字符原样留存；判别工具与被测实现矛盾时先停手修工具，靶串找不到=中止而非「测不出」。
- **[L21][L205] R3 轮次先判证据真伪，修复轮也要本地对抗自检**：无裁决或明确配额错误表示评审者未真正运行，**不算真实评审分歧**；先在同一 worktree 独立 probe reviewer（不经 `review.ps1`、不碰 `.rounds`）并保全完整 stderr，确诊外部故障，禁止靠反复 `ship` 烧轮次，更不得绕过 R3。若外部失败已误耗尽 cap，只能在确诊后 `ResetRounds`，随后仍须正常过 R3。一轮修 ≥3 条 finding 的修复 diff 常大于首轮实现，且「按条修不看整体」使新代码不再被任何人当新代码审——ship 前先派 fresh-context/换模型子代理只对**本轮修复 diff** 按 rubric 复核（新错误处理是否 fail-open？点名目标是否全覆盖？改动文件自身注释/文案还成立吗）。W2 五卡有四张的 R3 轮次通胀皆此模式（修复自己引入下一轮的缺陷）。
