...
**当前已解锁待做**（按 depends_on 核对，2026-09-25）：前置均已合并的产品卡包括 `T1-LOCAL-SECRET-STORE`（前置 `T1-LOCAL-SECRET-BOX` 于 2026-09-25 合并）、`T2-GHOST-EDGE-OVERLAY` 与 `T3-REPORT-IMPORT-COMMIT`；`T1-LOCAL-DATA-SECURITY` 按 2026-09-25 的多 PR 拆分另待 `T1-LOCAL-SECRET-STORE`。`T3-PDF-RENDER-DEVICE` 的 `T1-SPIKE-PLATFORM` 真机 spike（master `e8c2359a`）与图片 bridge（master `fee6451f`）前置已满足，但仍待 `T3-PDF-TEXT-METRICS-OPS` 与 `T3-PDF-ANDROID-TEXT-MEASURER`；
...
- **[L190] 验证必须用被测代码那套机制**：验证一个断言时必须用**被测代码实际使用的那套机制**去验，不能用「语义上等价」的另一个 API（正则覆盖面就用 `-match` 实测、别查类目 API；编码/落盘行为就真写一遍文件再读回）；断言对象若是一个**类目/属性**，取样证不了它——须由标量级权威 oracle（`Rune.GetUnicodeCategory`）枚举全集，再逐个走**生产那套机制**验证。
