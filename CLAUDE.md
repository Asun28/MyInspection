...
**W5 Golden Evidence JVM Core E2E 已闭环**（2026-08-27，PR #180–#183，终态 master `99254ef`）：真实
...
内容且不泄露 landlord/private/remediation sentinel。`scripts/verify.ps1` Gate 2 现精确执行
`:core:test --tests nz.myinspection.core.e2e.* --offline --no-daemon`，wrapper 缺失、命令未执行、选择器错误或测试非零一律
fail-closed；Android UI、权限、TalkBack、模拟器/真机与进程死亡仍只属于 `T7-SMOKE-POLISH`，不混入 JVM E2E。
