...
**2026-09-07 需求审校**：卡片与安全接口规格已由 `T7-AUDIT-CARDS-CLOSURE` 本地合并（`d6e22084`，正式 R3 第2轮 pass）。11张新产品卡保持待实施；本轮同步版本与页面合同，不代表功能已完成。V1 预设/键盘、V1.1 批量照片、产品 V2 语音；物业备份 format v2 导出与恢复仍在 V1 发布范围。
...
- **CI 触发形态**：`ci.yml` 跑 `[main, master]` push+PR；`verify` 是必需检查。`scaffold-selftest.yml` 跑默认分支 push/每日/手动 canary，每个 OS 跑 core/workflow/seeded-git/remote/scanner 五片（8.2d/8.2e 锁死）。push 延后三处元层夹具至每日/手动；生产检查保留，详见 DEVOPS-WORKFLOW。合并前仍由卡 DoD + verify + R3 守门。
...
- **工作流自检**：`pwsh -File scripts\selftest.ps1`；完整 17 闸本地聚合 core/workflow/seeded，CI canary 用 2 OS × 5 片。任务卡可用 `pwsh -File scripts\selftest.ps1 -TaskId <id> -Base master` 选既有覆盖：基线卡与 FrozenPaths、分支身份、提交/暂存/脏/未跟踪改动任一不可判即以非零退出拒绝选择；普通产品改动只报 `NOT-APPLICABLE`（仍须做产品 DoD/verify），普通文档/卡片改动跑 core。
