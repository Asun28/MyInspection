...
internal object DocxExtractorFixture {
    ...
    fun p(text: String) = "<w:p><w:r><w:t xml:space='preserve'>$text</w:t></w:r></w:p>"
    fun story(root: String, body: String) =
        "<w:$root xmlns:w='$W' xmlns:r='$R' xmlns:a='$A' xmlns:wp='$WP'>$body</w:$root>"
    fun drawing(id: String, kind: String = "inline") =
    ...
    fun image(size: Int, seed: Int, format: String = "png"): ByteArray {
    ...
    fun parts(body: String = p("Unknown original observation")): LinkedHashMap<String, ByteArray> = linkedMapOf(
        ...
        "_rels/.rels" to relationships(relationship("office", "word/document.xml", "officeDocument")).toByteArray(),
        "word/document.xml" to story("document", "<w:body>$body</w:body>").toByteArray(),
    ).apply {
    ...
    fun sample(): LinkedHashMap<String, ByteArray> {
    ...
    fun zip(parts: Map<String, ByteArray>, stored: Boolean = true): ByteArray = ByteArrayOutputStream().also { bytes ->
    ...
    fun read(parts: Map<String, ByteArray>, stored: Boolean = true) =
