...
class ReportImageSourceTest {
    ...
    fun `the allowlist is the three raster formats a browser decodes without a plugin`() {
        assertEquals(setOf("image/jpeg", "image/png", "image/webp"), EmbeddedImage.ALLOWED_MEDIA_TYPES)
        for (type in EmbeddedImage.ALLOWED_MEDIA_TYPES) {
...
/*
 * R4 receipt. 9 single-point mutations, each applied alone to ReportImageSource.kt at SHA-256
 * 4e1cd178733ce83202d07f3fb45213532776710ed80d505feb883db5f31df6ec and restored to that same hash before
 * the next. 9 killed, 0 survived, 0 compile-kills, 0 no-runs.
 *
