export const meta = {
  ...
  description: '把(修正后的)计划任务章节投影为带依赖关系的可执行任务卡 + 5 角度对抗式卡片审计',
  phases: [
    { title: 'Decompose', detail: '读计划(+可选审计报告) → 产任务卡结构化定义' },
    { title: 'Card-Audit', detail: '5 并行 lens 对抗审卡: 右尺寸/拓扑/DoD可机检/硬边界许可/allow_paths覆盖' },
  ],
}
...
const DECISIONS_BLOCK = DECISIONS.length
  ? '计划已修正，卡必须吸收以下已确认决定，否则就是把旧坑写进卡:\n' +
    DECISIONS.map((d, i) => (i + 1) + '. ' + d).join('\n') + '\n'
  : ''

// T0-PLAN-FORGE-FOLLOWUP：发给模型的文字按下列依据精简（依据都在 docs/references/）：
//   1. 去掉【】括号与加粗：claude-prompting-best-practices-llms.txt「工具使用」（强调写法在新模型上会过度触发）与
//      「输出与格式」（提示的格式会带进输出）。括号作小标题处改冒号，作插入语处改圆括号。
//   2. 卡审提示里的「只报真问题」换成覆盖式写法：能指出依据的问题都报、按 severity 分级，只略过纯措辞与风格。
//      claude-opus-5-prompting-llms.txt「代码评审 harness」：发现阶段求覆盖，过滤另由一步做（这里是人审 FATAL/HIGH）。
//   3. 下面「右尺寸」那条是本仓卡片尺寸标准的定义处，他处引用它、勿另立。这句维护说明原在提示里，模型用不上
//      （best-practices「通用原则」黄金律），移到这里；右尺寸 lens 里的「见上 CONSTRAINTS」同理改成模型看得到的节名。
// 另：meta 描述、阶段说明、日志与注释里写明的卡审角度数改为 LENSES 的实际条目数（5，含右尺寸；阶段说明补上漏列的右尺寸）。这些不是提示词，依据是 LENSES 本身。
// 其余提示文字只去掉强调标记，检查内容不改。
const CONSTRAINTS =
  ...
  DECISIONS_BLOCK +
  '通用硬要求(项目无关):\n' +
  '- 右尺寸·一个可评审/可验证单元:卡的大小标准(工具/模型无关):每张卡必须是一个可评审、可验证的单元——单一连贯产出、一条 dod_command 二值验收、能被一个评审者(人或模型)一次评审判完对错、独立开工。判据:\n' +
  ...
  '        - 长自主档(受支持模式):跨多文件/多子系统的弧,由长自主执行者一次推进,须显式声明并配间隔 fresh-context 校验(每推进一段用独立、新上下文的 verifier 子代理对规格核对——见 docs/references)。声明了长自主执行 + 有间隔校验的卡,超默认档不判过大。\n' +
  '    · 单一产出:一张卡只交付一个连贯能力(一个模块/接口/端点/页面),一句话讲得清"产出物是什么"。讲不清或要用"且/和"连接多件事 → 拆。\n' +
  '    · 验收单一:dod_command 是一条可机检命令、判一件事。要好几条命令各判一块 → 拆成对应几张卡。\n' +
  '    · 评审可判:一个评审者(人或模型)能在一次评审里判完对错——diff 大到看不动,对人对模型都是失败。经验阈值:allow_paths 通常 ≤ 3-5 个文件/目录;明显超出 → 拆。\n' +
  '    · 自足性(卡可独立开工):卡内 plan_ref/notes 要携带足够上下文,使卡能被独立拉起——执行者无需"记得上一张卡"就能从这张卡开工;跨卡共享的契约/类型走冻结点卡显式依赖,不靠隐式上下文。\n' +
  ...
  '- 每卡 dod_command 在目标 shell(本模板默认 Windows/PowerShell)下真能跑且二值可判; import 路径/包根与目录结构一致。\n' +
...
log('decompose-cards: 投影计划任务章节为任务卡 + 5 角度对抗卡审')

const decomp = await agent(
  '把(修正后的)计划任务章节投影为完整的带依赖关系任务卡集(specs/tasks 的薄投影，非第二份计划)。\n' + CONSTRAINTS + '\n\n' +
...
const LENSES = [
  '右尺寸/一个可评审可验证单元(尺寸定义见下方通用硬要求的右尺寸一条): 每卡是否为单一连贯产出、一条 dod_command 判一件事、可一次评审判完(默认档量化缺省:净改动约≤200-400行、touched 文件≈1-3最多5)。报过大卡(产出含"且/和"多件事、dod 要多条命令、allow_paths 远超5 → 给出建议拆法)与过碎卡(一函数/一行配置一张、测试与实现分卡 → 建议合并)。也查卡是否自足(plan_ref/notes 让卡可独立开工)。severity: 超默认档且未声明长自主执行才判 HIGH(开工就会跑偏/返工);已声明长自主档(多文件弧+间隔 fresh-context 校验)的卡超默认档不算过大;过碎判 MEDIUM',
...
const audits = await parallel(
  // TD63 item7：各审计 agent 此前共用同一个 label 'cardaudit'，令并行运行时的日志/追踪无法区分是哪个
  // lens 产出的 —— 按序号区分（cardaudit1..cardaudit5）。
  LENSES.map((l, i) => () =>
    agent(
      '你是卡片审计裁判，只看一个角度: ' + l + '\n\n' + CONSTRAINTS + '\n\n' +
        '审计下面这套任务卡(允许 Read 仓库真实文件如依赖清单/specs/ 来核实, 不要臆测):\n' + JSON.stringify(decomp, null, 1) + '\n\n' +
        '返回 lens + graph_ok + issues[{card,problem,severity(FATAL/HIGH/MEDIUM),fix}]。能在卡字段或仓库文件里指出依据的问题都报，拿不准的也报并在 problem 里注明，每条按 severity 分级；纯措辞与风格偏好略过。能 Read 核实就核实。',
