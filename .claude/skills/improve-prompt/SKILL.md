...
- 出了新模型没有专属 reference → 按 `docs/references/<name>-llms.txt` 命名新增 + 登记
  ...
  **换代 ≠ 旧篇作废**：动手删旧模型那份之前先查它是否仍在服役——尤其是不是**拒答回退的兜底档**
  （Fable 5.1、Fable 5、Opus 5.5 与 Opus 5 都带安全分类器，官方默认路由按拒答类目把请求改道到推荐的兜底模型（该类目无推荐兜底则拒答维持）；四者共用的
  refusals-and-fallback 页示例选中的是 Opus 4.8——示例非契约，读响应的 `fallback` 块，别写死），或是不是新篇的**基线层**（Opus 5.5 页是
