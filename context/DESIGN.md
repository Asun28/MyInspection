...
| `task-stepper` | ordered labelled phases, a state mark for every complete and failed phase, and one current marker | `BACKUP / RESTORE / REPORT_IMPORT / ERASE`; upcoming/current/complete/failed | Shows no fake percentage; completed steps remain visible; import uses Details → Choose file → Scan → Match → Review → Create draft | One progress group; current phase is announced once, and a failed phase is announced | `Column` |
