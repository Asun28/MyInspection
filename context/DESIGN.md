...
| `REPORT_IMPORT` | Current step, selected property, user-confirmed tenancy and report date, current locked Routine template, and exact blocker count | `Details → Choose file → Scan → Match → Review → Create draft`; Details owns tenancy and report-date fields, while mapping summary then leads stable rows/photos. Exact status suggestions start `MATCHED` and remain blockers until individual or previewed bulk confirmation; unknown status, photo association and transient `UNREVIEWED_EXCLUDED` privacy each require explicit resolution | `Create editable draft` appears as the sole primary action only when every manifest row is terminal (`CONFIRMED` or `EXCLUDED`), the preview is current, and blockers are zero | Existing draft blocks entry. Missing/invalid tenancy or report date, picker/provider/space/process failure, a `MATCHED`/`ACTION_REQUIRED` row, or stale preview is an adjacent blocker; focus moves to its exact field/row. Live recreation preserves the operation, while process death follows the reset/commit matrix below |
...
#### Report import state, confirmation, and recovery contract

The import operation holder may retain the source grant, parsed manifest, mapping decisions, scroll, and focus only while its process is alive. Durable request state is bounded to selected `propertyId`, `tenancyId`, report date, locked template version, the safe phase (`DETAILS` or `CHOOSE_FILE`), and a non-sensitive focus key. It never persists a source URI/path/name, source text, manifest, thumbnail, mapping decision, or raw/staged bytes. Activity recreation in the same process reuses the holder and exact step; process death uses this exhaustive matrix:

| Interrupted point | Startup evidence/action | Restored route and focus |
| --- | --- | --- |
| `Details` | No source grant or staging exists; restore and revalidate the bounded Details values | `Details`; first invalid field, otherwise the prior Details focus key |
| `Choose file` | No source grant or staging exists | `Choose file`; the file action |
| `Scan` | Delete request staging, release transient source access, discard the manifest | `Choose file`; announce `Choose the source file again`, focus the file action |
| `Match` | Delete request staging, release transient source access, discard matches/decisions | `Choose file`; same announcement and focus; confirmed Details values remain |
| `Review` | Delete request staging, release transient source access, discard review decisions | `Choose file`; same announcement and focus; never claim that mappings were restored |
| `Create draft` before DB commit | Recovery marker proves no committed draft/receipt; remove staged/final orphan media and release source access | `Choose file`; explain that no draft was created and focus the file action |
| `Create draft` after atomic DB commit | Recovery marker plus draft/receipt/media digests verify the committed result; finish idempotent cleanup | Replace import with `INSPECTION_CAPTURE`, push `INSPECTION_REVIEW`, focus completeness/first gap |

Every manifest element owns exactly one row state. `MATCHED` is a non-terminal suggested target and a blocker; only explicit individual confirmation or the previewed exact-match bulk action moves it to `CONFIRMED`. `ACTION_REQUIRED` is non-terminal and a blocker. `CONFIRMED` is terminal mapped content. `EXCLUDED` is terminal only with an explicit reason and any required user confirmation. Transient `UNREVIEWED_EXCLUDED` keeps its photo row `ACTION_REQUIRED`; it is not the terminal `EXCLUDED` state. The summary counts `CONFIRMED` as mapped, `EXCLUDED` as excluded, and every `MATCHED`/`ACTION_REQUIRED` row plus global error as a blocker. `READY` requires a complete manifest, only terminal rows, a current preview, and zero blockers; no other transition enables commit.

Implementation acceptance is table-driven: inject process death at every row of the recovery matrix and assert cleanup, route, announcement, focus, and the absence/presence of one atomic draft exactly as declared.
...
| Import `Create editable draft` | Complete manifest; every row `CONFIRMED`/`EXCLUDED`; current preview; zero blockers; reviewed media staged; draft/receipt transaction succeeds | `REPLACE` import with `INSPECTION_CAPTURE(inspectionId)`, then `PUSH` | `INSPECTION_REVIEW(inspectionId)` | Push | Entry focuses completeness summary/first gap; Back → editable Capture; Save and exit → property hub |
...
| Report action `Open` | Selected artifact has passed close/reopen verification; external-copy boundary is acknowledged | `LAUNCH_SYSTEM` | PDF → `PDF_VIEWER`; HTML → `HTML_VIEWER` | System | Return → source Open action |
| Report action `Save` | Selected artifact has passed close/reopen verification; external-copy boundary is acknowledged | `LAUNCH_SYSTEM` | `REPORT_CREATE_DOCUMENT` | System | Return → source Save action |
| Report action `Share` | Selected artifact has passed close/reopen verification; external-copy boundary is acknowledged; its scoped temporary read grant is then created | `LAUNCH_SYSTEM` | `SHARE_SHEET` | System | Return → Share action |
...
| `SHARE_SHEET` | `SYSTEM_SURFACE` | Verified report Share after boundary acknowledgement and temporary grant creation | `T3-REPORT-EXPORT-UI` | Return restores Share action | `system:share-sheet` |
...
Report PDF/HTML `Open`, `Save`, `Share`, and `Copy notice` always name the boundary: `A copy will leave MyInspection and may be stored by another app.` Open/Save/Share remain disabled until the selected artifact has a close/reopen verification receipt and the adjacent boundary callout is acknowledged. Share creates its scoped temporary read grant only after those guards pass. Success means the system handoff opened—not delivery or storage.
...
Color is never the sole state channel, and no status is carried by a glyph alone. Every status carries at least one of its own visible text, its position, or text or an announcement given by its owner. Where a glyph also marks a status, its owner gives that value as text or announces it, exactly as the domain-value rule under symbol-only chrome requires, and the glyph comes from the declared set: check for OK, exclamation for attention, cross/octagon for blocked, dash for not applicable, and shield for privacy.
...
```json
{
  ...
  "semanticsRole": "merged traversal group (isTraversalGroup=true; no Role)",
  "liveRegion": "none",
  "minTouchTargetDp": 48,
...
| Component | Anatomy | Variants / states | Deterministic behaviour | Semantics and focus | Compose base |
| --- | --- | --- | --- | --- | --- |
| `search-field` | search icon, query, clear, optional result count | collapsed/focused/query/no results; machine `COLLAPSED / FOCUSED / QUERY / NO_RESULTS / DISABLED` | Debounce affects filtering only, not visible typing; Clear restores the unfiltered list and heading | SearchBar supplies editable-text semantics; no Role; results count is a polite announcement after the query settles | `SearchBar` |
...
| `summary-stat` | decision number, full label, optional status icon | neutral/complete/attention/blocked | Used only for evidence totals and required decisions, never vanity metrics or charts | Number, label and any expressed state merge into one phrase | `Column` |
...
- Counts use complete, plural-aware phrases: `1 photo needed`, `2 photos needed`, `1 item needs review`; never `1 items`. Every count carries its numeral and announces its full value even where it clamps visually; a badge that marks a state rather than a count carries no numeral and is governed by symbol-only chrome.
...
- Press feedback is immediate; standard state transitions use `motion.stateChangeMs` (`180ms`), sheet entry 250ms, and exit 150ms with Material easing. Motion communicates expansion, hierarchy, or save completion and never decorates a static screen.
...
Infinite pulse and indefinite ambient animation are forbidden.
