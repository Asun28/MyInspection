...
function ConvertFrom-SelftestFailureSentinel {
  ...
  $match = [regex]::Match($sentinelLines[0], '^\[SELFTEST-FAILED-GATES\] shard=(?<shard>core|workflow|seeded(?:-(?:git|remote|scanner))?) gates=(?<gates>[A-Za-z0-9.,()/_-]+)$')
...
function Test-SelftestCiMatrixContract([string]$WorkflowText) {
  ...
  $pairs = @([regex]::Matches($matrix.Groups['body'].Value, '(?m)^\s{10}- os:\s*(windows-latest|ubuntu-latest)\s*\r?\n\s{12}shard:\s*(core|workflow|seeded-git|seeded-remote|seeded-scanner)\s*$') |
    ForEach-Object { "$($_.Groups[1].Value)/$($_.Groups[2].Value)" })
  $expected = @(
    'ubuntu-latest/core', 'ubuntu-latest/seeded-git', 'ubuntu-latest/seeded-remote', 'ubuntu-latest/seeded-scanner', 'ubuntu-latest/workflow',
    'windows-latest/core', 'windows-latest/seeded-git', 'windows-latest/seeded-remote', 'windows-latest/seeded-scanner', 'windows-latest/workflow'
  )
...
function Test-SelftestGateIdContract {
  param(
    ...
    [Parameter(Mandatory)][string]$InsertionAnchor
  )

  $tokens = $null
  $parseErrors = $null
  $parseFixturePath = Join-Path ([System.IO.Path]::GetTempPath()) ("selftest-gate-id-" + [guid]::NewGuid().ToString('N') + '.ps1')
  try {
    [System.IO.File]::WriteAllText($parseFixturePath, $Source, [System.Text.UTF8Encoding]::new($false))
    $ast = [System.Management.Automation.Language.Parser]::ParseFile($parseFixturePath, [ref]$tokens, [ref]$parseErrors)
  } finally {
    [System.IO.File]::Delete($parseFixturePath)
  }
  ...
  $declarations = @(
    foreach ($token in $tokens) {
      ...
      # Exactly one space after # distinguishes a gate heading from indented prose such as "#   15j:".
      # The lookahead prevents "8.0c" from backtracking to a false "8." declaration.
      $heading = [regex]::Match($token.Text, '^# (?:--- )?(?<id>[0-9A-Za-z.]+)(?<separator>[.:：])(?=\s|[^\w.]|$)')
      if (-not $heading.Success) { continue }
      $id = $heading.Groups['id'].Value
      if ($id -notmatch '^\d+(?:\.\d+)?[a-z]*\d*$') { continue }
  ...
  $failMessageByOffset = @{}
  $failMessages = @(
    foreach ($command in $failCommands) {
      ...
      if ($messageId.Success) {
        $message = [pscustomobject]@{ Id = $messageId.Groups['id'].Value; Line = $command.Extent.StartLineNumber }
        $failMessageByOffset[[string]$command.Extent.StartOffset] = $message
        $message
      }
    }
  )
  if ($failMessages.Count -eq 0) { return [pscustomobject]@{ Ok = $false; Code = 'GATE-ID-SCAN-EMPTY'; Id = ''; Locations = @(); Detail = 'no 闸<id> failure messages found' } } # GATE-ID-GUARD-SCAN-EMPTY
  ...
  # Build the report side from actual Fail ownership. An explicit 闸<id> contributes only its
  # preceding lexical heading owners; an unnumbered Fail owns headings opened since the previous
  ...
  foreach ($failCommand in $failCommands) {
    ...
    if ($failMessageByOffset.ContainsKey($messageKey)) {
      ...
      if ($owners.Count -eq 0) {
        return [pscustomobject]@{ Ok = $false; Code = 'GATE-ID-MESSAGE-MISMATCH'; Id = $message.Id; Locations = @($message.Line); Detail = "failure message id=$($message.Id) has no preceding gate heading" }
      }
      foreach ($owner in $owners) { [void]$reportedMessageIds.Add($owner.Id) }
    } else {
      foreach ($declaration in $declarations | Where-Object {
          $_.Line -gt $previousFailLine -and $_.Line -lt $failCommand.Extent.StartLineNumber
        }) {
        [void]$reportedMessageIds.Add($declaration.Id)
      }
    }
    $previousFailLine = $failCommand.Extent.StartLineNumber
  }
  ...
  if ([string]::Join([char]0, $reportedHeadingIdSet) -cne [string]::Join([char]0, $reportedMessageIdSet)) { $firstMismatch = @($reportedHeadingIdSet + $reportedMessageIdSet | Group-Object | Where-Object Count -eq 1 | Sort-Object Name | Select-Object -First 1); return [pscustomobject]@{ Ok = $false; Code = 'GATE-ID-MESSAGE-MISMATCH'; Id = if ($firstMismatch) { $firstMismatch[0].Name } else { '' }; Locations = @(); Detail = "heading ids=$($reportedHeadingIdSet -join ','); failure-owner ids=$($reportedMessageIdSet -join ',')" } } # GATE-ID-GUARD-MESSAGE-SET

  return [pscustomobject]@{ Ok = $true; Code = 'OK'; Id = ''; Locations = @(); Detail = "gate headings=$($declarations.Count); failure-owner ids=$($reportedMessageIdSet.Count); explicit messages=$($failMessages.Count)" }
}

if ($Fixture -eq 'gate-id-mutant') {
  if (-not $GateIdMutation) { throw '[GATE-ID-MUTATION-SETUP] -GateIdMutation is required.' }
  $gateIdMutantSource = Get-Content -LiteralPath $PSCommandPath -Raw
  $gateIdMutantTokens = $null
  $gateIdMutantParseErrors = $null
  $gateIdMutantAst = [System.Management.Automation.Language.Parser]::ParseInput(
    $gateIdMutantSource, [ref]$gateIdMutantTokens, [ref]$gateIdMutantParseErrors)
  $gateIdMutantFunctions = @($gateIdMutantAst.FindAll({
        param($node)
        $node -is [System.Management.Automation.Language.FunctionDefinitionAst] -and
          $node.Name -ceq 'Test-SelftestGateIdContract'
      }, $true))
  if ($gateIdMutantParseErrors -or $gateIdMutantFunctions.Count -ne 1) {
    throw "[GATE-ID-MUTATION-SETUP] contract function parse/count invalid: parseErrors=$(@($gateIdMutantParseErrors).Count) functions=$($gateIdMutantFunctions.Count)."
  }
  $gateIdMutantFunctionLines = @($gateIdMutantFunctions[0].Extent.Text -split "`r?`n")
  $gateIdMutantMarker = 'GATE-ID-GUARD-' + $GateIdMutation
  $gateIdMutantGuardIndexes = @(for ($lineIndex = 0; $lineIndex -lt $gateIdMutantFunctionLines.Count; $lineIndex++) {
      if ($gateIdMutantFunctionLines[$lineIndex].Contains($gateIdMutantMarker)) { $lineIndex }
    })
  if ($gateIdMutantGuardIndexes.Count -ne 1) {
    throw "[GATE-ID-MUTATION-SETUP] expected one $gateIdMutantMarker sentence, found $($gateIdMutantGuardIndexes.Count)."
  }
  $gateIdMutatedFunctionLines = @(for ($lineIndex = 0; $lineIndex -lt $gateIdMutantFunctionLines.Count; $lineIndex++) {
      if ($lineIndex -ne $gateIdMutantGuardIndexes[0]) { $gateIdMutantFunctionLines[$lineIndex] }
    })
  if ($gateIdMutatedFunctionLines.Count -ne ($gateIdMutantFunctionLines.Count - 1)) {
    throw '[GATE-ID-MUTATION-SETUP] mutation did not delete exactly one sentence.'
  }
  $gateIdMutantName = 'Test-SelftestGateIdContractMutant' + ($GateIdMutation -replace '-', '')
  $gateIdMutatedFunctionSource = $gateIdMutatedFunctionLines -join "`n"
  $gateIdMutatedFunctionSource = ([regex]::new('function Test-SelftestGateIdContract\b')).Replace(
  ...
  $gateIdMutantCases = @(switch ($GateIdMutation) {
    'DUPLICATE' {
    ...
    'SCAN-EMPTY' {
      [pscustomobject]@{ Name = 'empty'; Code = 'GATE-ID-SCAN-EMPTY'; Source = "$gateIdMutantAnchor`n" }
    }
    'ANCHOR' {
      [pscustomobject]@{ Name = 'anchor'; Code = 'GATE-ID-ANCHOR-NONUNIQUE'; Source = $gateIdMutantValidSource + "`n$gateIdMutantAnchor`n" }
    }
    'PARSE' {
  ...
  $gateIdMutationFailures = [System.Collections.Generic.List[string]]::new()
  foreach ($gateIdMutantCase in $gateIdMutantCases) {
    $gateIdMutantResult = & $gateIdMutantName -Source $gateIdMutantCase.Source -InsertionAnchor $gateIdMutantAnchor
    if ($gateIdMutantResult.Code -ceq $gateIdMutantCase.Code) {
      Write-Host "[GATE-ID-MUTATION-SURVIVED-$GateIdMutation] deleted guard still returned $($gateIdMutantCase.Code) for $($gateIdMutantCase.Name)."
      $gateIdMutationSurvived = $true
    } else {
      [void]$gateIdMutationFailures.Add("[GATE-ID-FIXTURE-$($gateIdMutantCase.Name)] expected code=$($gateIdMutantCase.Code); actual code=$($gateIdMutantResult.Code).")
    }
  }
  if ($gateIdMutationSurvived) { exit 0 }
  Write-Error ($gateIdMutationFailures -join ' ')
  exit 1
}
...
# A1's original false-green was a workflow run. Enforce the repository contract in the two dynamic
# gate shards named by the card; workflow also owns the full Gate 1i fixture/mutation matrix below.
if ($Shard -ceq 'workflow' -or $Shard -like 'seeded*') {
  ...
  if (-not $gateIdPreflightResult.Ok) {
    if ($gateIdPreflightResult.Code -ceq 'GATE-ID-DUPLICATE') {
      Fail "[GATE-ID-DUPLICATE] id=$($gateIdPreflightResult.Id) locations=$(@($gateIdPreflightResult.Locations) -join ',') detail=$($gateIdPreflightResult.Detail)"
    } else {
      Fail "[GATE-ID-PREFLIGHT-$($gateIdPreflightResult.Code)] id=$($gateIdPreflightResult.Id) locations=$(@($gateIdPreflightResult.Locations) -join ',') detail=$($gateIdPreflightResult.Detail)"
    }
    exit 1
  }
}
...
# 1i. Gate identifiers are coordination keys: a duplicate declaration makes concurrent additions
...
if (-not $gateIdContractCommand) {
  ...
  $wrongOwnerGateIdSource = $validGateIdSource.Replace($gateIdInsertionAnchor,
  ...
  foreach ($gateIdCase in $gateIdCases) {
    ...
    if ($gateIdResult.Ok -ne $gateIdCase.Ok -or $gateIdResult.Code -cne $gateIdCase.Code -or
        $gateIdResult.Id -cne $gateIdCase.Id -or $gateIdLocations -cne $gateIdCase.Locations) {
      Fail "[GATE-ID-FIXTURE-$($gateIdCase.Name)] expected ok=$($gateIdCase.Ok) code=$($gateIdCase.Code) id=$($gateIdCase.Id) locations=$($gateIdCase.Locations); actual ok=$($gateIdResult.Ok) code=$($gateIdResult.Code) id=$($gateIdResult.Id) locations=$gateIdLocations."
      $gateIdFixtureOk = $false
    }
  }
  ...
  foreach ($mutationAssertion in $gateIdMutationAssertions) {
    ...
    $missingMutationAssertions = @($mutationAssertion.Assertions | Where-Object { $mutationOutput -notmatch [regex]::Escape($_) })
    if ($mutationExit -eq 0 -or $missingMutationAssertions.Count -gt 0) {
      Fail "[GATE-ID-MUTATION-$($mutationAssertion.Suffix)] expected nonzero plus $($mutationAssertion.Assertions -join ','); missing=$($missingMutationAssertions -join ',') exit=$mutationExit output=$(($mutationOutput -replace '\s+', ' ').Trim())"
      $gateIdFixtureOk = $false
    }
  }
  ...
  if (-not $repositoryGateIdResult.Ok) {
    if ($repositoryGateIdResult.Code -ceq 'GATE-ID-DUPLICATE') {
      Fail "[GATE-ID-DUPLICATE] id=$($repositoryGateIdResult.Id) locations=$(@($repositoryGateIdResult.Locations) -join ',') detail=$($repositoryGateIdResult.Detail)"
    } else {
      Fail "[GATE-ID-REPOSITORY-$($repositoryGateIdResult.Code)] id=$($repositoryGateIdResult.Id) locations=$(@($repositoryGateIdResult.Locations) -join ',') detail=$($repositoryGateIdResult.Detail)"
    }
    $gateIdFixtureOk = $false
  }
...
if (-not $svMeta) { Fail '_config.ps1 缺 ScaffoldVersion current 字段。' }
elseif ($svMeta -notmatch '^\d+\.\d+\.\d+$') { Fail "ScaffoldVersion='$svMeta' 非 semver（应 x.y.z）。" }
if (-not $svOriginMeta) { Fail '_config.ps1 缺 ScaffoldOriginVersion provenance 字段。' }
...
try {
  ...
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'init-scaffold.ps1') -Destination (Join-Path $versionInitRoot 'init-scaffold.ps1')
  Copy-Item -LiteralPath (Join-Path $RepoRoot 'scripts/_config.ps1') -Destination (Join-Path $versionInitRoot 'scripts/_config.ps1')
  Set-Content -LiteralPath (Join-Path $versionInitRoot 'CLAUDE.template.md') -Value 'generated by scaffold v{{SCAFFOLD_VERSION}}' -Encoding utf8
  & pwsh -NoProfile -File (Join-Path $versionInitRoot 'init-scaffold.ps1') -ProjectName 'Version Child' -ProjectSlug 'version-child' -GhAccount 'smoke' -Force *> $null
  $versionInitExit = $LASTEXITCODE
  $versionChildConfig = Get-Content -LiteralPath (Join-Path $versionInitRoot 'scripts/_config.ps1') -Raw
...
function Test-LicenseGateAfterGradleWarmup([string]$WorkflowText) {
  ...
  return ($actualHash -ceq 'B813DA389397711A8E8E234AF8FB222FFD4A9D11DD7FC687366A52F6FBDD3B04' -and
...
if (-not (Test-Path $arScript)) { Fail '12e：scripts\archive.ps1 不存在（冷存压缩引擎缺失，TD86/T28 未落地）。' }
  ...
  $equalLengthDrift = [Text.UTF8Encoding]::new($false).GetString($correctCardsIndexBytes).Replace('merged three', 'merged threw')
  if ([Text.UTF8Encoding]::new($false).GetByteCount($equalLengthDrift) -ne $correctCardsIndexBytes.Length) {
    $arFail += '同长度索引漂移夹具未保持字节数（测试设置失效）'
  } else {
    [IO.File]::WriteAllText($cardsIndexPath, $equalLengthDrift, [Text.UTF8Encoding]::new($false))
    $equalLengthDriftHash = (Get-FileHash -LiteralPath $cardsIndexPath -Algorithm SHA256).Hash
    $cardsCheckEqualOut = & pwsh -NoProfile -File $arScript -RepoRoot $ar -CheckCardsIndex -Quiet 2>&1 | Out-String
    $cardsCheckEqualCode = $LASTEXITCODE
    if ($cardsCheckEqualCode -eq 0) { $arFail += '同长度 title 字节替换仍 exit 0（逐字节比较未承重）' }
    elseif ([regex]::Matches($cardsCheckEqualOut, '\[ARCHIVE-CARDS-INDEX-DRIFT\]').Count -ne 1) { $arFail += "同长度漂移未精确输出一枚 [ARCHIVE-CARDS-INDEX-DRIFT]：$cardsCheckEqualOut" }
    if ((Get-FileHash -LiteralPath $cardsIndexPath -Algorithm SHA256).Hash -ne $equalLengthDriftHash) { $arFail += '同长度漂移检查修改/自修了夹具索引' }
    [IO.File]::WriteAllBytes($cardsIndexPath, $correctCardsIndexBytes)
  }
  ...
  elseif ([regex]::Matches($verifyDriftOut, '\[ARCHIVE-CARDS-INDEX-DRIFT\]').Count -ne 1) { $arFail += "verify 非零但 archive 子进程状态码不是唯一一枚 [ARCHIVE-CARDS-INDEX-DRIFT]：$verifyDriftOut" }
...
if (Test-SeededShardRegionSelected -Region 'git') {
...
$runSeededGitRegion = Test-SeededShardRegionSelected -Region 'git'
if ($runSeededGitRegion -and -not $runSeededGitGates) {
...
if (Test-SeededShardRegionSelected -Region 'scanner') {
...
#   「取内容 → 判定 → 打印 MARKER → 按结果退出 0/1」，外层只读子进程的退出码 + MARKER 文本——
#   卡片 dod_assert 字面要求「非零 且 命中指定断言文本」才算杀死变异，不是任意 nonzero、不是任意文本；
#   17cc(case)/17ee 的每枚变异进一步声明**它该红在哪一条子断言**上并锚定精确匹配失败码。
#   变异对象一律不是生产文件本体（临时同目录/暂存副本），结尾核**三份**生产文件 SHA256 全程未变（L196）：
#   scripts/check-licenses.ps1、scripts/verify.ps1、docs/RELEASE-CHECKLIST.md。
#   （命名 17cc/17dd/17ee 避开既有 17aa TD68 远端基线评审套件撞号。）
...
#   判据**只有一份**，写在子进程脚本块 $caseProbe 里：三个比较器各自的两模式不变量、缺省跟随文件系统、
#   以及端到端命中集逐字比对**全在它里面**。基线跑一次未变异副本 = 本闸的真断言；每枚变异跑同一份判据 =
#   真在测那条断言。**绝不在父进程另写一份等价判据**——那样删掉真断言后变异照样绿（vacuous mutation）。
#   失败分五种 ABSENT 后缀（EQUALS-MODES / LIST-MODES / PREFIX-MODES / UNIT-DEFAULT / HITSET），
#   且**每枚变异声明它该红在哪一条**并做锚定精确匹配——否则一枚变异可能因别的子断言先失败而被误判为已杀死。
...
try {
  # 五个调用点各配探针。排除列表**显式传参**、且经 -Args 传进子进程（父子共用同一份字面量，不各写一份）：
  # 本闸测的是比较语义与守卫接线，真实 ignore 清单由 17cc(prune) 守。
  $caseSkipDirs = 'build'
  $caseSkipRelativePaths = 'wrong/data|right/data'
  $caseAndroidSkipDirs = '.kotlin'
  $caseNames = 'build.gradle'
  $caseProbes = @(
    @{ Rel = 'wrong/Build/x/build.gradle';               Kind = 'wrong'; Axis = '守卫② SkipDirs 名称级排除' }
    @{ Rel = 'right/build/x/build.gradle';               Kind = 'right'; Axis = '守卫② SkipDirs 名称级排除' }
    @{ Rel = 'wrong/Data/x/build.gradle';                Kind = 'wrong'; Axis = '守卫③ SkipRelativePaths 路径级排除' }
    @{ Rel = 'right/data/x/build.gradle';                Kind = 'right'; Axis = '守卫③ SkipRelativePaths 路径级排除' }
    @{ Rel = 'Android/deep/.kotlin/x/build.gradle';      Kind = 'wrong'; Axis = '守卫④ android/ 前缀（前缀错大小写）' }
    @{ Rel = 'android/deep2/.KOTLIN/x/build.gradle';     Kind = 'wrong'; Axis = '守卫④ AndroidSkipDirs（目录名错大小写，前缀正确）' }
    @{ Rel = 'android/deep/.kotlin/x/build.gradle';      Kind = 'right'; Axis = '守卫④ 全部正确大小写' }
    @{ Rel = 'names/BUILD.GRADLE';                       Kind = 'wrong'; Axis = 'Names 清单文件名' }
    @{ Rel = 'keep/build.gradle';                        Kind = 'ctrl';  Axis = '对照（正确大小写，两平台恒命中）' }
  )
  ...
  # 不敏感文件系统（Windows/macOS）：错大小写 == 正确大小写，故三道排除对两支都生效、BUILD.GRADLE 算清单。
  # 敏感文件系统（Linux/CI）：错大小写 ≠ 被 ignore 的那个，故排除只对正确大小写那支生效、BUILD.GRADLE 不算清单。
  #   注意 android/deep2/.KOTLIN 这一支：前缀正确、只有目录名错大小写——它是 **AndroidSkipDirs 比较器的
  #   casing 覆盖**（前缀那支变了大小写就走不到名称比较，测不到这一段）。
  $caseInsensitiveFs = ($IsWindows -or $IsMacOS)
  $expectedCaseHits = @(if ($caseInsensitiveFs) {
      'keep/build.gradle'; 'names/BUILD.GRADLE'
    } else {
      'Android/deep/.kotlin/x/build.gradle'; 'android/deep2/.KOTLIN/x/build.gradle'; 'keep/build.gradle'
      'wrong/Build/x/build.gradle'; 'wrong/Data/x/build.gradle'
    }) | Sort-Object
  ...
  $caseProbe = {
    param([string]$LibraryPath, [string]$ProbeRoot, [string]$Expected, [string]$MarkerId, [string]$InsensitiveFs,
          [string]$SkipDirs, [string]$SkipRelPaths, [string]$AndroidSkips, [string]$Names)
    Set-StrictMode -Version Latest
    $ErrorActionPreference = 'Stop'
    . $LibraryPath -AsLibrary
    # 三个比较器各自的两模式不变量：显式传 -Comparison 时答案必须相反（证语义真被参数化，而非写死一种）。
    $eqOrd = [bool](Test-GradleNameEquals -Left 'Build' -Right 'build' -Comparison ([System.StringComparison]::Ordinal))
    $eqIgn = [bool](Test-GradleNameEquals -Left 'Build' -Right 'build' -Comparison ([System.StringComparison]::OrdinalIgnoreCase))
    if ($eqOrd -or (-not $eqIgn)) { Write-Output "MARKER:${MarkerId}:ABSENT-EQUALS-MODES(ord=$eqOrd,ign=$eqIgn)"; exit 1 }
    $lsOrd = [bool](Test-GradleNameInList -List @('build') -Value 'Build' -Comparison ([System.StringComparison]::Ordinal))
    $lsIgn = [bool](Test-GradleNameInList -List @('build') -Value 'Build' -Comparison ([System.StringComparison]::OrdinalIgnoreCase))
    if ($lsOrd -or (-not $lsIgn)) { Write-Output "MARKER:${MarkerId}:ABSENT-LIST-MODES(ord=$lsOrd,ign=$lsIgn)"; exit 1 }
    $pfOrd = [bool](Test-GradlePathPrefix -Path 'Android/x' -Prefix 'android/' -Comparison ([System.StringComparison]::Ordinal))
    $pfIgn = [bool](Test-GradlePathPrefix -Path 'Android/x' -Prefix 'android/' -Comparison ([System.StringComparison]::OrdinalIgnoreCase))
    if ($pfOrd -or (-not $pfIgn)) { Write-Output "MARKER:${MarkerId}:ABSENT-PREFIX-MODES(ord=$pfOrd,ign=$pfIgn)"; exit 1 }
    # 缺省语义必须跟随本机文件系统（三个比较器共用同一个 $gradlePathComparison，测其一即测缺省源）。
    $def = [bool](Test-GradleNameEquals -Left 'Build' -Right 'build')
    if ($def -ne [bool]::Parse($InsensitiveFs)) { Write-Output "MARKER:${MarkerId}:ABSENT-UNIT-DEFAULT(def=$def)"; exit 1 }
    # 端到端：一棵树、五个调用点，命中集必须**逐字等于**本平台期望集（不是"包含/不包含"的宽断言，L165）。
    $h = @(Find-GradleManifests -Root $ProbeRoot -Names ($Names -split '\|') -SkipDirs ($SkipDirs -split '\|') -SkipRelativePaths ($SkipRelPaths -split '\|') -AndroidSkipDirs ($AndroidSkips -split '\|'))
    $rel = @($h | ForEach-Object { [System.IO.Path]::GetRelativePath($ProbeRoot, $_).Replace('\', '/') } | Sort-Object) -join '|'
    if ($rel -ne $Expected) { Write-Output "MARKER:${MarkerId}:ABSENT-HITSET(got=$rel)"; exit 1 }
    Write-Output "MARKER:${MarkerId}:PRESENT"; exit 0
  }
  # 判据调用的固定尾参（除 MarkerId 外全同），基线与每枚变异共用——参数一变两边同时变，不会漂移。
  $caseTail = @($caseInsensitiveFs.ToString(), $caseSkipDirs, $caseSkipRelativePaths, $caseAndroidSkipDirs, $caseNames)
  # **必须是函数、不能是 GetNewClosure() 的脚本块**：闭包会把 $LASTEXITCODE 这类自动变量在建闭包那一刻冻进
  # 私有作用域，闭包体内 & pwsh 更新的是外层动态作用域的值，闭包读到的还是冻结时的旧值（子进程明明 exit 1，
  # 父进程却读成 0，判据遂"退出 0 + ABSENT"自相矛盾）。函数体内读则取的是调用后的新鲜值——同
  # Invoke-MarkerAssertion 的写法，与 17cc(reparse-mut) 处那段注记同源。
  function Invoke-CaseProbe([scriptblock]$Body, [string[]]$ArgList) {
  ...
  Set-Content -LiteralPath $mutantCaseCL -Value $caseLines -Encoding utf8   # 规整化（同 17cc 主体）：令后续「变异→还原」哈希可比。
  $caseBase = Invoke-CaseProbe -Body $caseProbe -ArgList (& $caseProbeArgs 'GRADLE-CASE-BASE')
  if (Test-MarkerResult $caseBase 'GRADLE-CASE-BASE' $true "种子缺陷 17cc(case)：本机文件系统大小写$fsLabel，三个比较器的两模式不变量、缺省语义或命中集（期望 [$($expectedCaseHits -join ', ')]）不符——五个调用点里至少一个仍在用恒不敏感的比较或恒敏感的前缀判断") {
    Write-Host "  17cc(case) 基线（GREEN）：Equals/InList/Prefix 三比较器两模式各自相反 + 缺省跟随文件系统（$fsLabel）+ 五个调用点命中集逐字等于期望集（$($expectedCaseHits.Count) 项）OK" -ForegroundColor Green

    # 变异组。删除变异一律复用既有 Invoke-LineDeletionMutation（定位唯一行 → 删 → 跑判据 → 还原 → 核 SHA256），
    # 不另起一套 copy/locate/mutate/restore 循环（卡片 non_goals：复用 17cc/17dd 的 harness）。
    # 唯一例外是比较器缺省那一枚：它**只能替换、不能删除**——删掉该行后 $gradlePathComparison 未定义，子进程会
    # 崩在 StrictMode 而非给出错误命中集，那是"崩溃杀"，证明不了语义。替换成**本平台的错误语义**才是行为杀
    # （不敏感机器换成恒敏感、敏感机器换成恒不敏感；用同一常量则会在其中一个平台假幸存）。
    $wrongComparison = if ($caseInsensitiveFs) { '[System.StringComparison]::Ordinal' } else { '[System.StringComparison]::OrdinalIgnoreCase' }
    $caseMutations = @(
      @{ Id = 'guard2'; Code = 'ABSENT-HITSET'; Label = '守卫② SkipDirs 名称级排除'
         Marker = '$skip = Test-GradleNameInList -List $SkipDirs -Value $e.Name' }
      @{ Id = 'guard3'; Code = 'ABSENT-HITSET'; Label = '守卫③ SkipRelativePaths 路径级排除'
         Marker = '$skip = Test-GradleNameInList -List $SkipRelativePaths -Value $relativePath' }
      @{ Id = 'guard4'; Code = 'ABSENT-HITSET'; Label = '守卫④ android/ 前缀 + AndroidSkipDirs'
         Marker = 'Test-GradlePathPrefix -Path $relativePath -Prefix ''android/''' }
      @{ Id = 'names'; Code = 'ABSENT-HITSET'; Label = 'Names 清单文件名收集行'
    ...
    # 缺省语义替换变异（唯一非删除的一枚，理由见上）。
    $defMarker = '$gradlePathComparison = if ($IsWindows -or $IsMacOS)'
    $defIdx = @(0..($caseLines.Count - 1) | Where-Object { $caseLines[$_] -match [regex]::Escape($defMarker) })
    if ($defIdx.Count -ne 1) {
      Fail "17cc(case-mut/default) 前置：源码里含比较器缺省定义「$defMarker」的行数=$($defIdx.Count)（期望恰好 1）——变异定位不唯一，源码可能已漂移。"
    } else {
      $defBaselineHash = (Get-FileHash -LiteralPath $mutantCaseCL -Algorithm SHA256).Hash
      $defMut = $caseLines.Clone(); $defMut[$defIdx[0]] = "`$gradlePathComparison = $wrongComparison"
      Set-Content -LiteralPath $mutantCaseCL -Value $defMut -Encoding utf8
      $defRes = Invoke-CaseProbe -Body $caseProbe -ArgList (& $caseProbeArgs 'GRADLE-CASE-MUT-DEFAULT')
      Set-Content -LiteralPath $mutantCaseCL -Value $caseLines -Encoding utf8
      $defRestored = (Get-FileHash -LiteralPath $mutantCaseCL -Algorithm SHA256).Hash
  ...
  if (Test-Path $mutantCaseCL) { Fail "17cc(case) 收尾：临时同目录副本 $mutantCaseCL 未能删除——请手动清理，避免 git status 出现 ?? 残留。" }
}
...
try {
  ...
  if (-not $canLink) {
    ...
    if ($leakedHits.Count -gt 0) {
      ...
      $guardMarker = '$skip = [bool]($e.Attributes -band [System.IO.FileAttributes]::ReparsePoint)'
      $guardWeakened = '$skip = $false'
      ...
      if ($reparseGuardIdx.Count -ne 1) {
        ...
        $mutReparseLines[$reparseGuardIdx[0]] = $reparseLines[$reparseGuardIdx[0]].Replace($guardMarker, $guardWeakened)
...
#   判据锚 ASCII 哨兵（L165：本地化文案只给人读，编码链一变即假红/假绿）。
#   **判据只有一份**，写在子进程脚本块 $rcProbe 里：真实清单跑它 = 本闸的真断言；每枚变异把改过的内容写进
#   暂存副本、跑**同一个** $rcProbe = 真在测那条断言。绝不在父进程另写一份等价谓词——那样"变异"只证明副本
#   自洽，把真断言删掉照样绿（vacuous mutation）。
#   **为什么钉整行 SHA-256 而不是"不含②"**：`②` 只是当前那条替代路径**碰巧用的字形**；写成「或完成人工逐条
#   核验后即可勾选」照样是第二条解锁路径，却不含该字形——"不含某字形"这个断言**窄于契约**，测不出改写形态
#   （L165：断言面必须恰好等于被测契约）。任意散文替代无法用模式穷举，故改为**钉死这一项的规范文本**：
#   逐字节 SHA-256 匹配。代价是措辞一改就红——这正是意图：这一项是**人裁定下的契约**，改它必须是显式动作
#   （连同本闸的期望哈希一起改），而不是某次顺手润色悄悄带过去。
...
$rcCanonHash = '90A40E0A3CEFC75ABDE5B836177E3FB410BFE28D8B79D515E996841EBC80695F'
...
$rcProbe = {
  ...
  if ($actual -ne $CanonHash) { Write-Output "MARKER:${MarkerId}:ABSENT-CANON(actual=$actual)"; exit 1 }
  Write-Output "MARKER:${MarkerId}:PRESENT"; exit 0
}
...
try {
  ...
  if (Test-MarkerResult $rcBase 'RC-GATE-BASE' $true "种子缺陷 17ee：发布清单的 Gradle 阻断项未满足单一解锁契约（哨兵「$rcSentinel」恰好 1 处 + 紧邻编辑警示 + 指名 T0-LICENSE-SCANNER + 规范文本 SHA-256 == $rcCanonHash；措辞被改过就要连同本闸的期望哈希一起显式更新）") {
    ...
    $rcAltNumbered = ' 两者选一即满足：' + [char]0x2461 + ' 人工逐条核验并登记进 docs/LICENSE-POLICY.md 第 3 节。'
    $rcAltProse = ' 或者，完成人工逐条核验并登记后亦可勾选本项。'
    $rcMutations = @(
      @{ Id = 'sentinel'; Code = 'ABSENT-SENTINEL'; Label = '删掉整条 Gradle 阻断项（单句删除）'
         Lines = @(for ($i = 0; $i -lt $rcOrigLines.Count; $i++) { if ($i -ne $rcIdx[0]) { $rcOrigLines[$i] } }) }
      @{ Id = 'unlock'; Code = 'ABSENT-UNLOCK'; Label = '抹掉唯一解锁点 T0-LICENSE-SCANNER 的指名'
         Lines = @($rcOrigLines | ForEach-Object { $_.Replace('T0-LICENSE-SCANNER', 'TBD') }) }
      @{ Id = 'altnumbered'; Code = 'ABSENT-CANON'; Label = '把带序号的人工替代写回该项（本次人裁的真实回归）'
         Lines = @(for ($i = 0; $i -lt $rcOrigLines.Count; $i++) { if ($i -eq $rcIdx[0]) { $rcOrigLines[$i] + $rcAltNumbered } else { $rcOrigLines[$i] } }) }
      @{ Id = 'altprose'; Code = 'ABSENT-CANON'; Label = '把**不带序号**的散文替代写进该项（"不含②"式弱断言拦不住的形态）'
         Lines = @(for ($i = 0; $i -lt $rcOrigLines.Count; $i++) { if ($i -eq $rcIdx[0]) { $rcOrigLines[$i] + $rcAltProse } else { $rcOrigLines[$i] } }) }
    )
