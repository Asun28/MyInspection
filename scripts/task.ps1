      ...
      if ($sagaMsg -match 'TD85-RESUME') {
        ...
        if ($Local) {
          ...
          if ($sagaDone -contains 'R3 评审') {
            ...
            Write-Host "    R3 已 pass、合并腿未完成：先 gh pr view $sagaPrNum 查 state 与 base——已 MERGED 则走 cleanup 在线补验（或确认后 -Phase cleanup -Force）；未合并且 head 与 base 均未变（headRefOid == R3 已审 sha 且 baseRefName == $shipBase）**且已按总则手动补跑全部确定性闸（DoD、verify、范围闸、许可闸、防泄露闸）**才可 gh pr merge $sagaPrNum --squash；head 或 base 任一有变即评审失效，按总则手动补跑全部确定性闸后 pwsh -NoProfile -File scripts\review.ps1 -WorktreePath `"$Wt`" -Base $shipBase -PostStatus -PrNumber $sagaPrNum 重跑至 pass 再合并。" -ForegroundColor Yellow
          } elseif ($sagaPrNum -gt 0) {
            Write-Host "    PR #$sagaPrNum 已开：修复须新改动时在 worktree 内 git commit 并 git push，**随后按总则手动补跑全部确定性闸（DoD、verify、范围闸、许可闸、防泄露闸——不可仅靠 CI 复跑）**，再对该 PR review.ps1 -PostStatus 重跑评审+合并（对照基线审**全量** diff，修复提交不免检；精确命令见 TD85-RESUME 段）。" -ForegroundColor Yellow
          } else {
            ...
            Write-Host "    commit 已落、PR 状态未知（push+PR 段未完成——PR 可能已建而号未解析到）：先 gh pr view $TaskId 实查、再看提交是否已推送（git status 对比 origin/$TaskId）——**未推送**（push 段即失败/未达）则在 worktree 内 $sagaRemoteReset，修复后重跑整条 ship，全部闸重过、无闸门旁路（R3 r6 #9）；**已推送/已有 PR** 则按总则**先手动补跑全部确定性闸（DoD、verify、范围闸、许可闸、防泄露闸——不可仅靠 CI 复跑）**后对 PR review.ps1 -PostStatus 重跑评审+合并（审全量 diff）。" -ForegroundColor Yellow
          }
        }
      }
      throw
