...
# android/{app,core}/build.gradle.kts、android/build.gradle.kts、android/gradle/libs.versions.toml（版本
# 目录，~20 个依赖坐标实际 pin 在这里），对整条 Android 依赖图零覆盖却报 PASS（L165 同型失效：断言面窄于契约）。
# 改递归 glob，排除 .gradle/ 与 build/ 等缓存产物目录（避免把生成物目录里偶然同名的文件当成源清单误报）。
# -ErrorAction Stop + try/catch（不用 SilentlyContinue）：目录树里任何一段不可读都会让 Get-ChildItem 整体
# 抛出，落入 catch 记一条 coverageGap——绝不让「扫不完」悄悄退化成「扫完了、什么都没有」（-Strict 下才吃真）。
$gradleHits = @()
try {
  $gradleHits = @(Get-ChildItem -Path $RepoRoot -Recurse -File -Include 'build.gradle', 'build.gradle.kts', 'libs.versions.toml' -ErrorAction Stop |
    Where-Object { $_.FullName -notmatch '[\\/](\.gradle|build)[\\/]' })
} catch {
  $coverageGap += "Gradle：递归扫描依赖清单目录树失败（$($_.Exception.Message)）——零覆盖≠合规，须人工核验（按 docs/LICENSE-POLICY.md）。"
}
if ($gradleHits) {
