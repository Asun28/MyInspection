...
.DESCRIPTION
  ...
  非 git 仓（如本元仓、或 _config GhAccount 留空尚未建仓）：打印 ASCII 哨兵 [SECRETS-SKIP-NONGIT] 并退出 0（优雅降级，
...
function Find-LineSecret([string]$line, [switch]$PublicOutput) {
  # -PublicOutput (T169, upstream issue #259): drop the SOURCE-FIXTURE exemptions.
  # Both exemptions below are correct when scanning THIS repository's own tracked files, which
  # deliberately carry fixture secrets and placeholder templates. Both are unsafe for arbitrary text
  # about to be published as a PUBLIC issue:
  #   - `allowlist secret` is an escape hatch the REPORTER controls, so in public-output mode it lets the
  #     author of the text switch the scanner off for that line;
  #   - the placeholder skip only fires for Generic Secret Assignment, but a REAL value followed anywhere
  #     on the line by a word like `example` is then exempted - the exact false negative reported.
  # The normal source-scanning path is unchanged: with no switch, both exemptions apply as before.
  if (-not $PublicOutput -and $line -match '(?i)allowlist secret') { return $null }      # 逐行逃生舱
