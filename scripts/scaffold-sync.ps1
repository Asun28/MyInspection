...
'@
  ...
  $realV45Valid = $realV45.Count -eq 1 -and $realV45[0] -match 'db835867e6f1bab740f13b48e4bae009a34521ef' -and $realV45[0] -match 'seeded shard'
  $realV46Valid = $realV46.Count -eq 1 -and $realV46[0] -match 'd0c9145970e69626318a26ce922650f1a631c2f0' -and $realV46[0] -match 'origin/current coupling group'
  if ((Get-SyncedVersion $realLedger '0.29.0') -ne '0.46.0' -or $resolvedConfig.Version -cne '0.46.0' -or $resolvedOriginVersion -cne '0.29.0' -or -not $realV45Valid -or -not $realV46Valid) { $fails.Add('real v0.45 history, v0.46 applied row, or origin/current version split is absent') }
  if ((Get-SyncedVersion '' '0.29.0' @('v0.29.0','v0.45.0','v0.46.0')) -ne '0.29.0') { $fails.Add('missing ledger did not fail closed to ScaffoldOriginVersion') }
