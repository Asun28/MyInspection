...
- 2026-08-16 R3 第 1 轮（PR #2 @89bb1c2）：**block ×2，均属实、均在本卡 diff 内，当场修（L113）**。#15：`java.util.HexFormat` 需 Android API 34 而 app minSdk 26 且未配 desugaring——:core 按纯 JVM 测试但运行载体是 ART，JDK API 可用性按 Android API level 判（此 API 系步骤 4.6 建议清理时引入，advisory 评审在 ART 可用性上判错、Sol 抓回）；已还原为 API-26 安全的手写 hex 循环并注释缘由。#6：键序测试纯 ASCII 键区分不了 UTF-16 码元序与码点序；已加 U+10000（代理对，首码元 D800）vs U+E000 判别键对——码元序下代理对在前，码点序恰反。修复 b2e0f6f，DoD 20/20 绿。变异 M13（换成码点比较器）只红新测试、ASCII 序测试与全部向量保持绿=判别性证明，**累计 14/14**。同类扫全：#15 类逐一核过本 diff 其余 JDK API（Normalizer/TreeMap/MessageDigest/isHigh·LowSurrogate 均 API≤9）无第二例；#6 类核过其余钉死契约（NFC/转义/整数拼写/数组序/null 策略）均已有判别输入。
