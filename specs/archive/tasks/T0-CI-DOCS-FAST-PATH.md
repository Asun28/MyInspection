...
forbid:
  ...
  - 放松含代码的 push 或 workflow_dispatch 的完整 CI；纯文档 push 继续沿用既有 paths-ignore
non_goals:
...
dod_exit: 0
dod_assert: docs/**、specs/** 与任意 Markdown 的纯集合分类为 docs-only；源码/脚本/workflow/混合/空集合均 full；PR docs-only 仍运行卡片、archive 与密钥闸并产出 verify，跳过 Python/Java/Android/Gradle/license/E2E；纯文档 push 继续不触发，含代码 push 与手动触发全跑
...
- 仅 PR 需要产出轻量 required `verify`；默认分支纯文档 push 继续使用既有 `paths-ignore` 不触发，含代码 push 与手动触发始终完整执行。
