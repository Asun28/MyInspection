...
- **模板 JSON 形态**（一类型一文件，如 `routine-v1.json`）：
  ...
  - ~~房间定义带 `repeatable` 标记（如 BEDROOM）~~ → **本条已拆出，不在本卡**（见下「评审仲裁」段与 `T2-ROOM-REPEATABLE`）。仍成立的部分：建巡检时按物业实例化为 room_instance（Bedroom 1..N）——**房间是实例不是模板常量**（调研要点 6，schema 已有 room_instance 表）；非 repeatable 房间恒一实例。该实例化逻辑属 `T2-CAPTURE-CORE`。
