...
The 2026-09 local/origin reconcile reverted this card's local implementation (`c60a67ba`). Origin changed `scripts/_cards.ps1`, `scripts/check-cards.ps1`, `scripts/selftest.ps1`, `.claude/workflows/decompose-cards.mjs`, `specs/tasks/_TEMPLATE.md`, `specs/README.md` and `docs/DEVOPS-WORKFLOW.md` in parallel, and the reconcile took origin's whole file for each. That removed this card's rules together with its DoD fixture (the `selftest.ps1 -Fixture card-acceptance` entry and the gate 10h one-item and R-id guards). `docs/PLAN-TEMPLATE.md` was changed only locally; the reconcile reset it to the base text on purpose so that it does not describe a one-item rule origin does not enforce. Origin adopted upstream's template but keeps enforcing three or more acceptance items and defers requirement-citation validation to a corpus-migration card (`docs/SCAFFOLD-UPSTREAM-ADOPTION-20260910.md`). One-item acceptance lists, requirement R-id validation and the slimmer card generator are therefore not on master; TD182 tracks them. No other card supersedes this one.
