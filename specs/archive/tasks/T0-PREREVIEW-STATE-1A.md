...
Routed full selftest ran from the card worktree: seeded, workflow and core all passed (aggregate exit 0; ordinary mode, nightly meta fixtures deferred). Cleanup removed the worktree and branch using the matching merge receipt. Card/board/archive checks passed; R5 found no additional debt introduced by this card. L332 records the dot-source entry-mode overwrite and its prevention.

## Reconcile note (2026-09-24)

Origin's todo publication cards `T0-PREREVIEW-RECORDS` and `T0-PREREVIEW-STATE-1A` (registered by #303) share their IDs with local master's delivered cards; the reconcile replaced them with the local archived cards, so their remote-publication criteria were not run separately. Their disposition, like that of the other remote adoption cards, is a user decision. The three local prereview libraries also gained the gate 1h self-check entry in the reconcile, so their bytes on master differ from the pinned local files: `scripts/_prereview-facts.ps1` is SHA-256 `FD50745DA618EF4A...` (the `23A38171...` pin and its 18/18 receipt describe the pre-reconcile file), `scripts/_prereview-state.ps1` is `DE6D668E2E599C2E...` (the archived STATE-1A pin `88E64050...` describes the pre-reconcile file), and `scripts/_prereview-records.ps1` is `9CCB7C940AB22EC8...`.
