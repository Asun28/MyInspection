...
| Compose 接线 | REQ-036 的绘制、REQ-048 的 banner 视觉形态、行的点击目标 | **manual** · `ScheduleScreen.kt` 在 A4 的测试面之外：没有任何测试执行它，故「它确实画出了投影返回的东西」只有编译证据与人工设计评审。`A2 the row-by-row projection rebuilds exactly the content the state declares` 钉住的是**两条模型层路径彼此相等**，不是渲染 | 编译证据 / 人工评审 |
...
| 2026-09-08 | **R3 第 3、4、5 轮各一条 finding，皆属实并已修**：① `ScheduleScreen` 与测试节标题残留 token 归属的过期注释；② `countPhrase(0)` 渲染成 `No inspections due`，而内容值只有 `text` 会到达屏幕，数字 `0` 就此丢失（改为 `0 inspections due`，M23 重新瞄准）；③ 注释宣称模型与 Compose 渲染器「不会漂移」，但那条等式只钉住**两条模型层路径**，测试从不执行 `ScheduleScreen`。**三轮都是同一类错误——写下的保证超出证据**，故第三次不只修被点名那处，而是把本卡新增的每条注释逐一对着「它到底证明了什么」重审，并在验收表新增一行把 Compose 接线**显式记为人工评审证据**。 |
