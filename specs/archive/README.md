...
- 那份 id 清单可由 `lessons.ps1 archive -DryRun` 机械预筛得出（**选择规则的权威表述见 `docs/LESSONS.md` §3 PURIFY**，
  ...
  预览本身也是透传给它跑的，故这些拒绝在预览里同样出现、退出码一致；但经 `lessons.ps1 archive` 这个入口
  **实际可达**的只有「非规范别名 id」与「两侧并存内容不一致」两类——最高 id 已被选择器先行排除，候选又恒取自
  热账本故不会有未知 id。另：常驻 `CLAUDE.md` 缺席时，选择器直接以
...
- 锁：`scripts/selftest.ps1` 子闸 **12e** 以 hermetic 夹具验证热/冷分区、索引条数一致、幂等（含 lessons 子夹具：
  ...
  选择器一侧另有 **2g**（候选集逐字相等 + 四类排除面各一条夹具，含常驻引用的方括号/裸写/范围端点三形态，
  外加「路径行号 `x.ps1:L9` / ASCII 行段 `L10-13`」两种**不算引用**的否定形态，以及常驻文件全缺席时 fail-closed）、
  **2i**（元数据只认规范 meta 行，十五条敌意夹具读写两侧均留热、零写入、非零退出，四枚必填字段各自可杀）、
