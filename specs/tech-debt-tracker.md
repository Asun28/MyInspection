...
| TD182 | 2026-09-24 | 任务卡合同（`scripts/_cards.ps1`、`scripts/check-cards.ps1`、`scripts/selftest.ps1`（本地的 `-Fixture card-acceptance` 与本地闸 10h；master 上验收闸是 10k）、`.claude/workflows/decompose-cards.mjs`、`specs/README.md`） | 2026-09 local/origin reconcile 按 origin 整文件规则回退了本地 `T0-CARD-TEMPLATE-SLIM`（`c60a67ba`）：一条起的 acceptance、requirements 的 R-id 形态/重复/悬空引用校验与精简卡生成器不在 master；origin 仍要求 acceptance 至少 3 条，requirement 引用校验等待语料迁移卡（`docs/SCAFFOLD-UPSTREAM-ADOPTION-20260910.md`）。需决定是否在 origin 合同上重做。另：origin 模板写「无对应 live item 的引用 BLOCKS」，而 check-cards 未启用 requirement 引用校验（origin 既有不一致） | major | open | — |
| TD185 | 2026-09-24 | `specs/tasks/T4-DESIGN-SYMBOL-CHROME-V2.md` 的 origin 合同（`scripts/_symbol-markdown.ps1` 40 项） | 2026-09 local/origin reconcile 把本卡记为 merged；其 DoD 的 check 07 钉住的措辞已由 `T4-DESIGN-STATUS-CARRIERS`（`856268c3`）改写，自 `174c7ce6` 起在 master 上失败。需把 check 07 改钉到继任措辞，或记录它被有意退役 | minor | open | — |
| TD187 | 2026-09-24 | PR review v2 1a 兄弟卡验收行（本地 TD176 未提交行中仍开放的两项） | FACTPACK-SLICES A1 把截断清单放进封闭的 `$defs/facts`（`additionalProperties:false`，无此字段）；FACTPACK A3「只含已跟踪内容」与 FACTS-LIB A3 把未跟踪未忽略文件纳入快照相矛盾 | minor | open | 两卡开工前在 master 修正验收行（L18） |

| TD173 | 2026-09-06 | `scripts/selftest.ps1 -TaskId` 的既有 core/all 分流 | **SELFTEST-TASK-SOURCE**：从另一检出运行入口时，路由解析返回目标 WorktreePath，但既有 core/all 仍验证脚本所在检出，可能未覆盖目标卡改动。T0-SELFTEST-SKILL-ROUTING 已使新 skills 路径使用目标快照；本卡 A2 保留旧 core/all 行为，故单独登记。修法：将两种旧模式绑定解析后的目标工作区，并用脚本检出与目标检出内容不同的真实入口夹具证明正负例、dirty/untracked 可见性及失败退出；不得改变现有保守分类或公开分片枚举。 | major | open | 2026-09 local/origin reconcile：本地 -TaskId 路由实现已由 origin v0.47 实现取代（T0-SCAFFOLD-UPSTREAM-ADOPTION），需对照 origin 实现复核后再定关闭方式 |
...
| TD172 | 2026-09-03 | delivery 侧渲染器仍把身份两半各自校验（`T4-SCHEDULE-REMINDER-DIAGNOSTICS`，PR #233） | `ReminderDiagnostics.reminderLogMessage` 对 `occurrence_id` 与 `generation_number` **各自** `takeIf`：occurrence 不是摘要形状时仍发布 `generation_number`，反之亦然（只有 `work_request_id` 是原子的）。这正是本卡 A2 在注册侧修掉的那条缺陷——「半个身份与任何东西都关联不上」——两条日志线现在对同一件事给出不同答案。后果：delivery 侧一条坏记录仍会发布一个关联不上任何东西的半身份，读日志的人无从判断它属于哪次 occurrence。当前无已知产生该记录的路径（`LogRecord` 由 worker 从校验过的输入构造），故列 minor 而非缺陷。本卡刻意未修：`ReminderDiagnostics.kt` 在 allow_paths 之外，且它是 delivery 卡的冻结词汇面。修法：把两半的判定收成一个值，同注册侧；可测=给 `reminderLogMessage` 补「occurrence 坏 ⇒ generation 也为 null」与其反向的两条负例，改前必红 / 偿还卡：随 `T4-SCHEDULE-UI` 或下一张动 delivery 诊断面的卡 | minor | open | — |
| TD175 | 2026-09-08 | 一个配对同时服务两种职责，而 CI 对比度元数据规定每配对恰好一条条目（`T4-DESIGN-NOT-APPLICABLE-COLOR`，master `d6b369ee`） | `light.outline`/`dark.outline` on `surface-container` 的条目标 `"usage":"evidence-boundary"`，而同底另三个段色（`primary`/`tertiary`/`error`）标 `"usage":"evidence-segment"`。本卡把 `OPTIONAL` 与 `NOT_APPLICABLE` 明确定为用 `outline` 做**段色**之后，该配对同时承担边界与段色两种职责，但 `context/DESIGN.md` 的「Every rendered foreground/background pair … has one metadata entry」只许一条条目，一个 `usage` 标签只能命名一种——改标签会让另一种职责失准。当前无行为缺陷（闸只要求「该配对有条目」，不比对 usage 语义），风险是读元数据的人据 usage 判断职责时会漏掉段色这一半。刻意未在本卡修：真正的收口是让一条条目携多个 usage（或让 usage 变集合），属 metadata schema 决定，超出本卡「只补取色、不动已登记配对」的范围。修法：给条目加 `usages` 数组，或允许同配对多条并按 usage 去重；可测=`outline` on `surface-container` 同时被 evidence-boundary 与 evidence-segment 两种职责检索到，且既有「每配对恰好一条」的重复检测不因此失效 / 偿还卡：下一张动 CI 对比度元数据 schema 的设计卡 | minor | open | — |

Local TD176 (a local tech-debt row that was never committed to local master and exists only as an uncommitted edit in the main checkout; it is not the TD176 row above) partial resolution (2026-09-16): FACTS-LIB A3 now distinguishes untracked ignored paths from tracked paths (`704fbd69`); A5 now uses a positive hunk ordinal after the body hash (`1220f108`, schema revision merge `71895645`). FACTS-LIB implementation merged at `b675d6a6`. Of its other items, CHECKLISTS A3 was closed by `4134c73d` (2026-09-15), RECORDS A4 by the RECORDS merge `dec30514` (its A4 records the line-free duplicate key decision) and STATE-1A A4 by the STATE-1A merge `62ec5f3b` (A4 revised before execution); the original 2026-09-16 "remain open" wording was already stale for the first two. The two still open are registered as TD187; see the two archived prerequisite/FACTS-LIB cards for delivery evidence.

| id | 发现日 | 位置 | 偏离了什么（债） | 严重度 | 状态 | 偿还指针 |
|---|---|---|---|---|---|---|
| TD183 | 2026-09-16 | FACTS-LIB R5: deleted-file candidate anchors | FACTS-LIB A5 emits deleted-file units using MergeBase blobs, but candidate.file and anchor prose in specs/prereview-record.schema.json and docs/PREREVIEW-PROTOCOL.md only describe snapshot-tree paths. RECORDS checks unit membership, not this anchoring meaning. Before worker/prompt integration, use a separate frozen-contract version review to define deleted-path file/anchor/line/evidence_refs semantics and add a deleted-file candidate fixture. The current library correctly follows its approved A5 contract. | minor | open | Follow-up before WORKERS/PROMPT integration; no schema or protocol change in FACTS-LIB.；2026-09 local/origin reconcile 由本地 TD177 改号（origin 已用 TD177） |
| TD178 | 2026-09-24 | `AppStoragePolicyTest` 的非夹具用例不清理临时目录（`T1-APP-STORAGE-POLICY` R3 第 3 轮附注，master `f68d7006`） | 前 9 个用例用 `createTempDirectory` 建目录且从不删除，每次运行在 `%TEMP%` 留约 10 个空目录（评审时已累积约 4,000 个）；另有冗余断言（`assertIs` 检查编译期已保证的类型、`rootLocation` 重复传入 `assertNoSensitiveText`）与一处 `assertEquals(null, …)` 风格不一。无行为缺陷。修法：改用 `withStoragePaths` 或自删临时目录，删去冗余断言 / 可测：运行前后 `%TEMP%` 下这些前缀的目录数不变，DoD 与既有 45 枚变异仍全杀 / 偿还卡：下一张 allow_paths 含该测试文件的卡 | minor | open | — |
| TD188 | 2026-09-24 | `scripts/_config.ps1` 的 `PrereviewRiskyEffort` 没有读取方（`T0-OPUS55-PROMPT-FIT`，origin `ecdeb4b8`） | 后果：风险路由（Opus 5.5）该跑 `medium`，但 `scripts/_prereview-facts.ps1` 的模型路由只返回模型（`PrereviewRiskyModel` / `PrereviewModel`），没有任何代码读档位；内置 claude 适配器接线时若只取 `PrereviewEffort`，Opus 5.5 会静默跑 `high`，与配置注释、`docs/PREREVIEW-PROTOCOL.md` 里 `PRE_EFFORT` =「本 worker 的路由档位」不一致。修法：路由函数与模型一起返回档位（风险路由取 `PrereviewRiskyEffort`，标准路由取 `PrereviewEffort`），把它写进 `PRE_EFFORT` 与 facts 的 `discoverer.effort`。可测：风险路径夹具断言 `PRE_EFFORT=medium`、标准路径断言 `high`；删去风险分支那一行的变异须变红。前置：`T0-PREREVIEW-WORKERS-CLAUDE`（或其 REMOTE 对应卡）开工时并入其 allow_paths | minor | open | — |
| TD189 | 2026-09-25 | `scripts/post-merge.ps1` 的看板行匹配（`T0-POST-MERGE-R5-BOARD-TABLE` 实现记录的 follow-up，master `5ea3ea2d`） | 后果：`docs/TASK-BOARD.md` 主卡片表里有 34 格卡 id 带装饰（`★` 后缀 10 格、删除线 5 格、markdown 链接 19 格，含第二张主表 382-399 行全部），`Test-PostMergeBoardRow` 要求第二格逐字等于卡 id，所以 `post-merge.ps1 r5` 对这些卡停在 `[POST-MERGE-ANCHOR]`；其中 11 张是在飞的 `T0-PREREVIEW-*` 卡，合并后只能手工做 R5。修法：二选一，(a) 把这些格改成纯卡 id、装饰移到别的列；(b) 行匹配接受固定几种装饰，并同步白名单判定（二者共用 `Test-PostMergeBoardRow`，改它须重做 `T0-POST-MERGE-DOCS-PR` A2 的变异）。可测：对 origin/master 的看板，主表每个含卡 id 的格都能被 r5 的行搜索找到恰好一行。前置：无 | minor | open | — |
| TD190 | 2026-09-25 | `CLAUDE.md`「命令」节工作流自检那一行（`T0-POST-MERGE-R5-BOARD-TABLE` 执行时发现，master `8146d794`） | 后果：该行写显式 `-TaskId <id> -Base origin/master`，而 `scripts/selftest.ps1` 的参数只有 `-StrictLint`、`-Only`、`-Parallel`、`-TaskId`、`-IncludeMeta`，照抄即报 `A parameter cannot be found that matches parameter name 'Base'` 并退出 1；基线其实由脚本自动取（`[SELFTEST-TASKID-BASE]`）。修法：删去该行的 `-Base origin/master`，或给 selftest.ps1 加这个参数。可测：CLAUDE.md 该行的命令原样运行不报参数错误。前置：无 | minor | open | — |
