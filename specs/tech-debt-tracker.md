...
Local TD176 (a provenance ID from local master's PR review v2 plan, not the TD176 row above) partial resolution (2026-09-16): FACTS-LIB A3 now distinguishes untracked ignored paths from tracked paths (`704fbd69`); A5 now uses a positive hunk ordinal after the body hash (`1220f108`, schema revision merge `71895645`). FACTS-LIB implementation merged at `b675d6a6`. The other sibling-card items remain open; see the two archived prerequisite/FACTS-LIB cards for delivery evidence.
