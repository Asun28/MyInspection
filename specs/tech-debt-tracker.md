...
| TD11 | 2026-08-16 | `scripts/selftest.ps1` 闸 17ee（`$rcCanonHash`）+ `docs/RELEASE-CHECKLIST.md` 的 Gradle 阻断项 | **发布清单那一项被整行 SHA-256 钉死**（T0-GATE-FIXFORWARD 的人裁产物）：「不含第二条解锁路径」这个契约无法用模式穷举散文替代（评审实证：不带序号的「或完成人工核验后即可勾选」绕过任何字形断言），故改钉规范文本哈希。代价是**该项任何措辞改动都会让 selftest 变红**，包括纯润色/错别字修正，且修法要求改动者同时更新闸里的期望哈希——这是刻意的（契约变更须显式），但对不知情者是一次「看不懂为什么红」的绊脚石。后果：文档小改可能被误判为闸门坏了 / 修法：`T0-LICENSE-SCANNER` 落地后本项的解锁语义会变（扫描器跑通即可勾），届时重估——要么把哈希换成「结构化标记 + 机器可读 unlock 列表」，要么在该项上方加一行醒目注释说明「改它要同步改 selftest 17ee 的 `$rcCanonHash`」/ 可测：改该行任一字符 → `selftest -Shard seeded` 必红且报 `ABSENT-CANON` / 前置：无 | minor | open | 随 `T0-LICENSE-SCANNER` 重估 |
...
| TD9 | 2026-08-16 | `scripts/selftest.ps1`（8.2e stub 夹具 + 分片汇总器） | **selftest 可诊断性 + CI 稳定性双债**（W2 编排会话实证）：①PR #6/#9 的 `selftest (ubuntu-latest, core)` FAIL 但全日志无一行点名失败闸——汇总器只打 `selftest: FAIL`；②与通过运行的日志 diff 显示约 45 项检查（8.2e 全量证明、闸 9 前半、10b/10c/10f 族）静默未执行、也无 skip 行；③同一 merge commit 重跑即全绿（A/B 实证，run 31941736470）——8.2e 并发 stub 夹具在 CI 高负载下退化 WARNING 并计入失败，属 load-flake。修法方向：FAIL 必点名闸 id；被跳过检查必打印 skip 行；8.2e 并发证明加负载容差或独立慢分片。三张 T0 selftest 卡串行约束同样适用（见 T0-GATE-FIXFORWARD 备注） | major | open | — |
