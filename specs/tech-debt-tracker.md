...
| TD14 | 2026-08-17 | `core/media/`(落盘↔入库) + `app/media/`(清理调度) | **跨 FS+DB 的真原子性未做**：落盘在前、入库在后，入库失败靠补偿撤销刚写的字节。两条具体丢数据路径已在 T2-PHOTO-PIPELINE 内各自封死（补偿绝不删仍被活跃行引用的路径——同 photoId 重试时赢家那行正引用它，判据为已冻结的 `selectActiveAssetsByContentHash`；复用路径本次不写字节故永不补偿），但**共享临界区**这个架构级解法未做。后果：极端交错下仍可能出现「字节在盘、行不在库」的孤儿；现有 `OrphanedAssetCleanup` 只看软删行，无法发现无 DB 行的补偿失败 JPEG。修法已拆为 sidecar lease：在 publish 前 durable 创建并独占同目录 `.jpg.pending`，record 成功或已补偿拒绝后才移除；周期 worker 在同一 lease 下以既有 `selectById(photoId)` + 精确路径复核活跃引用，非活跃才删 JPEG，并继续跑 `OrphanedAssetCleanup` 清软删行。这样 crash/补偿窗口可回收且不与仍在 record 的操作竞争；不动冻结 schema。历史或损坏的 cross-ID `rel_path` alias 不按不完备查询冒险删除，须等 TD4 后用全局 rel_path 活跃查询另开 follow-up。**注意 TD10 仲裁：不得再以多连接场景 block 单连接卡** / 可测：marker 前/后 publish、record 提交、补偿失败、lease 竞争、活跃重采用、形状拒绝及 driver close 的定向用例；一律不跑 full selftest / 前置：T2-PHOTO-PIPELINE 已合；T1-SPIKE-PLATFORM 仅 app 资源串行，非业务前置 | major | carded | `specs/tasks/T2-PHOTO-ORPHAN-CLEANUP-SCHEDULER.md` |
