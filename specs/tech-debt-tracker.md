...
| id | 发现日 | 位置 | 偏离了什么（债） | 严重度 | 状态 | 偿还指针 |
|---|---|---|---|---|---|---|
| TD177 | 2026-09-16 | FACTS-LIB R5: deleted-file candidate anchors | FACTS-LIB A5 emits deleted-file units using MergeBase blobs, but candidate.file and anchor prose in specs/prereview-record.schema.json and docs/PREREVIEW-PROTOCOL.md only describe snapshot-tree paths. RECORDS checks unit membership, not this anchoring meaning. Before worker/prompt integration, use a separate frozen-contract version review to define deleted-path file/anchor/line/evidence_refs semantics and add a deleted-file candidate fixture. The current library correctly follows its approved A5 contract. | minor | open | Follow-up before WORKERS/PROMPT integration; no schema or protocol change in FACTS-LIB. |
