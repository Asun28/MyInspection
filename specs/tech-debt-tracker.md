...
| TD146 | 2026-08-23 | `scripts/selftest.ps1` 闸号命名 | 本卡新增闸 `2d`，而并行的 `T0-LESSONS-COLD-RECALL`(PR #51) 也在引入 `2d`/`2e`——两处独立挑了同一个「下一个空号」。合并时点上 master 尚无 2d/2e，故本卡先落地；#51 合入后会出现两个不同的闸共用一个编号 / 修法：#51 落地后把本卡这枚改号为其后的下一个空号（DoD 锚的是夹具名 `LSN-PLANE-WORKTREE`、不锚闸号，改号不动 DoD），并同步 4 处 `闸2d(a\|b\|c\|d\|e)` 失败文案与成功行 / 可测：selftest 内闸号唯一（可加一条闸号唯一性断言，根治「各挑下一个空号」这一类冲突） / 前置：PR #51 合并 | minor | open | — |
