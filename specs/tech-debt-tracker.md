...
| TD12 | 2026-08-16 | `core/backup/format/BackupManifest.kt`(BackupScope，冻结物) ↔ `T5-BACKUP-IO` | **format v1 的 Property scope 仍带整库 `db.sqlite`，且 manifest 不记每文件属主**。后果：任何标为“按物业”的 v1 包都会连带其它 tenancy/物业 DB 行，不能作为物业级保密出口。ADR-0006 已裁定 v1 产品只允许 Full：UI 隐藏 Property，writer 对 Property fail closed，reader 仅为诊断/拒绝保留冻结枚举；这先消除实际泄露路径但不改冻结字节。真正物业级导出须 format v2 + 过滤逻辑/SQLite 快照 + 独立恢复语义与版本评审 / 可测：v1 UI/写侧无法创建 Property；旧 Property 包不得被当 Full 恢复；未来 v2 包不含其它物业任一行/资产 / 前置：T5-BACKUP-IO | major | carded | `T5-BACKUP-IO`（关闭 v1 创建路径）+ `docs/adr/0006-offline-security-backup-hardening.md`；format v2 在产品重新提出物业级导出时另卡 |
...
| TD24 | 2026-08-17 | `Photo.sq` `selectActiveAssetsByContentHash` → `PhotoIngest.plan()` ↔ 物业级媒体保留/回填 | **照片按内容哈希全局复用，允许不同物业共享同一 `rel_path`**：查询不按物业过滤，`PhotoIngest` 也只校验路径形状；现有 recorder 测试已允许 B 物业复用 A 物业路径。后果：按物业清理本机字节时，A 的归档动作可能删掉 B 仍为 PRESENT 的证据；未来 format v2 物业导出也无法表达单一 owner。ADR-0006 已取消 v1 物业包，但不消除媒体生命周期风险 / 修法：专属媒体债卡把物理去重限制为同一物业；跨物业同 hash 走现有 `WriteNewAsset`，保留同物业复用；历史共享路径只读检测，不改 finalized 证据 / 可测：A 路径作为 B 候选必须写 B 新路径；同物业仍复用；清理 A 不影响 B；删除物业过滤时测试翻红 / 前置：已合并 `T2-PHOTO-PIPELINE`；须在媒体归档/清理前独立偿还，不得与 TD12/TD14/TD15 合卡 | major | carded | `specs/tasks/T2-PHOTO-PROPERTY-DEDUPE.md` |
...
| TD135 | 2026-08-20 | SQLDelight `selectById`/baseline setter ↔ capture/retention 写入口 | **active 与 historical 生命周期语义未在查询/API 名称上分开，字段权限依赖调用方自律**。后果：`createInspection`/物业抑制可接受 soft-deleted parent；通用 baseline setter 可绕过 same-property/tenancy/type/finalized 规则；未来 setter 可能在 `purged_at` 后回填已清理 PII。修法：active/any 点查分流、两种具名 baseline 操作、purge 终态 CHECK、deleted override guard；保留“软删 tenancy 仍可做隐私清理”的 any-lifecycle 例外 / 可测：逐条删 active predicate/终态/关系守卫均命中专属红灯，且历史报告与 deleted tenancy purge 正例保持绿 / 前置：TD4；与其它 schema 卡串行版本评审 | major | carded | `specs/tasks/T1-DATABASE-LIFECYCLE-AUTHORITY.md` |
| TD136 | 2026-08-20 | 运行时 structured logs ↔ 本机支持诊断 | **现有日志只有脱敏规则，没有持久、有界、可由用户授权导出的诊断证据**。后果：admin/support 无法重建 backup/restore/finalize 等失败现场；若直接扩普通日志又容易把 PII/路径/URI 带出或让 logger 故障影响主事务。修法：独立 no-backup diagnostics DB + typed event recorder + 90 天/20,000 行裁剪，再做用户确认的只读健康摘要/脱敏导出；不进主库、`.mibk`、网络或证据哈希 / 可测：敏感字段/CRLF/未知 key/超限全部拒绝，满盘/损坏 logger 不改变业务结果，飞行模式导出包零禁记字段且 manifest/hash 可复验 / 前置：event store 先等 TD4 + T1-LOCAL-DATA-SECURITY，导出再等分享隐私卡 | major | carded | `T5-OPERATION-EVENT-STORE` → `T5-DIAGNOSTIC-EXPORT`；两卡 merged + 总验收后才可 paid |
