...
| TD9 | 2026-08-16 | `scripts/selftest.ps1`（8.2e stub 夹具 + 分片汇总器） | **selftest 可诊断性 + CI 稳定性双债**（W2 编排会话实证）：①PR #6/#9 的 `selftest (ubuntu-latest, core)` FAIL 但全日志无一行点名失败闸——汇总器只打 `selftest: FAIL`；②与通过运行的日志 diff 显示约 45 项检查（8.2e 全量证明、闸 9 前半、10b/10c/10f 族）静默未执行、也无 skip 行；③同一 merge commit 重跑即全绿（A/B 实证，run 31941736470）——8.2e 并发 stub 夹具在 CI 高负载下退化 WARNING 并计入失败，属 load-flake。阶段性修法：先由 `T0-DEBT-SELFTEST-CRITICAL-PATH` 移除 PR 关键路径耦合；剩余 FAIL 点名、skip 可见性与 post-merge load-flake 继续单独偿还。三张 T0 selftest 卡串行约束同样适用（见 T0-GATE-FIXFORWARD 备注） | major | carded | `specs/tasks/T0-DEBT-SELFTEST-CRITICAL-PATH.md` |
