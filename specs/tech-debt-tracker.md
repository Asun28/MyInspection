...
| TD174 | 2026-09-07 | `core/report/importing/docx/package/` 自定义文档属性边界 | 原始来源包含 `docProps/custom.xml`，现有 reader 在 extractor 前返回 `UNSUPPORTED_PART`。合成 extractor DoD 通过不代表该原包可导入 / 修法：独立评审有界读取后丢弃自定义 metadata 的兼容方案，保持 ZIP/XML 上限、外链禁令和作者信息排除，不宽泛放开 allowlist / 可测：公开合成包含 custom properties 时按契约通过且 metadata 不进入 manifest；恶意关系、超限与结构错误仍拒绝，私样仅作本地只读复核 / 前置：`T3-DOCX-PACKAGE-READER`；本卡仅登记，无 reader 改动 | major | carded | T3-DOCX-CUSTOM-PROPERTIES |
