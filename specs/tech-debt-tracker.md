...
| TD148 | 2026-08-21 | `docs/lessons/LEDGER.md` L177 ↔ `CLAUDE.md` L17 PowerShell 铁律 | **blocking mutation 批次假全绿经验未进入默认上下文**：PowerShell 变量名大小写不敏感，`foreach ($m in $M)` 会在第一轮覆盖集合，令 16 枚计划只跑 1 枚；若汇总只看每条 OK 而不核记录数，假证据仍被宣称完整。L177 已记录真实事故但仍是 ledger，triage 持续报警 / 修法：把 L177 晋升 must，与近义 L17 合并为同一 CLAUDE bullet，加入“循环变量不与集合仅大小写不同 + 实跑记录数等于计划数”；不提高 cap / 可测：lessons check 证明 ledger must=11 但常驻条目仍=10；triage 不再报告 L177 / 前置：TD147 | major | carded | `specs/tasks/T0-DEBT-MUTATION-BATCH-COUNT.md` |
