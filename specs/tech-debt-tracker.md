...
| TD146 | 2026-08-21 | `specs/archive/cards-index.md` ↔ `specs/archive/tasks/` | **归档卡索引静默漂移**：归档目录已有 39 张卡，自动生成索引仍声明并列出 36 张。`archive.ps1` 每次真运行会重算正确投影，但 R5 可手工移动卡；现有 selftest 只在隔离夹具里证明生成器，没有普通 PR 闸验证仓库当前产物。后果：按精简索引检索历史卡会漏项，且“勿手工编辑/由脚本生成”的承诺无机检 / 修法：给 archive.ps1 增加复用同一生成函数的只读 exact-check 模式并接入 verify；漂移非零且不自动写盘 / 可测：缺行、错行、头注计数漂移均失败，正常 archive 重建后通过；删除 verify 接线或 exact 比较时翻红 / 前置：无 | minor | carded | `specs/tasks/T0-DEBT-ARCHIVE-CARDS-INDEX-GATE.md` |
