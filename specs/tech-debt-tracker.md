...
| TD11 | 2026-08-16 | `scripts/selftest.ps1` 闸 17ee（`$rcCanonHash`）+ `docs/RELEASE-CHECKLIST.md` 的 Gradle 阻断项 | **发布清单那一项被整行 SHA-256 钉死**（T0-GATE-FIXFORWARD 的人裁产物）：「不含第二条解锁路径」这个契约无法用模式穷举散文替代（评审实证：不带序号的「或完成人工核验后即可勾选」绕过任何字形断言），故改钉规范文本哈希。代价是**该项任何措辞改动都会让 selftest 变红**，包括纯润色/错别字修正，且修法要求改动者同时更新闸里的期望哈希——这是刻意的（契约变更须显式），但对不知情者是一次「看不懂为什么红」的绊脚石。后果：文档小改可能被误判为闸门坏了 / 修法：`T0-LICENSE-SCANNER` 落地后本项的解锁语义会变（扫描器跑通即可勾），届时重估——要么把哈希换成「结构化标记 + 机器可读 unlock 列表」，要么在该项上方加一行醒目注释说明「改它要同步改 selftest 17ee 的 `$rcCanonHash`」/ 可测：改该行任一字符 → `selftest -Shard seeded` 必红且报 `ABSENT-CANON` / 前置：无 | minor | carded | `specs/tasks/T0-DEBT-RELEASE-CHECKLIST-HASH-GUARD.md` |
