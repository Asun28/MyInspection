...
| TD150 | 2026-08-21 | `docs/lessons/LEDGER.md` L172 ↔ `CLAUDE.md` L17/L177 PowerShell 铁律 | **blocking detached pwsh 编码假红纪律未进入默认上下文**：harness 外启动的 pwsh 默认 OEM 编码曾让中文断言 mojibake，整晚 mutation 批次全部假红；L172 已记录事故但仍是 ledger / 修法：把 L172 晋升 must，与近义 L17/L177 合并为同一 CLAUDE bullet，加入“harness 外先 dot-source scripts/_encoding.ps1、机检优先 ASCII 哨兵”；不提高 cap、不改编码脚本 / 可测：lessons check 证明 ledger must=13 但常驻条目仍=10；triage 不再报告 L172 / 前置：TD148 | major | carded | `specs/tasks/T0-DEBT-POWERSHELL-DETACHED-ENCODING.md` |
