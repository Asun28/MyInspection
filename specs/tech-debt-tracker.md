...
| TD9 | 2026-08-16 | `scripts/selftest.ps1`（8.2e stub 夹具 + 分片汇总器） | **selftest 可诊断性 + post-merge 稳定性残债**（W2 编排会话实证）：①PR #6/#9 的 `selftest (ubuntu-latest, core)` FAIL 但全日志无一行点名失败闸——汇总器只打 `selftest: FAIL`；②与通过运行的日志 diff 显示约 45 项检查静默未执行、也无 skip 行；③同一 merge commit 重跑即全绿（run 31941736470），8.2e 并发 stub 在 CI 高负载下会 flake。`T0-DEBT-SELFTEST-CRITICAL-PATH` 已把完整矩阵移出 PR 关键路径，不再阻塞产品推进；剩余修法只针对合并后 canary：FAIL 点名闸 id、skip 可见、8.2e 负载容差。 | major | carded | `T0-DEBT-SELFTEST-CRITICAL-PATH` 已偿还 PR 关键路径耦合；`T0-DEBT-SELFTEST-FAIL-DIAGNOSTICS`（PR #31 · master `b8dee45` · merged）已偿还失败点名；`T0-DEBT-SELFTEST-SKIP-VISIBILITY`、`T0-DEBT-SELFTEST-LOAD-STABILITY` 待串行执行（无业务互依但共享 selftest，执行/合并宽度 1；三卡全 merged + post-merge core 重放后才可 paid） |

| TD137 | 2026-08-20 | `scripts/check-licenses.ps1` ↔ `scripts/license-scanner-check.ps1` ↔ `scripts/selftest.ps1` 17cc scanner | **Gradle diagnostics 权威实现与 selftest 旧回归漂移**：PR #33 为验证无 git skip 取消尾段切片后，17cc 暴露 CLI/plain password/token 泄漏、旧逐规则 marker 已消失、bounded tail 令 configuration/detail/safe-prefix 断言失配、parse 局部 mutation 被统一出口兜底而 vacuous。后果：seeded 长期红；若在 skip 卡内删/并断言换绿，又会降低安全回归并污染 TD9 scope。修法：独立恢复生产脱敏语义，把互相冲突的单记录夹具拆成 credential-continuation 与 benign-detail 两组，并将旧 marker 迁移为当前 record boundary 上逐类非 vacuous mutation，覆盖数与语义不得减少 / 可测：权威 diagnostics mutation 不减少，原 CLI/plain/key/auth/URI/config/detail/prefix/parse 语义全在；正常 seeded 尾段通过；逐个删除迁移后的守卫均专码 RED；无 git 全量 child 由后继 #33 合入本卡后证明 / 前置：T0-LICENSE-GAV-BOUNDS 已 merged；与 #33 共享 selftest，须先合并本卡再恢复 #33。 | major | carded | `T0-LICENSE-SELFTEST-DRIFT`（先于 PR #33） |
