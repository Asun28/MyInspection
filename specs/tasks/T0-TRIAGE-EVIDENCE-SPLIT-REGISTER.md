...
acceptance:
  ...
  - "A2 Preserve the existing CASE DoD, final obligations and original history; add only the approved predecessor dependency, structured restatements of acceptance, paired-document scope and doc_sync correction. Neither behavior is delivered by registration."
...
Only the two task-card payloads are registered. CASE keeps its original executable DoD; three acceptance rows restate its existing obligations. The previous doc_sync:none judgment is corrected because source diagnostics and CaseModeProfile alter the contract taught by LOOP-ENGINEERING. Product documentation changes belong to the two later behavior PRs, not this registration.
