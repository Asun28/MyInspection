...
## Current remote-candidate verification (2026-09-08)

The current scoped implementation changes only the 17a3 fixture and its shared cleanup helper. Trigger, matrix, task-help classification and operating-contract requirements already exist in the remote baseline and remain unchanged. Primary RED was recorded on ae40d6b7 before the scoped implementation.

Actual temporary-file checks reproduce and reject pre-existing-probe deletion and restoration I/O failure; the final helper preserves unowned content and attempts every restoration before worktree cleanup. Final full default selftest exited 0 in 2370.022 seconds, including real migration failure/order and cleanup assertions. Before/after selftest SHA-256 was identical: 1BCDA93FEF27E0ECFF639530A90900CEEFD8EA4A31A7DCADA48D93D9654170DF. Raw output and terminal receipt are in `.review/T0-SELFTEST-SCAFFOLD-ONLY.full-selftest.raw.log` and `.review/T0-SELFTEST-SCAFFOLD-ONLY.full-selftest.json`. This is functional validation, not a performance claim. Formal ship gates and independent R3 are recorded separately by task-loop.
