...
acceptance:
  ...
  - "A8 This is the remote delivery alias of T1-STORAGE-PATH-BOUNDARY, not a new product card. Preserve all 20 named direct boundary tests and rerun all 35 distinct final-source mutations with compile exit 0, test exit 1 and the specified primary AssertionError. Re-execute clean RED, full GREEN/DoD, restored DoD, formal R3 and CI on the new origin-based branch; record actual full app regression counts and exact source/test/diff pins. Local historical results do not satisfy remote execution gates. Do not reset or rewrite the original local branch, RED, five Policy BLOCK verdicts or local merge history."
...
# T1-STORAGE-PATH-BOUNDARY-REMOTE
...
Existing evidence remains with the blocked Policy branch and preserved handoff; it is historical only. For this delivery alias, use the original D:/Projects/MyInspection task-loop scripts: start -Base origin/master only after the registration PR is merged remotely, then red/ship -Base master without -Local. Do not transplant unrelated local master commits. After actual R3/merge/R5 closure, Policy may absorb the prerequisite by a non-rewriting merge and then replace its embedded path implementation with this API, rerunning RED/regression, DoD, final-pin R4 and formal R3 for its actual successor diff.
## Remote delivery identity and evidence

This alias completes the same product capability as T1-STORAGE-PATH-BOUNDARY and does not increment the ten-product-card delivery count. It is not the Policy successor. Preserve C:/wt/T1-STORAGE-PATH-BOUNDARY and its existing receipts until root separately authorizes cleanup; all phase, heavy, review, remote, R5 and cleanup resources remain explicitly scheduled by root.

Local history: feature c201c793f6c1bb3bab7ad0ac7d0168be968daf9c, local merge 115138a4306ae99a2c3a3066f7e46b209c5df97d, first formal Sol/high R3 pass with no findings. Production SHA256 41DCC7DFA834D3C04AF6EB2CF925B70FE5EF700C32EC739D0CEC54509204F3DB; the 390-line test body before its local receipt has SHA256 6CDE4BE12DD76B26056E645F03EF5415DCDC04DE0A1F2DAE67997433F87B1202. The 206 app tests, 20 direct tests, 35 named mutation kills and Windows Junction execution are historical local results. POSIX was not executed. The local reviewer could not rerun tests because of its process policy; raw refusals and independent test evidence are preserved in the 446-file local manifest.
