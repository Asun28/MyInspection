...
把 Field Ledger 补全为可实现的体验合同：页面类型、导航栈、触发入口、焦点恢复、证据主流程，以及离线安全/备份/恢复/清除旅程。

## 验收

执行 `dod_command`；所有路线有返回语义、失败有恢复动作，既有备份范围保持。
