...
title: Gradle 许可扫描套件接线与 TD2 总验收（TD2 收口卡 4/4）
...
doc_sync: 本卡 merge 后把 TD2 置 paid，并记录 PR #20 基线及四张收口卡的 PR/commit；归档四张已 merged 卡；同步 LICENSE-POLICY 与 RELEASE-CHECKLIST
---
...
PR #20 已在 master `b0a76d0` 合入 CI 顺序和 scanner 初版，但专用四套件、selftest 聚合与权威文档事实同步尚未完成。本卡只验收新形成的串行合同，并如实把 PR #20 记为共同基线；不得倒填四个不存在的历史 PR。
...
`T0-LICENSE-SCANNER`、`T0-LICENSE-POLICY`、`T0-LICENSE-DIAGNOSTICS`、本卡全部 merged；本卡 integration DoD、仓库 verify、R3 均通过；tracker 补齐 PR #20 基线与四张收口卡 PR/commit 后，TD2 才能从 `carded` 变 `paid`。
