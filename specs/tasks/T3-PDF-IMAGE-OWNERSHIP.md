...
that throws is not proof the image was released. At most one decoded image is held by the bridge at any moment;
that is the ownership property `PdfPageProgram.decodedByteBound` assumes, not a bound on native memory.
