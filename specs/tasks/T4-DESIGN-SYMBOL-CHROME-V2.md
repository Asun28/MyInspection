...
non_goals:
  - 非颜色载体分类学与各组件行的无障碍补齐（归 T4-DESIGN-STATUS-CARRIERS，2026-09-08 用户裁定拆出）
...
acceptance:
  ...
  - "A5 the amended status clause still forbids colour from being the sole carrier of state and no longer demands a visible text label unconditionally, resolving instead to the domain-value rule; the full non-colour carrier taxonomy and the component rows that do not yet satisfy it belong to T4-DESIGN-STATUS-CARRIERS"
