...
acceptance:
  ...
  - "A2 覆盖率受控：纯色图产出零个描边像素；单一高对比阶跃图的描边像素全部落在阶跃两侧的邻域内；请求覆盖率越低，产出的描边像素数不增；目标像素数**向上取整**，请求不被静默少给（非整数覆盖率需回归测试）；图中边缘少于目标时返回它有的全部边缘而非空层"
  - "A3 边框像素永不成为描边像素（1 px 边框无完整 3x3 邻域），且尺寸不足 3x3、像素数与宽高不符、覆盖率越界的入参一律拒绝而非静默产出"
  - "A4 dash 变体始终是 solid 变体的**子集**，但**不承诺严格更小**：周期粗过整幅图时什么也切不掉，而落在保留条纹上的孤立边缘能挠过任何周期（含最细的 1）——严格性取决于边缘落点，两处退化各需一枚反例测试钉住；且条纹周期可由入参改变并被断言看见"
