...
acceptance:
  ...
  - "A8 导出核心只在 close/reopen、bytes/size/SHA-256/type/semantic fingerprint 与 format-aware receipt 全部核验后创建不可伪造的 typed VerifiedReportArtifact；UI、路径、File、MIME、hash 或 Boolean 均不能构造或替代它。"
...
requirements:
  ...
  - "R3 当本机下游需要分享已验证报告时，导出核心应只交付由自身关闭、重开、字节/size/SHA-256/type/fingerprint 核验及格式回执后创建的 typed VerifiedReportArtifact；调用方不得用 raw File/path/MIME/hash/Boolean 伪造。"
...
复用现有 ReportOptions/ReportContent，在 exporting 边界创建不可由 UI 任意构造的确认值；并由同一边界唯一创建 typed `VerifiedReportArtifact`，其内部相对位置、format、size、SHA-256 和 semantic fingerprint 只能来自成功的 close/reopen 核验与格式回执。下游 ShareStaging 仅消费该值；不就地改变已归档投影合同。签名草案与安全边界见 `specs/android-module-boundaries.md`。
