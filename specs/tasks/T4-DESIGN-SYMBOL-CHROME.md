...
>
> **关闭（2026-09-25）**：承接卡 `T4-DESIGN-SYMBOL-CHROME-V2` 已合并（master `53673571`），本卡的
> A1–A4 范围由它交付。front matter 改为 `status: merged` + `superseded_by: T4-DESIGN-SYMBOL-CHROME-V2`，
> 沿用 `T0-REVIEW-LOW-RISK`（PR #340）的关闭形态：它只表示本卡经已合并的承接卡关闭，**不**声称本卡自己的
> DoD、R4 或 R3 已完成（本卡 R3 停在第 6 轮 block、PR #236 未合并）。
...
## R3 第 6 轮：一条 finding 属实，**未修**，编排者就此停手转人裁

`:1557-1564` 我给的两条分类互相重叠：边界说「记录派生的枚举状态永不折成字形」，
条件 1 与边界例外又接纳「拥有者已点明的二元存在标记 / 组件自定义二元状态标记」，
而**一个记录派生的二元状态同时满足两边** ⇒ 中心规则不可判定。属实。

**诊断（留给接手者，本轮未实施）**：病根是我自造的「读数是否超过两种」这条基数判据。
文档里真正成立的规则不是基数，而是**载体**——`:922` 的 `Settings` 错误点之所以可以无标签，
是因为它 `Merged into owner`、其拥有者（`Settings` 目的地）本身有可见标签并播报该状态；
`state-badge` 的「dots require an owning row label」是同一件事。故正解应是：
**领域值永不由字形单独承载；字形可以额外标记它，只要拥有者以文字或播报给出该值**——
一条规则同时覆盖计数、巡检项状态、错误点与 DOT，且删掉基数判据这个我发明出来的东西。
评审给的备选（「所有记录派生值一律保留可见文字」）在本文档里**不成立**：它会判 `:922` 违规。

## 停手依据（2026-09-06）

六轮 R3 共 **11 条 finding，全部属实**，且形态高度一致——**新写的中心规则与文档既有实例不符**，
每轮修完措辞、下一轮就在另一处冒出新缝。轮次上限已两次经用户裁定 reset。编排者按
`docs/QUALITY-RUBRIC.md` §5 与 CLAUDE.md「maker 与 checker 两轮互不认可即停」的精神主动停手，
**不请求第三次 reset**，把 PR #236 的处置交人裁。经验已入账 **L300**（成熟文档加中心规则须先做
「实例代入表」），并 bump **L196**（变异批重叠）。
...
| 2026-09-25 | 关闭：`status: todo` → `merged` + `superseded_by: T4-DESIGN-SYMBOL-CHROME-V2`（承接卡已合并 `53673571`），不再列为待做；本卡自身未交付，见顶部退役块。 |
