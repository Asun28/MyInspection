...
| 错峰实现代码树（连续两次正式默认 `all` 全绿） | `pwsh -NoProfile -File scripts/selftest.ps1` | 250.6 / 239.5 秒 |

早期五次优化样本虽均低于 300 秒，R3 连续反例证明“三分片同时冷启动”在 Windows 受限资源下不稳定。最终策略让 `workflow` 与 `seeded` 先并发，75 秒后以低优先级加入短 `core`；正式默认 `all` 连续两次全绿为 250.6 / 239.5 秒，较顺序基线缩短约 54.6–56.6%。正确性的可复现代理不是单次硬计时，而是 selftest 8.2e 的长分片 barrier + core 时间差变异锁：两个长分片必须并发，core 必须在它们就绪后真实错峰，否则夹具确定性失败。
