---
id: T0-HARNESS-PERF
title: 横切优化 selftest 与 CI 墙钟时间（约 270 行 harness/测试改动）
...
dod_exit: 0
dod_assert: core 分片退出 0；8.2e 证明 2 OS×3 分片显式接线、矩阵变异必红、all 聚合器 Git/非 Git 快照、失败传播、StrictLint 转发及清理。完整 all 另须退出 0 且墙钟不高于 300 秒。
...
这是横切 harness 性能卡，明确批准约 270 行生产/回归测试改动，范围限于 selftest 分片聚合、CI 接线与三份权威说明。当前顺序基线为 552.4 秒；目标是在不减少任何闸门或 OS 覆盖的前提下，把默认本地墙钟降到 300 秒以内。
