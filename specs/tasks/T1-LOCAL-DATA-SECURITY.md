...
forbid:
  ...
  - 修改前置 T1-LOCAL-SECRET-BOX 的信封格式、alias/AAD 隔离与状态映射（LocalSecretBox.kt 只许改 SecretKeyPort.keyForSeal 的 KDoc，写明下述 R2 重建例外），或 T1-LOCAL-SECRET-STORE 的信封存储；新增 Gradle/runtime 依赖、Robolectric 或仪器测试框架；源码文本断言冒充平台行为
