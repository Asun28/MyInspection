...
acceptance:
  ...
  - "A2 Resolve-PrereviewBase -Local resolves the base name through Resolve-ScaffoldBaseRef -PreferLocal from scripts/_gitbase.ps1 (dot-sourced, not mirrored) and returns base_mode local with the pinned OID; without -Local it returns the origin ref and base_mode origin; no merge-base with HEAD yields [PRE-NO-MERGE-BASE]."
  - "A3 Get-PrereviewSnapshotTree equals git write-tree of a temporary index built from read-tree HEAD plus add -A, so dirty and untracked non-ignored files are included and gitignored files are excluded; the fixture proves both with a planted .env and a planted untracked file, and the same tree twice yields the same oid."
