...
「一律以 TASK-BOARD 为准」。本卡初稿照 `CLAUDE.md` 概述取值，把 `T2-CAPTURE-CORE` 记成 7（实为 6）、
把 `T2-PHOTO-PIPELINE` 记成触顶那轮 9（实为人裁后又跑到 11），R3 第 1 轮据仓库记录当场指出，**两处已订正**，
结论不变（r^2 从误算的 0.18 变为 0.19；front-matter 的 A5 保留初稿写法 `0.18` 不动，订正记在 L241 正文里）。

真正的判别式是**验收集合封不封闭**。24 份终局裁决共 25 条 finding，其中 **18 条（72%）** 是 rubric #6
...
而不是撤回它们。每条须点名精确边界值、精确 ASCII 哨兵/异常类型、注入机制，以及能杀死该守卫的单句变异。

两份清单的条目来源是三方交叉的：各卡历史 R3 裁决（`.review/<branch>.json`）、各自 `*-R3-CLOSURE` 卡
已写下的「单一产出 / 验收证据」节，以及 2026-08-23 对两个 PR 全量 diff 的独立复核。
...
`review.ps1:354` 把整张卡原文注入评审者提示词，所以清单确实抵达 R3。
