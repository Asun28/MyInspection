...
acceptance:
  ...
  - "A5 runtime acceptance tests invoke compiled reducers and screen entry points with concrete inputs and production-used injected permission, scheduler and route ports, assert only domain state and recorded effects, and carry executable semantic mutation receipts; source, resources, and inspected compiled artifacts are never an oracle"
...
dod_exit: 0
dod_assert: black-box production-state tests pin no startup request, action-time/resume permission refresh, grant/revoke/deny/settings/retry transitions, badges, filters and route callbacks, with A1-A5 semantic-mutation receipts and no source-derived oracle; assembleDebug compiles Compose wiring.
