...
acceptance:
  ...
  - "A7 非默认通知上下限：noticeMinHours=72 且 noticeMaxDays=7 的配置下，48h 被 NOTICE_TOO_SHORT 阻断、72h 放行、10 天被 NOTICE_TOO_EARLY 阻断，三例都跨过 48/14 这两个默认常量。**注入机制**：配置用测试夹具的 JSON 生成器造一份**完整合法**文档并经 `ComplianceConfigLoader.load` 装载（既不改 `configs/compliance/nz-rules-v1.json`、也不用 override 文件）。只有加载器按 v1 契约必然拒绝的取值（A11 的非 Pacific/Auckland 时区）才准用 `ComplianceConfig` 的 internal 构造器直接建——对加载器接受得了的取值改用 internal 构造器，等于把一条**同时验证了加载器**的证据换成一条绕过校验的弱证据。"
  - "A8 非默认时窗：start=09:00/end=17:00/boardingHouseEnd=16:00 的配置下，08:30 与 18:00 被 OUTSIDE_VISIT_WINDOW 阻断、17:00 放行、同一 17:00 在寄宿模式被阻断，四例都跨过 08:00/19:00/18:00。**注入机制**：配置用测试夹具的 JSON 生成器造一份**完整合法**文档并经 `ComplianceConfigLoader.load` 装载（既不改 `configs/compliance/nz-rules-v1.json`、也不用 override 文件）。只有加载器按 v1 契约必然拒绝的取值（A11 的非 Pacific/Auckland 时区）才准用 `ComplianceConfig` 的 internal 构造器直接建——对加载器接受得了的取值改用 internal 构造器，等于把一条**同时验证了加载器**的证据换成一条绕过校验的弱证据。"
  - "A9 非默认频率天数：frequencyDays=42 的配置下，相距 30 个民历日的 ROUTINE 历史仍以 FREQUENCY_LIMIT 阻断（30 已越过默认 28）。**注入机制**：配置用测试夹具的 JSON 生成器造一份**完整合法**文档并经 `ComplianceConfigLoader.load` 装载（既不改 `configs/compliance/nz-rules-v1.json`、也不用 override 文件）。只有加载器按 v1 契约必然拒绝的取值（A11 的非 Pacific/Auckland 时区）才准用 `ComplianceConfig` 的 internal 构造器直接建——对加载器接受得了的取值改用 internal 构造器，等于把一条**同时验证了加载器**的证据换成一条绕过校验的弱证据。"
  - "A10 非默认豁免集合：exemptTypes=[\"ROUTINE\"] 的配置下，27 日内的 ROUTINE 历史不再阻断 ROUTINE 请求，而 27 日内的 INGOING 历史反过来阻断 INGOING 请求，两例合起来证明豁免集合读自配置而非 ComplianceEngine 里的字面量。**注入机制**：配置用测试夹具的 JSON 生成器造一份**完整合法**文档并经 `ComplianceConfigLoader.load` 装载（既不改 `configs/compliance/nz-rules-v1.json`、也不用 override 文件）。只有加载器按 v1 契约必然拒绝的取值（A11 的非 Pacific/Auckland 时区）才准用 `ComplianceConfig` 的 internal 构造器直接建——对加载器接受得了的取值改用 internal 构造器，等于把一条**同时验证了加载器**的证据换成一条绕过校验的弱证据。"
  ...
  - "A15 历史用途未知即 fail closed：history 行的 entryPurpose 不在 config.rules 键集内时以 INVALID_HISTORY_ENTRY 阻断；另有一例用**第二个已配置用途**证明「同物业不同用途」仍正常跳过——两例必须分属不同测试，不得共用一个既未配置又用作『其它用途』的夹具。**注入机制**：权威 JSON 只有 `inspection` 一个用途键，故第二个用途由测试夹具的 JSON 生成器多写一条 rules 键（如 `fixture-purpose`，取值须与 inspection 逐项不同，好让「按用途取到的是哪条规则」可分辨）并经 `ComplianceConfigLoader.load` 装载；权威 JSON 本身不动。"
  - "A16 拒绝分支逐条可证伪：built-in schemaVersion、effectiveDate、未知时区、非 v1 时区、sourceRefs 空、sourceRefs 重复、rules 空、purpose 非法、noticeMinHours/noticeMaxDays/frequencyLimit.days 非正、min 超 max、exemptTypes 重复/未知、时刻非 HH:mm、普通窗倒置、寄宿窗宽于普通、寄宿窗倒置，每例断言 ComplianceConfigException.errors **包含该分支自己的诊断串**（不得只断言 errors 非空），**且这些诊断串两两不同**——两条分支共用一串时，删掉其中一条会由兄弟分支在同一夹具上补位，没有任何测试变红。sourceRefs 的逐条校验有**五个子条件**（ISO 控制符 / URI 不可解析 / 非 https / 空 host / 带 userInfo），各配一枚负例，除诊断串外再断言 **errors 的完整有序列表**与该条目的下标前缀（形如 `sourceRefs[1]:`），并各配一枚删除该子条件的变异。后四条各写成**只触发该子条件**的单错负例（实测：`https://` 只出 `must be a parsable URL`、`http://…` 只出 `must use HTTPS`、`https:///path` 与 `https:x` 只出 `must name a host`、`https://attacker@host/` 只出 `must not carry credentials`）。**控制符那一条不是单错负例**：`java.net.URI` 对任何 ISO 控制字符都抛异常（Java 的 URI 只放行「码位 > 128 且非空白非控制」的非 ASCII 字符），故它必然**同时**产出 `must not contain control characters` 与 `must be a parsable URL` 两条——它的期望列表就写这两条、按此顺序（控制符预检排在 parse 之前，所以控制符那条在前）。把它写成「只触发该子条件」会逼实现删掉控制符预检、或把它挪到 parse 之后（那里它永不触发），即验收条目反过来制造缺陷。另加一例**同一 JSON 对象内的重复键**，断言 errors 含点名该键名的诊断（权威 override 的 SHA-256 认证的是整份字节，重复键让『字节说什么』落进信任边界内）。坏 UTF-8 以 CharacterCodingException 精确失败、未知 JSON 键以 SerializationException 精确失败"
  ...
  - "A19 阻断不可绕过（`forbid` 那条点名『本身就是验收断言』的面）：**只用 `java.lang.Class` 反射**（`::class.java`）——`kotlin-reflect` 不在 `android/gradle/libs.versions.toml`、离线缓存也没有本项目 pin 的 2.4.10 版，引入它要改本卡 `allow_paths` 之外的依赖目录与 build 脚本。断言：① `ComplianceEngine.declaredMethods` 里 public 且非 synthetic 的**恰好只有** `validateSchedule(ScheduleRequest): ScheduleValidation` 一个，无任何 public 字段或 setter（areTooClose / blocked / config 一律 private）；② `ComplianceEngine.declaredConstructors` 里非 synthetic 的恰好一个，形参类型精确等于 [ComplianceConfig]；③ `ScheduleRequest.declaredFields` 里非 synthetic 者的 **(名字 → genericType) 完整映射**精确等于 {propertyId:String, entryPurpose:String, inspectionType:String, isBoardingHouse:boolean, scheduledAt:Instant, noticeGivenAt:Instant, tenantConsented:boolean, existingEntries:List<ExistingScheduledEntry>, currentEntryId:String} 九项——是**完整集合而非名字黑名单**，任何新增的可空/布尔形参都会让它红（用 declaredFields 而非构造器形参名：Kotlin 默认不发 `MethodParameters`，Java 反射只读得到 arg0..argN；`currentEntryId` 有默认值，declaredConstructors 里还会多一个带 `int, DefaultConstructorMarker` 的合成构造器，须按 `isSynthetic` 滤掉）。变异：给 validateSchedule 或 ScheduleRequest 加一个 `bypass: Boolean = false`，反射断言必红。"
  ...
  - "A21 变异证明：A1–A20 每条守卫各配一枚**单句删除/反转**变异，跑一遍并把「变异 → 哪一条断言变红 → 失败文本」的对照表落进 **diff 本身**（写进被改测试类的 KDoc 或同包一份新的证据注释块），**不要只写 PR body**：`review.ps1` 只把 git diff 喂给评审者，PR 描述从不进入其上下文（L227）。任一条守卫找不到能杀死它的单句变异，即视为该条未完成。判据按 L165 的分类器：**非零退出且命中指定断言文本**才算击杀。"
dod_command: cmd /c android\gradlew.bat -p android --offline --no-daemon -q --rerun-tasks --no-build-cache :core:test --tests "nz.myinspection.core.compliance.*"
dod_exit: 0
