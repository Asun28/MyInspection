...
allow_paths:
  # 横切卡（L97）：同步产品与教学面；既有 selftest 14g roster parser 另卡更新。
...
dod_exit: 0
dod_assert: `scaffold-sync selfcheck` 打印 ASCII 哨兵 PASS（覆盖版本解析 / 更新集 / Downstream 块切割 / 决策账高水位与账域四负例 / 心跳离线不变量），且 `triage.ps1` 恰 11 个探针函数；reference-only 教学面同步但不宣称既有 14g roster parser 已更新
acceptance:
