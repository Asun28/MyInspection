...
dod_exit: 0
dod_assert: docs/SMOKE-CHECKLIST.md 存在且含全流程条目（建物业→Ingoing→Routine（草稿恢复/杀进程）→拍照 overlay/系统图片选择→听写/短语→finalize→双版 PDF→通知生成回记/时区频控→备份导出→清库恢复→保留期清理→全量本机 ERASE），并引用 T3-FIELD-UX-ACCEPTANCE 的 56 项全绿证据，覆盖飞行模式完整流程、本地/USB 与云 SAF 离线、授权收回、低空间、Keystore 失效/重输口令、错误口令/损坏敌意包、restore journal 各 commit 点杀进程、敏感页截图/recents、PDF 临时分享授权、本机健康提示/崩溃恢复/release mapping 回执；每条有真机勾选结果；发现的缺陷逐条列出并标注（本卡内修 / 立新卡）；verify 全绿
...
## 上下文包（执行模型必读）
- 清单结构照 PLAN §2 设备侧冒烟 + 需求 §14 + ADR-0006 威胁/离线矩阵：相机旋转/overlay 对位/系统图片选择/离线听写/飞行模式完整巡检与 PDF/本地和 USB 备份恢复/Drive-OneDrive provider 离线与授权收回/低空间/Keystore 失效/敌意包/restore commit 点进程死亡/全量本机数据清除/本机健康与崩溃恢复/release mapping 回执/敏感页 screenshot-recents/PDF 临时授权与中文字形。
