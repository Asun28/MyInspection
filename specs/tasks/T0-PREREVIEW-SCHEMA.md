...
dod_exit: 0
dod_assert: Default mode exits 0 and prints [PREREVIEW-SCHEMA-OK] after every records/valid sample validates, every records/reject sample (candidate with verdict, status pass, unknown field, missing schema_version, worker-emitted id, worker-emitted missing, provenance inside records[], cross-file $ref) returns Test-Json $false under -ErrorAction SilentlyContinue, -Anchors on anchors/ok.md passes while missing-row, extra-code and duplicate-row each print [PREREVIEW-SCHEMA-FAIL], and -Schema/-Samples on mini/ passes; the non-comment lines of scripts/_config.ps1 contain PrereviewEnabled = $true, PrereviewGateEnforced = $false and PrereviewBatchCap = 2.
...
Amendment (2026-09-15, task owner): the split rule above (over 500 lines before RED moves -Anchors to a follow-up card) is withdrawn for this card; -Anchors ships here so PROTOCOL-DOC's DoD keeps its dependency. R3 round 1 asked for this explicit amendment.
