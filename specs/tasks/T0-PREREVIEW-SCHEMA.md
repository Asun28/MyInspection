...
dod_exit: 0
dod_assert: Default mode exits 0 and prints [PREREVIEW-SCHEMA-OK] after the record schema passes hygiene and the contract walk, every records/valid sample validates (facts.*.json and units.*.json against their $defs), every records/reject sample (candidate with verdict, status pass, unknown field, missing schema_version, worker-emitted id, worker-emitted missing, provenance inside records[], blocked without missing_context, symbol null without line_start, facts base_mode outside the enum) returns Test-Json $false under -ErrorAction SilentlyContinue and the reject/ inventory equals the declared class list, records/reject/cross-file-ref.schema.json fails -Schema, -Anchors on anchors/ok.md passes while missing-row, extra-code and duplicate-row each fail (the CLI prints [PREREVIEW-SCHEMA-FAIL] for them), and -Schema worker-envelope.min.json -Samples mini/ passes; the non-comment lines of scripts/_config.ps1 contain PrereviewEnabled = $true, PrereviewGateEnforced = $false and PrereviewBatchCap = 2.
...
- Gaps the plan left, closed in the schema and pinned by the default mode: `base_mode` = `local` | `remote` (FACTS-LIB A2 still says `origin`: one word to align on master before that card starts); `risk_class` = `standard` | `risky`; `models` = `{discoverer: {model, effort}, lens: {enabled, model, doc_model, effort}}`; `pack_layout_version` integer >= 1; `unit.hunk_header` null for a `path#file` unit; `severity_guess` carries `null` as an enum member; the closed object behind A1's "units" is `$defs/unit`, `$defs/units` being the array of it. Enum values are lower_snake except the identifier enums `category` and `status_code`; the contract walk admits exactly those two shapes and compares case-sensitively.
- Projection rule (3769 bytes, one line, UTF-8 without BOM, no trailing newline): keep the root and the `$defs` reachable from it (candidate, coverage, category, unit_id), keep only the A3 keywords, drop the rest (`$schema`, `title`, `$comment`, `pattern`, `minLength`, `minimum`, `minItems`, `uniqueItems`, `if`, `then`). Re-derive after any in-window patch.
- Samples: `<def>.<name>.json` validates against `$defs/<def>` through an in-memory `{$ref, $defs}` wrapper, anything else against the whole schema; `*.schema.json` is a schema for -Schema, never a sample. The reject/ inventory (11) is declared in the script and must equal the directory: the ten Test-Json classes of dod_assert plus cross-file-ref.schema.json, which proves the cross-file `$ref` check (its copied `$defs/candidate` keeps the no-target check silent; copying `$defs` is forbidden by this card, not detected by the script).
- Size at ship: about 800 changed lines / 58K diff chars by review.ps1's rule. The Notes' split rule (over 500 lines before RED moves -Anchors to a follow-up card) was measured too late (L266) and then not applied on purpose: moving -Anchors out leaves about 600 lines and orphans PROTOCOL-DOC's DoD, which calls -Anchors; the budget was met by trimming instead.
- R4: 34/34 single-point mutants killed, each by a behavioural failure line, every script mutant still parsing. Script: S01-S11 delete one inventory line each (kill: inventory drift), S12 duplicate check, S13 extra-code check, S14 missing-row check (kill: that negative doc passes), S15 `$expect = $true`, S16 def routing off (kill: facts sample rejected), S17 cross-file check `if ($false)` (kill: the cross-file schema passes). Fixture: F01 the cross-file `$ref` turned `#/$defs/candidate` (kill: same). Schema: M01/M02 `additionalProperties` removed from candidate/coverage, M03 status enum + `missing`, M04 + `pass`, M05/M06 candidate/coverage `if` removed (kill: the if/then reject sample validates), M07 `schema_version` dropped from required, M08 a 21st status code (kill: anchors/ok.md red), M09 base_mode + `origin` (kill: facts reject sample validates), M10 a `verdict` property in facts, M11 `Risky`, M12 a cross-file `$ref` in unit_ids, M13 `const: 2`, M14 `Medium` (kill: contract walk). Config: C01 `PrereviewGateEnforced = $true`, C02 `PrereviewBatchCap = 3` (kill: the DoD's config assertion). SHA-256 of the mutated files' final bytes: schema 5bee116e86b3ec0589dd66dc69fc6f96244e6fc2483b404a65adbd97a48efcce, script f9a9acd4b868fd5a8513072a607a62ba9a9a561d444c5a0872720dbe5b226c1b, _config 279ddf09fa9fb0cafc93e08380d4a6c96602404f8d40fb7c32f41c1b09f7da32, cross-file fixture 9333a74c6208d7826b1d8729bd22193c852cc7776637ae1abfca28dfa6c1520a.
- `-Anchors specs/tasks/T0-PREREVIEW-SCHEMA.md` (the table below) prints [PREREVIEW-ANCHORS-OK]. Eight in-flight branches modify scripts/_config.ps1 relative to master (collision rule); this card appends one block and keeps the file's BOM, retiring them stays a user decision.
## Notes
