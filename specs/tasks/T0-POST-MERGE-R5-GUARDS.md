...
- A2: one real `-DryRun` of `T0-POST-MERGE-LESSONS` (status todo) with sample prose, on base `8146d794`, exit 0
  (taken before the last edit, which changed only the synopsis line and one comment),
...
- A3: the `.SYNOPSIS`, the description, an example and the usage line name `-DryRun`.
  `post-merge.ps1 prune -Branch T0-POST-MERGE-R5-WIRING -DryRun` printed `[POST-MERGE-INPUT] prune has no preview:
