...
- `scripts/_prereview-records.ps1` (`-AsLibrary` / `-SelfCheck`, 80 assertions; the header comment states the rules) + `scripts/fixtures/prereview/records/` (four units, 11 stamped records from two workers, 12 pinned reject classes). Identity is ordinal; the exact-duplicate and `(worker, local_id)` / `(worker, unit_id)` keys are JSON-array tuple encodings; the counter is `[long]`, exhaustion pre-checked on unique exact keys; coverage recomputes the candidates result from the supplied `StartId` and accepts only a byte-identical `Candidates` / `LocalIdMap` / `NextId`; each contributor's scalars stay in its provenance entry; units must be unique; `-SelfCheck` dot-sources nothing.
- Local pre-review before R4 (`/code-review high`): 7 findings, all fixed in this diff.
- R3 round 1 (5 findings, all fixed): guard dereferenced `LocalIdMap` and accepted an altered map; `[int]` NextId; `-SelfCheck` dot-sourced `_encoding.ps1` against A6; unions / one-axis near duplicates untested; no unit without a discoverer row. Round 2 (2, fixed): provenance-coherence check still accepted a coordinated id + map rewrite and a provenance-free pad, replaced by recomputation from `StartId` (ten forged shapes refused; the BatchId digest became redundant and was removed); exhaustion counted raw records before merging, now unique keys. Round 3 after a user-ruled `ResetRounds` (2, fixed): canonical map keys sorted with culture `Sort-Object` (case-variant keys tie), now `StringComparer.Ordinal` with a `d1` / `D1` reverse-insertion regression; `#requires` raised to 7.4 with the three reasons in the header.
- R4: 46/46 single-point mutants killed by a named behavioural FAIL line (the runner counts parse errors and unnamed exits as survivors; a redundant `Ok` guard, S22, and an unreachable map-lookup branch found this way were deleted; S28 plants PRE_LIVE / PRE_LENS_ENDPOINT before spawning). S01-S08 one per reject class, S09/S10 monotonic NextId, S11-S13 merge key (exact, NFC, whitespace), S14/S15 union / provenance, S16-S18 fingerprint / root_group / related_to, S19/S20 missing / discoverer-only, S21 candidate_ids, S23/S24 fail-closed batch per normaliser, S25-S27 ordinal maps / tuple keys, S28 env, S29 units schema, S30 ordinal content, S31 binding refusal, S32 units dictionaries, S33 one row per (worker, unit), S34 `-NoEnumerate`, S35 contributor scalars, S36 key covers `actual`, S37 missing row without any row, S38 ordinal sort, S39 long counter, S40 recompute equality, S41 NextId equality, S42 duplicate unit_id, S43 exhaustion, S44 exhaustion on unique keys, S45 ordinal key sort; F01 reject made clean, F02 l1 not a duplicate. Final script SHA-256: f8375d99dc72e0e7b5f157cc56306903b413daebdd8827d026d830ebf0db265a.
