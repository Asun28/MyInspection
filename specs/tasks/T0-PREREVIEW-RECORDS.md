...
- `scripts/_prereview-records.ps1` (`-AsLibrary` / `-SelfCheck`, 78 assertions; the header comment states the rules) + `scripts/fixtures/prereview/records/` (four units, 11 stamped records from two workers, 12 pinned reject classes). Identity is ordinal; the exact-duplicate and `(worker, local_id)` / `(worker, unit_id)` keys are JSON-array tuple encodings; the counter is `[long]` with exhaustion pre-checked; coverage accepts only the candidates result minted from its own batch (BatchId, LocalIdMap equal to the map derived from `Candidates[*].provenance[*]`) and re-validates the batch; each contributor's scalars stay in its provenance entry; units must be unique; `-SelfCheck` dot-sources nothing.
- Local pre-review before R4 (`/code-review high`): 7 findings, all fixed in this diff.
- R3 round 1: block, 5 findings, all accepted and fixed here: (1) BatchId-only guard dereferenced `LocalIdMap` and accepted an altered map, now the result is validated against its own provenance (missing / non-dictionary / out-of-set / short / in-set remap / padded maps refused without a throw; a fresh-context check of this fix round found the in-set remap and the generic-dictionary throw, both closed); (2) `[int]` NextId, now `[long]` with an Int32 boundary test and an Int64 exhaustion test; (3) `-SelfCheck` dot-sourced `_encoding.ps1` against A6, removed; (4) unions / one-axis near duplicates untested, now unsorted disjoint `evidence_refs` / `evidence_needed` in the fixture and a loop over the six key fields; (5) no unit without any discoverer row, U4 added.
- R4: 44/44 single-point mutants killed by a named behavioural FAIL line (the runner counts parse errors and unnamed exits as survivors; a redundant `Ok` guard, S22, and an unreachable map-lookup branch found this way were deleted; S28 plants PRE_LIVE / PRE_LENS_ENDPOINT before spawning). S01-S08 one per reject class, S09/S10 monotonic NextId, S11-S13 merge key (exact, NFC, whitespace), S14/S15 union / provenance, S16-S18 fingerprint / root_group / related_to, S19/S20 missing synthesis / discoverer-only, S21 candidate_ids, S23/S24 fail-closed batch per normaliser, S25-S27 ordinal maps / tuple keys, S28 env, S29 units schema, S30 ordinal content, S31 binding refusal, S32 units dictionaries, S33 one row per (worker, unit), S34 `-NoEnumerate`, S35 contributor scalars, S36 key covers `actual`, S37 missing row without any row, S38 ordinal sort, S39 long counter, S40 map coherence, S41 map type, S42 duplicate unit_id, S43 counter exhaustion; F01 reject made clean, F02 l1 not a duplicate. Final script SHA-256: ef572e452dc0db9c97a15f780dab8a17550c4caea05f69ce8f5c38afcd7f19e1.
