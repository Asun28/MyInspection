...
- `scripts/_prereview-records.ps1` (`-AsLibrary` / `-SelfCheck`, 66 assertions; the header comment states the rules) + `scripts/fixtures/prereview/records/` (four units, 11 stamped records from two workers, 7 reject files plus 5 reject cases derived in memory). Identity is ordinal throughout, library and harness (`[string]::Equals(..., Ordinal)`, ordinal sets; PowerShell's comparison operators and `Select-Object -Unique` are culture comparisons that equate soft hyphens, zero-width characters and NFC/NFD variants); tuple keys are JSON-array encodings; the counter is `[long]`, exhaustion pre-checked on unique exact keys; bad caller input (NextId, Kind, Path, a null result) is a failed result or `$false`, never a throw; coverage recomputes the candidates result from `StartId` and requires the whole result object (seven properties, canonical JSON, exact property-name set) to match; state shapes (candidate, provenance entry, coverage row) are pinned key by key; `-SelfCheck` dot-sources nothing.
- R3 rounds 1-5 (two user-ruled `ResetRounds`, every finding accepted and fixed): binding dereference and altered / coordinated / padded / status-only forgeries (now recomputation), `[int]` counter and exhaustion on raw records, `-SelfCheck` dot-sourcing, untested unions and missing-row cases, culture `Sort-Object` and `#requires` floor, throwing on bad NextId, culture operators in the harness. Two fresh-context sweeps between rounds closed: `-ceq` itself culture-based (soft hyphen / ZWSP / NFD), collection unrolling, extra property names, array map values, `Select-Object -Unique`, null / bad-Kind / bad-Path throws, unpinned output shapes, `-match` in the harness, the inventory filter, a dead guard.
- R4: 56/56 single-point mutants killed by a named behavioural FAIL line (parse errors and unnamed exits count as survivors; redundant guards S22 / S50 and an unreachable lookup branch were deleted; S28 plants PRE_LIVE / PRE_LENS_ENDPOINT before spawning). S01-S08 reject classes; S09/S10 NextId; S11-S13 merge key; S14/S15 union, provenance; S16-S18 fingerprint, root_group, related_to; S19/S20 missing; S21 candidate_ids; S23/S24 fail-closed; S25-S27 ordinal maps, tuple keys; S28 env; S29 units schema; S30 ordinal content; S31 binding; S32 units dictionaries; S33 one row per (worker, unit); S34 `-NoEnumerate`; S35 contributor scalars; S36 key covers `actual`; S37 missing without rows; S38 sort; S39 long; S40/S41/S46 recompute equality incl. NextId and Ok / Code / Reasons; S42 duplicate unit_id; S43/S44/S52 exhaustion, unique keys, invalid NextId; S45 key sort; S47 ordinal equality; S48 no unrolling; S49 property names; S51 ordinal discoverer; S53 provenance anchor; S54 `-DiscovererWorkerId`; S55 categories_checked kept; S56 only local_id dropped; F01/F02 fixtures. Final script SHA-256: c8a20f7e89edbc0bebf3b28777794d25f0326ec00665909a029f26a0d69362ec.
