...
acceptance:
  ...
  - "A3 selftest 复制专用假 gradlew fixture 到临时目录，验证 Gate 2 success/failure/missing 三态，且不写真实 verify.ps1"
...
- selftest 从 `scripts/fixtures/gate2/android/gradlew.bat` 复制专用假 wrapper 到临时仓，执行真实 verify 副本并验证 success/failure/missing；不得在 selftest 内联构造 wrapper，变异只写临时副本，不碰生产文件。
