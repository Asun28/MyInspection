...
dod_command: $w = (& pwsh -NoProfile -File scripts/selftest.ps1 -Shard workflow *>&1 | Out-String); $we = $LASTEXITCODE; $w; function EL([string]$s) { '(?m)^[ \t]*' + [regex]::Escape($s) + '\r?$' }; $wp = EL '15r(e) receipt-loss fail-closed 行为合同 OK（missing/corrupt/inconsistent/store-unavailable 四态均在 RED/receipt 边界 T35 非零停止；review/status/comment/candidate-CI/merge/cleanup/T24/reset 哨兵全缺；有效四谓词 -NoAutoMerge 正例走完 normal ship；满足无远端后继且 HEAD 为有效 redSha 非等后继的未发布 reset-safe 正例允许精确归位；已发布/远端分叉均禁止历史改写）'; if ($we -ne 0 -or ([regex]::Matches($w,$wp).Count -ne 1)) { exit 1 }; $r = (& pwsh -NoProfile -File scripts/selftest.ps1 -Shard seeded-remote *>&1 | Out-String); $re = $LASTEXITCODE; $r; $rp = EL '闸15t / T37-REMOTEMX/4 receipt-loss fail-closed 单一路径 OK（真实 push fixture；唯一 publish-start marker；receipt 四态失效负例在 RED/receipt 边界停止且下游哨兵全缺；已发布/partial/remote-ahead/diverged 均不 reset/rebase/历史改写；有效四谓词 receipt 的 -NoAutoMerge 正例经 normal ship 到最终快照）'; if ($re -ne 0 -or ([regex]::Matches($r,$rp).Count -ne 1)) { exit 1 }
dod_exit: 0
dod_assert: 真实已铸 receipt 在 watershed 后进入 missing/corrupt/inconsistent/store-unavailable 四态，均在 RED/receipt 边界以稳定 T35 状态非零停止，保留 worktree/branch/PR/evidence 并指向人工升级；review/status/comment/candidate-CI/merge/cleanup/T24/reset 哨兵全缺。有效四谓词 receipt 的 -NoAutoMerge 正例仍经 normal ship 到最终快照。未发布正例须证明 reset 前 HEAD 是有效 redSha 的非等后继、远端无 post-RED 提交，reset --soft 后 HEAD 精确归位 redSha；partially-pushed/remote-ahead/diverged 必须零 reset/rebase/历史改写。DoD 专用成功行必须逐字为：15r(e) receipt-loss fail-closed 行为合同 OK（missing/corrupt/inconsistent/store-unavailable 四态均在 RED/receipt 边界 T35 非零停止；review/status/comment/candidate-CI/merge/cleanup/T24/reset 哨兵全缺；有效四谓词 -NoAutoMerge 正例走完 normal ship；满足无远端后继且 HEAD 为有效 redSha 非等后继的未发布 reset-safe 正例允许精确归位；已发布/远端分叉均禁止历史改写）；闸15t / T37-REMOTEMX/4 receipt-loss fail-closed 单一路径 OK（真实 push fixture；唯一 publish-start marker；receipt 四态失效负例在 RED/receipt 边界停止且下游哨兵全缺；已发布/partial/remote-ahead/diverged 均不 reset/rebase/历史改写；有效四谓词 receipt 的 -NoAutoMerge 正例经 normal ship 到最终快照）。
