...
dod_exit: 0
dod_assert: triage selfcheck 打印 PASS，覆盖 cap 两侧、promote/demote、delivery-blocked 与 resident Markdown marker/boundary/fence；常设 selftest 同步探针接线与选择性夹具依赖
review_gate: codex {verdict:pass}
hygiene: triage 与其直接 selftest 接线从 PR #127 精确提取；文档 roster 断言仍留原尾卡，且本卡完整 diff 低于 1000 行 / 60000 字符
doc_sync: 原 PR #127 的文档尾卡统一收口，避免拆分过程中制造双源
---

# T0-LESSONS-CAP-TRIAGE-SPLIT

依赖共享核落地后，本卡提取 PR #127 的 `triage.ps1` 完整消费者、hermetic 夹具，以及随探针数/
dot-source 接线必须同提交更新的常设 `selftest.ps1` 断言。
