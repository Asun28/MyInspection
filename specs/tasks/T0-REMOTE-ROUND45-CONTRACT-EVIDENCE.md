...
hygiene: Synthetic authority controls must reject co-edited candidate and pins plus wrong record repository/ref/path/commit/blob/whole SHA/scope; source and scope corruption controls must fail named guards. An actual root approval record and full DoD are still required.
