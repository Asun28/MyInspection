...
PR #314 merged on 2026-09-18 as `6ae82ab6`, but R5 never set this card's status. The card-status check that opened `T0-POST-MERGE-CARD-DRIFT` found it. Its DoD passes on origin/master `8146d794`. What happens to the rest of the prereview adoption chain is still pending the user's ruling (see the TASK-BOARD note); this status records only that this card's PR merged. This commit changes only the status line and adds this note.
