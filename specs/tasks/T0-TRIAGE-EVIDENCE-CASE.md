...
depends_on: [T0-LESSONS-CAP-TRIAGE-SPLIT, T0-TRIAGE-EVIDENCE-INPUTS]
status: todo
...
non_goals:
  - lessons parser、探针 roster、与本次裁决证据无关的文档教学面或其它探针语义
  - 重写 PR #127 / #137 历史
...
acceptance:
  - "A1 portable selfcheck 证明当前 actual-root、两 comparer 纯语义与 portable 单文件证据身份；本卡 DoD 在已配置 runner 上以 require-dual-actual 证明 sensitive/insensitive 两种 actual-root，第二模式能力不可用时具名 FAIL；同来源冲突必选 block"
  - "A2 不同 root 使用不同 SHA 且 review→triage 精确绑定被测；worktree 优先于 local 的既有来源顺序保持"
  - "A3 相关证据 unreadable/unknown 时仍 exit 0 但产出明确 finding；删除任一守卫时自检必红"
...
doc_sync: docs/LOOP-ENGINEERING.md（实际目录身份、来源优先与 CaseModeProfile 验收）；合并后按既有 R5 更新卡片和当前阶段
---
...
以下 2026-09-09 各段原文逐字保留；其中“current-source”只指各段记录时所绑定的旧工作源，
不代表当前 master 或 PR #294 候选已 PASS。当前交付仍须按最终源码 SHA 重做 RED/GREEN、DoD、R4 与 R3。
...
## 语义拆分承接（协调任务已批准技术路线，非交付记录）

T0-TRIAGE-EVIDENCE-INPUTS 先交付既有 discovery/read/字段/HEAD 失败可观测性；本卡再交付实际目录 comparer、leaf/root 身份、稳定来源选择与 dual-actual。最终验收、全部历史记录及9个当前 oracle 均保留；R3 计数1与原始收据不重置。
本卡须在最终源码上重跑全部9项当前变异及历史19项语义、8项 Add-Finding AST 删除、1项 profile guard；按实际 source SHA 保存证据，不借用前置项或历史收据宣称完成。
