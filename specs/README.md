...
| `review_gate` | 默认 `codex {verdict:pass}`（blocking）；精确 `advisory` 只允许基线已授权且真实 diff 属于非权威竞品笔记闭集 的卡，边界见 `docs/QUALITY-RUBRIC.md` §0.1。有效负面意见保留 JSON `block`，运行故障仍停止 |
...
**TD 与任务卡不是 1:1**：TD 是「哪笔债何时算整体还清」的登记/验收单元；任务卡与 PR 是「一次能实现、验证、评审」
的交付单元。每个 TD 在 `open → carded` 前都要做一次 decomposition，产出 **1–N 张**依赖有序的卡；只有小债已经满足
一张卡的右尺寸标准时才取 N=1。`偿还指针` 仍用现有一格，写 card set 与顺序（例如
`T0-A → T0-B → T0-C`），不新增列。任一子卡只表示**部分偿还**；全部子卡 merged、TD 总验收通过并补齐所有 PR/commit
证据后才能 `paid`。这条规则防止登记层的一条宽债被硬塞进一张超大 PR，令 R3 无法一次读完或一次判完。
